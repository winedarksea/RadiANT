...
/*
 * ant_channel.c - the per-channel state machine, its configuration, and the
 * slot clock.
 *
 ...
 *    reached by one path.
 *
 * 2. NO HAL CALLS. Not ant_radio_now(), not ant_radio_abort(). Time arrives as
 *    an argument. That is what lets the state machine be tested as a pure
 *    function and what stops a channel from arming a window ant_sched.c wanted
 ...
 * complain. */
_Static_assert(ANT_CHANNEL_COUNT <= 32u,
	       "more channels than the burst header's five bits can address");
_Static_assert(ANT_CHANNEL_COUNT >= 1u, "a stack with no channels");

/* The state fits in the uint8_t the per-channel record stores it in. */
_Static_assert(ANT_CH_STATE_COUNT <= 255, "state enum outgrew its uint8_t");
...
/* flags */
#define CHF_ID_SET               0x01u /* ant_channel_id_set() has been called */
...
struct chan {
	 ...
	 * ant_radio_hal.h defines the term; using anything else here would put
	...
	uint8_t state;              /* enum ant_channel_state */
	uint8_t type;
	uint8_t network;
	uint8_t ext_assign;
	uint8_t device_type;
...
static struct chan channels[ANT_CHANNEL_COUNT];

/* Raised by ant_sched.c at init from caps.min_arm_lead_us. The protocol floor
 * is the default and is also the minimum a caller can set. */
static uint16_t period_floor = ANT_CHANNEL_PERIOD_MIN_COUNTS;
...
{
	return channel < ANT_CHANNEL_COUNT;
}
...
{
	return state == ANT_CH_STATE_SEARCHING ||
	       state == ANT_CH_STATE_TRACKING ||
	       state == ANT_CH_STATE_CLOSING;
}

static bool state_assigned(uint8_t state)
{
	return state != ANT_CH_STATE_UNASSIGNED;
}

static ant_time_t period_us(const struct chan *c)
{
	return ant_channel_counts_to_us(c->period);
}
 ...
 * sensor instead of being told ANT_CH_ERR_ID_NOT_SET. Every host library sets
...
static void chan_defaults(struct chan *c)
{
	...
	c->state = ANT_CH_STATE_UNASSIGNED;
	c->t_next = ANT_TIME_NEVER;
	c->t_search_start = ANT_TIME_NEVER;

	/*
	 * 8182 counts, the ANT+ 4.005 Hz default, measured here as a
	 * 249,696.4 us mean over 744 intervals. NOT 8192 - see the note in
	 * ant_channel.h. A default that is 305 us per slot away from what
	 ...
	 */
	c->period = ANT_CHANNEL_PERIOD_ANT_PLUS;
	...
	c->crc_mode = ANT_CH_CRC_MODE_DEFAULT;

	/* Level 3 is 0 dBm, the middle of the range and what the bench link
	 * was characterised at. */
	c->tx_power_level = 3u;

	c->search_timeout = ANT_CHANNEL_SEARCH_TIMEOUT_DEFAULT;
	c->lp_search_timeout = ANT_CHANNEL_LP_SEARCH_TIMEOUT_DEFAULT;
	c->waveform = ANT_CH_SEARCH_WAVEFORM_DEFAULT;
	c->sdu_mask_config = ANT_CH_SDU_MASK_OFF;
}
...
static void chan_finish_close(uint8_t channel, struct chan *c)
{
	...
	ant_channel_event_out(channel, ANT_CH_EVENT_CHANNEL_CLOSED);
}
...
{
	if (c->state == ANT_CH_STATE_TRACKING) {
...
/* The absolute instant a searching channel gives up, or ANT_TIME_NEVER.
 *
...
static ant_time_t search_deadline(const struct chan *c)
{
	ant_time_t budget;

	if (c->t_search_start == ANT_TIME_NEVER) {
		return ANT_TIME_NEVER;
	}
	if (c->search_timeout == ANT_CHANNEL_SEARCH_INFINITE ||
	    c->lp_search_timeout == ANT_CHANNEL_SEARCH_INFINITE) {
		return ANT_TIME_NEVER;
	}

	budget = ant_channel_search_ticks_to_us(c->search_timeout) +
		 ant_channel_search_ticks_to_us(c->lp_search_timeout);
...
void ant_channel_init(void)
{
	...
	period_floor = ANT_CHANNEL_PERIOD_MIN_COUNTS;
	stale_ops = 0u;
	search_timeouts = 0u;
}
...
void ant_channel_reset_all(void)
{
	...
	for (i = 0u; i < ANT_CHANNEL_COUNT; i++) {
		...
		if (state_on_air(c->state)) {
			ant_channel_event_out(i, ANT_CH_EVENT_CHANNEL_CLOSED);
		}
	 ...
	 * module cannot abort one. ant_sched.c calls ant_radio_abort() before
...
{
	if (counts > ANT_CHANNEL_PERIOD_MIN_COUNTS) {
		period_floor = counts;
	} else {
		period_floor = ANT_CHANNEL_PERIOD_MIN_COUNTS;
	}
				     ...
				     uint8_t network, uint8_t ext_assign)
{
	...
	if (!ch_valid(channel)) {
		return ANT_CH_ERR_INVALID_MESSAGE;
	}

	c = ch_at(channel);
	if (state_assigned(c->state)) {
		return ANT_CH_ERR_WRONG_STATE;
	}

	if (network >= ANT_CHANNEL_NETWORK_COUNT) {
		return ANT_CH_ERR_INVALID_NETWORK;
	}
...
ant_channel_err_t ant_channel_unassign(uint8_t channel)
{
	 ...
	 * ant_channel_status_get() keeps reporting SEARCHING/TRACKING until
	 * the close completes rather than reporting ASSIGNED and inviting the
	 * call this rejects.
	 */
	if (c->state != ANT_CH_STATE_ASSIGNED) {
		return ANT_CH_ERR_WRONG_STATE;
	}
				   ...
				   ant_time_t now)
{
	...
	if ((c->flags & CHF_ID_SET) == 0u) {
		return ANT_CH_ERR_ID_NOT_SET;
	}

	start = now + ant_channel_counts_to_us(offset);

	if ((c->type & ANT_CH_TYPE_MASTER_BIT) != 0u) {
		/*
		 ...
		 */
		c->t_next = start;
		c->state = ANT_CH_STATE_SEARCHING;
...
ant_channel_err_t ant_channel_close(uint8_t channel, ant_time_t now)
{
	 ...
	 * reopen twice.
	 */
	if (c->state != ANT_CH_STATE_SEARCHING &&
	    c->state != ANT_CH_STATE_TRACKING) {
		return ANT_CH_ERR_WRONG_STATE;
	}
...
static ant_channel_err_t require_assigned_closed(uint8_t channel,
						 struct chan **out)
{
	struct chan *c;
...
static ant_channel_err_t require_assigned(uint8_t channel, struct chan **out)
{
	struct chan *c;
...
static ant_channel_err_t require_range(uint8_t channel, struct chan **out)
{
	if (!ch_valid(channel)) {
		return ANT_CH_ERR_INVALID_MESSAGE;
	}
...
{
	struct chan *c = NULL;
	ant_channel_err_t err = require_assigned_closed(channel, &c);
...
{
	struct chan *c = NULL;
	ant_channel_err_t err = require_range(channel, &c);

	if (err != ANT_CH_OK) {
		return err;
	}
	if (out == NULL) {
		return ANT_CH_ERR_INVALID_PARAM;
	}
...
{
	struct chan *c = NULL;
	ant_channel_err_t err = require_assigned(channel, &c);
	 ...
	 * hardware has it is a caps question, and ant_sched.c clamps against
...
ant_channel_err_t ant_channel_crc_mode_set(uint8_t channel, uint8_t mode)
{
	 ...
	 */
	if (mode != ANT_CH_CRC_MODE_DEFAULT) {
		return ANT_CH_ERR_INVALID_PARAM;
	}
						 ...
						 uint8_t freq1, uint8_t freq2)
{
	...
	if (freq0 > ANT_RF_INDEX_MAX || freq1 > ANT_RF_INDEX_MAX ||
	    freq2 > ANT_RF_INDEX_MAX) {
		return ANT_CH_ERR_INVALID_PARAM;
	}
					      ...
					      uint8_t threshold, uint8_t custom)
{
	...
	if (threshold > ANT_CH_PROX_THRESHOLD_MAX) {
		return ANT_CH_ERR_INVALID_PARAM;
	}
...
ant_channel_err_t ant_channel_sharing_cycles_get(uint8_t channel, uint8_t *out)
{
	struct chan *c = NULL;
	ant_channel_err_t err = require_range(channel, &c);
 ...
 * byte that says ASSIGNED would invite an unassign that ant_channel_unassign()
 * then refuses. */
static uint8_t status_state_bits(const struct chan *c)
{
	switch (c->state) {
	case ANT_CH_STATE_UNASSIGNED:
		return ANT_CH_STATUS_UNASSIGNED;
	case ANT_CH_STATE_ASSIGNED:
		return ANT_CH_STATUS_ASSIGNED;
	case ANT_CH_STATE_SEARCHING:
		return ANT_CH_STATUS_SEARCHING;
	case ANT_CH_STATE_TRACKING:
		return ANT_CH_STATUS_TRACKING;
	case ANT_CH_STATE_CLOSING:
		return ((c->flags & CHF_CLOSING_FROM_TRACKING) != 0u)
			       ? ANT_CH_STATUS_TRACKING
...
ant_channel_err_t ant_channel_status_get(uint8_t channel, uint8_t *out)
{
	...
	status = (uint8_t)(c->type & ANT_CH_STATUS_TYPE_MASK);
	status |= (uint8_t)(((uint8_t)(c->network << ANT_CH_STATUS_NETWORK_SHIFT)) &
			    ANT_CH_STATUS_NETWORK_MASK);
	status |= (uint8_t)(status_state_bits(c) & ANT_CH_STATUS_STATE_MASK);
...
{
	if (!ch_valid(channel)) {
		return ANT_CH_STATE_UNASSIGNED;
	}

	return (enum ant_channel_state)channels[channel].state;
}
...
bool ant_channel_is_master(uint8_t channel)
{
	...
	return (c->type & ANT_CH_TYPE_MASTER_BIT) != 0u;
}
...
ant_time_t ant_channel_next_slot(uint8_t channel)
{
	...
	/*
	 * A CLOSING channel returns ANT_TIME_NEVER even though its clock still
...
{
	ant_time_t best = ANT_TIME_NEVER;
	uint8_t best_ch = 0u;
	bool found = false;
	uint8_t i;

	for (i = 0u; i < ANT_CHANNEL_COUNT; i++) {
		ant_time_t t = ant_channel_next_slot(i);
	...
	return found ? best : ANT_TIME_NEVER;
}
...
int ant_channel_op_owner(uint32_t op)
{
	...
	/* op 0 is not an operation - ant_radio_tx()/rx() only ever hand out
...
bool ant_channel_on_slot_missed(uint8_t channel, ant_time_t now)
{
	...
	if (c->miss_count < ANT_CHANNEL_RX_FAIL_TO_SEARCH) {
		return false;
	}
	...
	ant_channel_event_out(channel, ANT_CH_EVENT_RX_FAIL_GO_TO_SEARCH);
			    ...
			    ant_time_t now)
{
	...
	(void)status; /* The close completes whatever the terminal status was.
		       ...
		       * ant_sched.c's call - only it knows whether the window
		       * was merged with another channel's. */

	owner = ant_channel_op_owner(op);
	if (owner < 0) {
		/*
	...
	if (c->state == ANT_CH_STATE_CLOSING) {
...
uint32_t ant_channel_tick(ant_time_t now)
{
	...
	for (i = 0u; i < ANT_CHANNEL_COUNT; i++) {
		...
		if (deadline == ANT_TIME_NEVER || now < deadline) {
			continue;
		}
		 ...
		 */
		ant_channel_event_out(i, ANT_CH_EVENT_RX_SEARCH_TIMEOUT);
