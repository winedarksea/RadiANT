...
/*
 * ant_radio_null.c - a radio backend that has no radio.
 *
 * Provenance: clean-room. Written from ant_core/include/ant_radio_hal.h alone -
 ...
 * ---------------------------------------------------------------------------
 * ant_core is a link layer. It decides what goes on the air and when; a BACKEND
 * decides how. ant_radio_hal.h is the whole of that boundary and exactly one
 * implementation of it exists in this tree today - ant_core/tests/fake_radio.c,
 ...
 * module in the fan-out was asked to write: ant_radio_caps_get, _init, _enable,
 ...
 * beside it: two definitions of ant_radio_now() is a duplicate-symbol link
 * error, which is the right way for that mistake to be discovered.
 * ant_core/CMakeLists.txt selects between them with ANT_CORE_RADIO_BACKEND and
 * the comment there says so.
 *
 * ---------------------------------------------------------------------------
 * The capability block is not zeroed, and that is deliberate
 * ---------------------------------------------------------------------------
 * ant_search.c computes its sweep geometry from caps.max_filters (8 filters ->
 * 32 sets on nRF, 2 -> 128 on RAIL); ant_sched.c reads max_filters and
 * min_arm_lead_us on every pass; ant_transfer_init() refuses outright a backend
 * whose min_arm_lead_us + ANT_TRANSFER_ACK_GUARD_US exceeds
 * ANT_TRANSFER_REPLY_US, or whose rx_to_tx_us exceeds it. A backend advertising
 ...
 * false, because this backend captures neither. ant_event.c's default policy
...
static const enum ant_phy null_phys[] = { ANT_PHY_1M_GFSK };

static const struct ant_radio_caps null_caps = {
	.name = "null (no radio; ant_core link-layer bring-up only)",

	/* One base address plus eight prefixes. This is the number the whole
	 * of ant_search.c's sweep geometry and ant_sched.c's merge width are
	 * computed from, and getting it wrong here would change both. */
	.max_filters = 8u,
	.filter_wildcard_dev = false,
	.addr_len_hw_max = 5u,
	.max_body_len = ANT_RADIO_BODY_MAX,
	 ...
	 * because ant_transfer_init() checks the acknowledged-data budget
	 ...
	 * ant_transfer_init() requires this plus ANT_TRANSFER_ACK_GUARD_US
	 * (250) to stay under ANT_TRANSFER_REPLY_US (1560), so anything up to
	 ...
	 * this backend would report is an inference; ant_radio_hal.h's rule is
	 * to degrade the advertised accuracy rather than report the worse
	 * number as the good one, and ant_event.c honours that by suppressing
	 * the field.
	 */
...
static const struct ant_radio_cbs *null_cbs;
...
const struct ant_radio_caps *ant_radio_caps_get(void)
{
	 ...
	 * init" - ant_sched_init() and ant_search_init() both rely on the last
	 * clause. */
	return &null_caps;
}
...
ant_time_t ant_radio_now(void)
{
	 ...
	 * freely, ant_search.c credits dwell in microseconds, ant_channel.c
	 ...
	 */
	return (ant_time_t)k_ticks_to_us_floor64(k_uptime_ticks());
}
...
/*
 * ANT_RADIO_ENOTSUP, and specifically not ANT_RADIO_EIO or ANT_RADIO_ESTATE.
 *
 * The HAL defines ENOTSUP as "well-formed, but this backend cannot do it",
 * which is the literal truth, and ant_sched.c's arm_failed() treats it the way
 * that keeps a scheduling pass terminating: the request is CONSUMED and its
 * owner is told ANT_SCHED_DONE_FAILED. EBUSY and ESTATE describe the radio
...
{
	/* "Returns ANT_RADIO_OK_RC when there was nothing to do", and there
	 * never is. */
	(void)null_cbs;
	(void)null_user;
	return ANT_RADIO_OK_RC;
