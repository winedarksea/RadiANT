...
/*
 * ant_sched.c - the radio schedule.
 *
 * Provenance: clean-room. Written from ant_core/include/ant_radio_hal.h (the
 * frozen backend contract) and ant_core/include/ant_sched.h, against the mock
 * in ant_core/tests/fake_radio.c, with the two bounded constants in the header
 ...
 *   - ant_sched_tick(), the commit point for anything posted from thread
 *     context.
 *
 ...
 * want_preempt().
 *
 * A pass costs one ant_radio_now() and at most one arm call, whatever the state
 ...
 * ---------------------------------------------------------------------------
 * ant_radio_hal.h: an aborted operation's terminal event is STILL DELIVERED,
 ...
 * ant_radio_abort(), and every event handler's first act is to compare the
 ...
 * t_sync. t_end is the receive window's close, or ANT_TIME_NEVER for a
...
struct sched_slot {
	...
	const struct ant_rx_filter  *filters; /* receive; caller-owned */
...
static struct {
	...
	struct sched_slot ch[ANT_SCHED_MAX_CHANNELS];
	...
	ant_time_t                   armed_open;
	ant_time_t                   armed_end;
	const struct ant_pkt_format *armed_fmt;
	uint8_t                      armed_rf;
	uint8_t                      members[ANT_SCHED_MAX_FILTERS];
	uint8_t                      n_members;

	/*
	 * The filter array handed to ant_radio_rx(). The HAL requires it to
	 ...
	 */
	struct ant_rx_filter merged[ANT_SCHED_MAX_FILTERS];
	uint8_t              owner[ANT_SCHED_MAX_FILTERS];
	uint8_t              local[ANT_SCHED_MAX_FILTERS];
	uint8_t              n_merged;
...
static uint8_t max_filters(void)
{
	...
	if (mf > ANT_SCHED_MAX_FILTERS) {
		mf = ANT_SCHED_MAX_FILTERS;
	}
	return mf;
}

/* The slack budget: an arm call inside this window of now() fails
 * ANT_RADIO_ETIME rather than running late, so the scheduler subtracts it
 * itself and never asks for something the backend must refuse. */
static ant_time_t arm_lead(void)
{
	return (s.caps == NULL) ? 0u : (ant_time_t)s.caps->min_arm_lead_us;
}

static ant_time_t t_max(ant_time_t a, ant_time_t b)
{
	return (a > b) ? a : b;
}

static ant_time_t t_min(ant_time_t a, ant_time_t b)
{
	return (a < b) ? a : b;
}
...
static ant_time_t t_back(ant_time_t t, ant_time_t d)
{
	return (t > d) ? (t - d) : 0u;
}
...
static void notify_done(uint8_t ch, enum ant_sched_done why)
{
 ...
 * tidying up.
 */
static void end_armed(enum ant_sched_done why)
{
	uint8_t list[ANT_SCHED_MAX_FILTERS];
...
static bool abort_armed(uint8_t skip_ch)
{
	...
	started = s.armed_open <= ant_radio_now();
	...
	for (i = 0u; i < n; i++) {
		...
		slot_clear(c);
		if (c != skip_ch) {
			notify_done(c, ANT_SCHED_DONE_ABORTED);
		}
	}
	return started;
}
...
static enum step arm_failed(uint8_t ch, int rc)
{
	...
	case ANT_RADIO_ETIME:
		...
		s.stats.arm_rejected++;
		slot_clear(ch);
		notify_done(ch, ANT_SCHED_DONE_MISSED);
		return STEP_RETRY;
	default:
		s.stats.arm_rejected++;
		slot_clear(ch);
		notify_done(ch, ANT_SCHED_DONE_FAILED);
		return STEP_RETRY;
	}
}
 ...
 * one ant_radio_rx() carrying one filter each. Without it, the scheduler must
 ...
 *   4. THE UNION IS CAPPED. See ANT_SCHED_MERGE_SPAN_MAX_US - a merged window
 ...
 */
static enum step arm_rx_window(uint8_t leader, ant_time_t earliest,
			       ant_time_t limit)
{
	...
	ant_time_t           open;
	ant_time_t           close;
	ant_time_t           lead_open;
	ant_time_t           lead_close;
	ant_time_t           cap_close;
	uint32_t             flags = 0u;
	uint32_t             k;
	uint32_t             op = 0u;
	uint8_t              i;
	...
	if (lead->n_filters == 0u || lead->n_filters > mf) {
		...
		s.stats.arm_rejected++;
		slot_clear(leader);
		notify_done(leader, ANT_SCHED_DONE_FAILED);
		return STEP_RETRY;
	}
	...
	if (lead->continuous) {
		uint32_t chunk = (lead->chunk_us != 0u) ? lead->chunk_us
							: ANT_SCHED_SCAN_CHUNK_US;

		close = open + (ant_time_t)chunk;
	} else {
		close = lead->t_end;
	}
	if (limit != ANT_TIME_NEVER) {
	...
	cap_close = t_max(lead_close,
			  lead_open + (ant_time_t)ANT_SCHED_MERGE_SPAN_MAX_US);
	...
	if (lead->report_crc_fail) {
		flags |= ANT_RX_REPORT_CRC_FAIL;
	}

	for (k = 1u; k < ANT_SCHED_MAX_CHANNELS; k++) {
		uint8_t            c = (uint8_t)(((uint32_t)leader + k) %
						 ANT_SCHED_MAX_CHANNELS);
		...
		if (s.n_merged >= mf || s.n_members >= ANT_SCHED_MAX_FILTERS) {
			break;
		}
	...
	/*
	 * ANT_RX_STOP_ON_FIRST saves receive current on a single-master window
	...
	s.armed_stop_on_first = (flags & ANT_RX_STOP_ON_FIRST) != 0u;
...
static enum step arm_tx_op(uint8_t ch)
{
	...
	s.stats.tx_armed++;
	s.cursor = (uint8_t)(((uint32_t)ch + 1u) % ANT_SCHED_MAX_CHANNELS);
	return STEP_DONE;
}
 ...
 * per tick for ever.
 */
static bool could_join_armed(const struct sched_slot *sl, ant_time_t earliest)
{
	ant_time_t o;
	ant_time_t c;
	...
	c = sl->continuous ? ANT_TIME_NEVER : sl->t_end;
 ...
 * already owed.
 */
static bool want_preempt(ant_time_t earliest)
{
	const ant_time_t lead = arm_lead();
	uint32_t         i;
 ...
 */
static enum step arm_next(ant_time_t earliest)
{
	const ant_time_t lead = arm_lead();
	int              tx_ch = -1;
	int              rx_ch = -1;
	int              scan_ch = -1;
	ant_time_t       tx_at = ANT_TIME_NEVER;
	ant_time_t       rx_at = ANT_TIME_NEVER;
	ant_time_t       scan_at = ANT_TIME_NEVER;
	ant_time_t       committed;
	uint32_t         k;

	for (k = 0u; k < ANT_SCHED_MAX_CHANNELS; k++) {
	...
	if (scan_ch >= 0) {
		ant_time_t s_open = t_max(scan_at, earliest);
		ant_time_t s_limit = ANT_TIME_NEVER;
		uint32_t   chunk = s.ch[scan_ch].chunk_us;

		if (chunk == 0u) {
			chunk = ANT_SCHED_SCAN_CHUNK_US;
		}
		if (committed != ANT_TIME_NEVER) {
			s_limit = t_back(committed, lead);
		}
		if (s_limit == ANT_TIME_NEVER ||
		    s_limit >= s_open + (ant_time_t)ANT_SCHED_SCAN_MIN_US) {
			ant_time_t want = s_open + (ant_time_t)chunk;
			enum step  st;
	...
	if (rx_ch >= 0 && (tx_ch < 0 || rx_at <= tx_at)) {
		ant_time_t limit = ANT_TIME_NEVER;
		enum step  st;

		if (tx_ch >= 0) {
			limit = t_back(tx_at, lead);
		}
		if (limit == ANT_TIME_NEVER || limit >= rx_at) {
 ...
 * ANT_SCHED_DONE_MISSED at the moment it becomes unreachable, not silently
 ...
 */
static enum step pass_step(ant_time_t now)
{
	const ant_time_t earliest = now + arm_lead();
	uint32_t         i;
	...
	for (i = 0u; i < ANT_SCHED_MAX_CHANNELS; i++) {
		...
		if (expired) {
			...
			notify_done((uint8_t)i, ANT_SCHED_DONE_MISSED);
			return STEP_RETRY;
		}
	}
	...
	if (s.armed_kind != (uint8_t)SLOT_IDLE) {
		...
		if (abort_armed(ANT_SCHED_CH_NONE)) {
			s.stats.preempted++;
		} else {
			s.stats.replans++;
		}
 ...
 *
 * ant_radio_now() is read once and shared by every step: a pass reasons about
...
static void pass(void)
{
	...
	for (guard = 0u; guard < (2u * ANT_SCHED_MAX_CHANNELS) + 4u; guard++) {
		enum step st;
...
static void route_rx(const struct ant_rx_event *evt)
{
...
static void hal_rx(const struct ant_rx_event *evt, void *user)
{
	bool terminal = false;
	...
	case ANT_RADIO_STATUS_CRC_FAIL:
		route_rx(evt);
		/* With ANT_RX_STOP_ON_FIRST the first accepted frame's event is
		 ...
		 * academic. */
		if (evt->status == ANT_RADIO_STATUS_OK && s.armed_stop_on_first) {
			end_armed(ANT_SCHED_DONE_OK);
			terminal = true;
		}
		break;
	...
	case ANT_RADIO_STATUS_ABORTED:
		end_armed(ANT_SCHED_DONE_ABORTED);
		terminal = true;
		break;
	case ANT_RADIO_STATUS_FAILED:
	default:
		end_armed(ANT_SCHED_DONE_FAILED);
		terminal = true;
		break;
	}
...
static void hal_tx(const struct ant_tx_event *evt, void *user)
{
	uint8_t ch;
	...
	ch = (s.n_members > 0u) ? s.members[0] : ANT_SCHED_CH_NONE;

	s.in_pass = true;
	if (evt->status == ANT_RADIO_STATUS_OK && ch != ANT_SCHED_CH_NONE &&
	    s.cbs.tx != NULL) {
...
static const struct ant_radio_cbs sched_radio_cbs = {
	.rx = hal_rx,
	.tx = hal_tx,
};
...
int ant_sched_init(const struct ant_sched_cbs *cbs, void *user)
{
	...
	s.caps = ant_radio_caps_get();
	if (s.caps == NULL) {
		return ANT_RADIO_EIO;
	}
	if (cbs != NULL) {
		s.cbs = *cbs;
	}
...
int ant_sched_request_rx(uint8_t ch, const struct ant_sched_rx *req)
{
	...
	if (ch >= ANT_SCHED_MAX_CHANNELS || req == NULL) {
		return ANT_RADIO_EINVAL;
	}
	if (req->fmt == NULL || req->filters == NULL) {
		return ANT_RADIO_EINVAL;
	}
	if (req->n_filters == 0u || req->n_filters > ANT_SCHED_MAX_FILTERS) {
		return ANT_RADIO_EINVAL;
	}
	if (req->t_open == ANT_TIME_NEVER) {
		return ANT_RADIO_EINVAL;
	}
	if (req->t_close != ANT_TIME_NEVER && req->t_close < req->t_open) {
		return ANT_RADIO_EINVAL;
	}
	...
	sl->continuous = (req->t_close == ANT_TIME_NEVER);
...
{
	if (!s.inited || ch >= ANT_SCHED_MAX_CHANNELS) {
		return false;
	}
