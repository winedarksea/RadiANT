...
/*
 * ant_search.c - wildcard search: the sweep, the seen cache, and scan mode.
 *
 * Provenance: clean-room. Written from ant_core/include/ant_search.h (which
 ...
 *                       goes through ant_frame_addr() rather than laying out
 *                       three bytes by hand;
 *   handle_ok()       - devnum_lo comes from filter_lo[filter_index] and from
 *                       nowhere else;
 *   ant_search_on_rx()- ANT_RADIO_STATUS_CRC_FAIL is counted and dropped.
 *
 ...
 * and which cost a bench run - is a backend's problem, and ant_frame.h's
 * ant_addr_pack_leading() / ant_addr_pack_trailing() are where it is solved
 * once. This module speaks on-air byte order, exactly as struct ant_rx_filter
...
static bool inst_ok(const struct ant_search *s)
{
 ...
 */
static bool id_match(const struct ant_search_id_filter *want,
		     const struct ant_channel_id *id)
{
	...
	if (want->device_type != 0u &&
	    (uint8_t)(want->device_type & ANT_DEVICE_TYPE_MASK) !=
...
static bool seen_live(const struct ant_search *s, const struct ant_search_seen *e,
		      ant_time_t now)
{
	if (!e->valid) {
		return false;
	...
	return now <= e->t_seen + (ant_time_t)s->cfg.seen_lifetime_us;
}
 ...
 * tolerate.
 */
static void build_set(struct ant_search *s, uint16_t set)
{
	...
	for (uint8_t j = 0; j < s->n_filters; j++) {
		...
		rc = ant_frame_addr(ANT_FRAME_CFG_SEARCH, s->cfg.net_addr, &id,
				    addr, sizeof(addr));
		if (rc != (int)ANT_FRAME_ADDR_LEN_SEARCH) {
...
static void push_steer(struct ant_search *s, uint16_t set)
{
	if (set >= s->n_sets) {
		return;
	}
	...
	if (s->n_steer >= (uint8_t)ANT_SEARCH_SEEN_ENTRIES) {
		return;
	}
...
static void select_next_set(struct ant_search *s)
{
	uint16_t set;
	bool steered = false;
...
void ant_search_cfg_default(struct ant_search_cfg *cfg)
{
	...
	cfg->net_addr[0] = ANT_NET_ADDR_ANT_PLUS_0;
	cfg->net_addr[1] = ANT_NET_ADDR_ANT_PLUS_1;
	cfg->rf_index = ANT_RF_INDEX_ANT_PLUS;
	cfg->dwell_us = ANT_SEARCH_DWELL_DEFAULT_US;
	cfg->seen_lifetime_us = ANT_SEARCH_SEEN_LIFETIME_US;
	cfg->min_rssi_dbm = 0;
		    ...
		    const struct ant_search_cbs *cbs, void *user)
{
	...
	if (s == NULL || cfg == NULL) {
		return ANT_SEARCH_EINVAL;
	}
	if (cfg->rf_index > ANT_RF_INDEX_MAX || cfg->dwell_us == 0u ||
	    cfg->seen_lifetime_us == 0u) {
		return ANT_SEARCH_EINVAL;
	}

	fmt = ant_frame_format(ANT_FRAME_CFG_SEARCH);
	if (fmt == NULL) {
		return ANT_SEARCH_EINVAL;
	}
	 ...
	 * no field in struct ant_rx_filter in which to ask for that, so there
	...
	nf = caps->max_filters;
	if (nf > (uint8_t)ANT_SEARCH_MAX_FILTERS) {
		nf = (uint8_t)ANT_SEARCH_MAX_FILTERS;
	}
	 ...
	 */
	s->n_sets = (uint16_t)((ANT_SEARCH_LO_VALUES + nf - 1u) / nf);
	s->inited = true;
...
int ant_search_begin(struct ant_search *s, uint8_t channel,
		     ...
		     uint32_t timeout_us)
{
	struct ant_search_chan *ch;

	if (!inst_ok(s) || want == NULL) {
		return ANT_SEARCH_EINVAL;
	}
	if (channel >= (uint8_t)ANT_SEARCH_MAX_CHANNELS) {
		return ANT_SEARCH_ENOSPC;
	}
	if (mode != ANT_SEARCH_MODE_ACQUIRE && mode != ANT_SEARCH_MODE_SCAN) {
		return ANT_SEARCH_EINVAL;
	}
	...
	ch->mode = mode;
	ch->want = *want;
	ch->deadline = (timeout_us == ANT_SEARCH_TIMEOUT_NONE)
			       ? ANT_TIME_NEVER
			       : now + (ant_time_t)timeout_us;
	...
	if (want->device_number != 0u) {
		push_steer(s, ant_search_set_for_devnum(s, want->device_number));
	} else {
		struct ant_channel_id hit;

		if (ant_search_seen_lookup(s, want, now, &hit)) {
			push_steer(s, ant_search_set_for_devnum(s, hit.device_number));
		}
	}
...
{
	if (!inst_ok(s) || channel >= (uint8_t)ANT_SEARCH_MAX_CHANNELS) {
		return false;
	}
		      ...
		      struct ant_search_window *out)
{
	...
	if (s->op != 0u) {
		return ANT_SEARCH_ESTATE;
	}
	if (ant_search_n_searching(s) == 0u) {
		return ANT_SEARCH_ENOENT;
	}
	...
	out->t_close = t_earliest + (ant_time_t)remaining;
	/*
	 * ANT_RX_STOP_ON_FIRST is never set, and the HAL forbids it here. More
	 ...
	 */
	out->flags = s->cfg.report_crc_fail ? ANT_RX_REPORT_CRC_FAIL : 0u;
 ...
 */
static void handle_ok(struct ant_search *s, const struct ant_rx_event *evt,
		      uint8_t filter_index)
{
	struct ant_frame_wire w;
	struct ant_frame f;
	struct ant_channel_id lo_id;
	struct ant_search_result r;
	int rc;
	...
	if (evt->body == NULL || evt->body_len > (uint8_t)ANT_FRAME_BODY_MAX) {
	...
	memset(&w, 0, sizeof(w));
	rc = ant_frame_addr(ANT_FRAME_CFG_SEARCH, s->cfg.net_addr, &lo_id, w.addr,
			    sizeof(w.addr));
	if (rc != (int)ANT_FRAME_ADDR_LEN_SEARCH) {
		s->stats.decode_failed++;
		return;
	...
	/*
	 * FACT TWO. ANT_FRAME_TRUSTED_CRC, and it is not a shortcut. An
	 * ANT_RADIO_STATUS_OK event means the CRC was verified - by the
	 ...
	 * against. The flag is opt-out for the reason ant_frame.h gives: a
	 * caller who forgets it gets a loud rejection of a good frame, and only
	 * one of the two polarities has a discoverable failure.
	 */
	rc = ant_frame_decode(ANT_FRAME_CFG_SEARCH, &w, ANT_FRAME_TRUSTED_CRC, &f);
	if (rc != ANT_FRAME_OK) {
		s->stats.decode_failed++;
		return;
	}
	...
	s->stats.frames_ok++;
	ant_search_seen_note(s, &f.id, evt->t_sync);
	 ...
	 */
	for (uint8_t c = 0; c < (uint8_t)ANT_SEARCH_MAX_CHANNELS; c++) {
		struct ant_search_chan *ch = &s->chans[c];
		...
		if (ch->mode == ANT_SEARCH_MODE_ACQUIRE) {
 ...
 *
 * ANT_SEARCH_OP_EXTERNAL means the arming authority merged our request with
 ...
 * HAL case.
 */
static bool op_matches(const struct ant_search *s, uint32_t op)
{
	if (s->op == 0u) {
		return false;
	}
	if (s->op == ANT_SEARCH_OP_EXTERNAL) {
		return true;
	}
	return op == s->op;
}

static void on_event(struct ant_search *s, const struct ant_rx_event *evt,
		     uint8_t filter_index)
{
	switch (evt->status) {
	...
	case ANT_RADIO_STATUS_TIMEOUT:
		...
		ant_search_on_done(s, true);
		break;

	case ANT_RADIO_STATUS_ABORTED:
	case ANT_RADIO_STATUS_FAILED:
	default:
		ant_search_on_done(s, false);
		break;
	}
}
...
void ant_search_tick(struct ant_search *s, ant_time_t now)
{
	...
	for (uint8_t i = 0; i < (uint8_t)ANT_SEARCH_SEEN_ENTRIES; i++) {
	...
	for (uint8_t c = 0; c < (uint8_t)ANT_SEARCH_MAX_CHANNELS; c++) {
		...
		if (!ch->active || ch->deadline == ANT_TIME_NEVER) {
			continue;
		}
		if (now < ch->deadline) {
			continue;
...
static bool same_device(const struct ant_channel_id *a, const struct ant_channel_id *b)
{
	return a->device_number == b->device_number &&
	       (uint8_t)(a->device_type & ANT_DEVICE_TYPE_MASK) ==
		       (uint8_t)(b->device_type & ANT_DEVICE_TYPE_MASK) &&
			  ...
			  ant_time_t now)
{
	...
	s->seen_next = (uint8_t)((s->seen_next + 1u) % (uint8_t)ANT_SEARCH_SEEN_ENTRIES);
}
...
{
	const struct ant_search_seen *best = NULL;
	...
	for (uint8_t i = 0; i < (uint8_t)ANT_SEARCH_SEEN_ENTRIES; i++) {
		const struct ant_search_seen *e = &s->seen[i];
