...
/*
 * ant_api.c - the integration layer: src/ant_radio.h's fifty antr_* entry
 * points on top of the six ant_core modules, and the seams none of them could
 ...
 * six ant_core module headers this file composes. Nothing here derives from
 ...
 * ---------------------------------------------------------------------------
 * src/ant_wire.h must be in scope BEFORE ant_channel.h. That header spells its
 * response and event codes as literals so that it compiles against nothing but
 * ant_core/include, and guards a block of _Static_asserts on
 ...
 *   - the per-channel device ID list, because ant_search.c turned out not to
 ...
 *   - the event thread, and the surrogate operation ids that let ant_channel.c
 *     attribute a terminal event to a channel when the arming authority in
 *     between is ant_sched.c rather than the HAL.
 *
 * Everything else is a forward. Channel state, every configuration byte and the
 * slot clock are ant_channel.c's; the schedule is ant_sched.c's; the sweep is
 * ant_search.c's; the queue and the wire assembly are ant_event.c's; the
 * acknowledged/burst engine is ant_transfer's. Where a forward looks like it is
 * doing nothing, that is the shape working.
 *
 * ---------------------------------------------------------------------------
 * POSTING IS NOT COMMITTING
 * ---------------------------------------------------------------------------
 * ant_sched_request_rx()/_tx() change the plan; ant_sched_tick() acts on it.
 ...
 * meets the deadline.
 *
 * ant_sched.c owns no timer. A request that lost an arm race (ANT_RADIO_EBUSY)
 * or was posted before the radio was enabled (ANT_RADIO_ESTATE) stays pending
 * with nothing scheduled to retry it, so ANT_API_HOUSEKEEP_MS exists to bound
 * how long that can last.
 *
 * ---------------------------------------------------------------------------
 * ant_event_drain() IS THREAD CONTEXT ONLY
 * ---------------------------------------------------------------------------
 * docs/sdk-ant-contract.md forbids a backend calling antr_on_message()
 * recursively from inside a call the bridge made, and ant_event.h says the
...
LOG_MODULE_REGISTER(ant_api, LOG_LEVEL_INF);

/* ---------------------------------------------------------------------------
 * The agreement between src/ant_radio.h and ant_transfer.h
 *
 * ANT_TRANSFER_SEG_* were chosen equal to ANTR_BURST_SEGMENT_* so that
 * antr_burst_tx() forwards the byte untouched instead of remapping it, and
 * ant_transfer.h cannot include src/ant_radio.h to check that - it must compile
 ...
 * ---------------------------------------------------------------------------
 */
BUILD_ASSERT(ANT_TRANSFER_SEG_CONTINUE == ANTR_BURST_SEGMENT_CONTINUE,
	     "burst segment drift: CONTINUE");
BUILD_ASSERT(ANT_TRANSFER_SEG_START == ANTR_BURST_SEGMENT_START,
	     "burst segment drift: START");
BUILD_ASSERT(ANT_TRANSFER_SEG_END == ANTR_BURST_SEGMENT_END,
...
BUILD_ASSERT(ANT_CHANNEL_COUNT == ANT_SCHED_MAX_CHANNELS,
	     "channel count drift: ant_channel vs ant_sched");
BUILD_ASSERT(ANT_CHANNEL_COUNT <= (ANT_SEARCH_MAX_CHANNELS),
	     "channel count drift: ant_channel vs ant_search");
BUILD_ASSERT(ANT_CHANNEL_COUNT == (ANTW_BURST_HEADER_CHANNEL_MASK + 1),
	     "channel count drift: ant_channel vs the burst header");
...
#define API_CHANNELS ((uint8_t)ANT_CHANNEL_COUNT)
#define API_NETWORKS ((uint8_t)ANT_CHANNEL_NETWORK_COUNT)
 ...
 * comfortably inside ANT_SCHED_MERGE_SPAN_MAX_US, so two tracked channels
 * asking for nearby instants can still merge into one hardware window - which
 * is the whole return on the scheduler. ant_sched.c subtracts caps.ramp_up_us
...
/* The event thread. 1280 bytes covers ant_event_drain()'s frame - one queued
...
struct api_chan {
	/*
	 * The surrogate operation id bound into ant_channel.c.
	 *
	 * ant_channel.h asks for the id ant_radio_tx()/ant_radio_rx() returned,
	 * because a late terminal event has to be recognisable. With
	 * ant_sched.c between the two there is no such id to hand over - the
	 ...
	 * used only for attribution, which is all ant_channel.c does with it.
	 */
	uint32_t op;
	...
	struct ant_rx_filter filter;

	/* Likewise the transmit body: backends DMA straight out of it. */
	uint8_t tx_body[ANT_FRAME_BODY_MAX];
	uint8_t tx_body_len;
...
static struct ant_transfer api_xfer[API_CHANNELS];
static struct ant_search   api_search;

static uint8_t api_net_addr[API_NETWORKS][ANT_NET_ADDR_LEN];
...
static uint8_t  api_search_slot = ANT_SCHED_CH_NONE;
static bool     api_inited;

static struct ant_api_stats api_stats;
 ...
 * and both call ant_sched_tick(); without this they would interleave inside a
 ...
 * take a mutex. ant_sched.c's `in_pass` flag is what stands between the two,
...
/* ---------------------------------------------------------------------------
 * The three port hooks ant_event.c declares and deliberately does not define
 *
 ...
 * ever drains. ant_event.c includes no Zephyr header - it compiles as a
...
{
	/*
	 * O(1), ISR-safe, and it does not call back into ant_event - the "queue
...
/* ---------------------------------------------------------------------------
 * The one symbol ant_channel.c expects ant_event.c to define, and it does not
 *
 * ant_channel.c raises channel events through ant_channel_event_out();
 * ant_event.c spells the same operation ant_event_post_channel_event(). Neither
 ...
 * do. It may be called from a radio callback - ant_sched.c's context is a radio
...
{
	(void)ant_event_post_channel_event(channel, event_code);
}
...
{
	/* Never zero: ant_channel_op_owner() treats 0 as "no operation" so that
...
static void api_bind_accepted(uint8_t ch, enum api_slot_kind kind)
{
	...
	ant_channel_bind_op(ch, api_ch[ch].op);
}
...
static void api_bind_rejected(uint8_t ch)
{
	...
	ant_channel_bind_op(ch, 0u);
}

static const uint8_t *api_net_addr_for(uint8_t ch)
{
	uint8_t net = ant_channel_network_get(ch);
...
{
	uint8_t rf = ANT_RF_INDEX_ANT_PLUS;

	(void)ant_channel_rf_freq_get(ch, &rf);
	return rf;
}
 ...
 */
static struct ant_tx_power api_power_of(uint8_t ch)
{
	static const int8_t dbm[6] = { -20, -12, -4, 0, 4, 8 };
	struct ant_tx_power p;
	uint8_t level = 3u;
	uint8_t custom = 0u;

	memset(&p, 0, sizeof(p));
	(void)ant_channel_tx_power_get(ch, &level, &custom);
...
/* ant_transfer's return codes onto the wire, exactly as ant_transfer.h states
...
{
	switch (rc) {
	case ANT_TRANSFER_OK:
		return (antr_err_t)ANTW_RESPONSE_NO_ERROR;
	case ANT_TRANSFER_EBUSY:
		return (antr_err_t)ANTW_TRANSFER_IN_PROGRESS;
	case ANT_TRANSFER_ESTATE:
		return (antr_err_t)ANTW_CHANNEL_IN_WRONG_STATE;
	case ANT_TRANSFER_ESEQ:
		return (antr_err_t)ANTW_TRANSFER_SEQUENCE_NUMBER_ERROR;
	case ANT_TRANSFER_ENOTSUP:
		return (antr_err_t)ANTW_INVALID_PARAMETER_PROVIDED;
	case ANT_TRANSFER_EIO:
		return (antr_err_t)ANTW_TRANSFER_IN_ERROR;
	case ANT_TRANSFER_EINVAL:
	default:
		return (antr_err_t)ANTW_INVALID_MESSAGE;
	}
}

static enum ant_radio_status api_done_to_status(enum ant_sched_done why)
{
	switch (why) {
	case ANT_SCHED_DONE_OK:
		return ANT_RADIO_STATUS_TIMEOUT; /* the window closed */
	case ANT_SCHED_DONE_ABORTED:
		return ANT_RADIO_STATUS_ABORTED;
	case ANT_SCHED_DONE_MISSED:
	case ANT_SCHED_DONE_FAILED:
	default:
		return ANT_RADIO_STATUS_FAILED;
	}
}
 ...
 * The device ID list
 *
 * ant_search.c owns the sweep, the seen cache and one struct
 * ant_search_id_filter per channel - "what this channel is looking for" - but
 ...
 * ---------------------------------------------------------------------------
 */
static bool api_id_list_permits(uint8_t ch, const struct ant_channel_id *id)
{
	...
	for (i = 0u; i < c->id_list_size; i++) {
		...
		if (c->id_list[i][2] != 0u &&
		    (uint8_t)(c->id_list[i][2] & ANT_DEVICE_TYPE_MASK) !=
			    (uint8_t)(id->device_type & ANT_DEVICE_TYPE_MASK)) {
			continue;
		}
		if (c->id_list[i][3] != 0u &&
...
static void api_post_rx(uint8_t ch, uint8_t msg_id,
			...
			const struct ant_rx_event *hal, const uint8_t *payload,
			uint8_t payload_len, uint8_t burst_seq, bool burst_last)
{
	struct ant_event_rx r;
	...
	if (ant_event_post_rx(&r) == ANT_EVENT_OK) {
		api_stats.rx_posted++;
	} else {
...
static void api_xfer_done(void *ctx, const struct ant_transfer_result *r)
{
	 ...
	 */
	switch (r->ev) {
	case ANT_TRANSFER_EV_NEXT_BLOCK:
		(void)ant_event_post_transfer_next_block(ch);
		break;
	case ANT_TRANSFER_EV_TX_COMPLETED:
		(void)ant_event_post_transfer_tx_completed(ch);
		break;
	case ANT_TRANSFER_EV_TX_FAILED:
	default:
		(void)ant_event_post_transfer_tx_failed(ch);
		break;
	}
}
			     ...
			     bool last)
{
	...
	if (ant_channel_id_get(ch, &id) != ANT_CH_OK) {
		return;
	}
	...
	/*
	 * ant_event_post_rx() needs the HAL event, because the CRC gate and the
	 * 0xE0 RX timestamp both come out of it and there must be nowhere else
	 * to put a clock reading. ant_transfer_ops::rx_data does not carry one -
	...
	hal.status = ANT_RADIO_STATUS_OK;
	hal.t_sync = ant_radio_now();
	hal.t_sync_exact = false;

	api_post_rx(ch, msg_id, &id, &hal, payload, len, seq, last);
}

static bool api_xfer_broadcast(void *ctx, uint8_t out[ANT_TRANSFER_PKT_BYTES])
{
	uint8_t ch = (uint8_t)(uintptr_t)ctx;
	...
	memcpy(out, api_ch[ch].bcast, ANT_TRANSFER_PKT_BYTES);
	return true;
}

static const struct ant_transfer_ops api_xfer_ops = {
	.done = api_xfer_done,
...
static int api_xfer_setup(uint8_t ch)
{
	...
	cfg.id = id;
	memcpy(cfg.net_addr, api_net_addr_for(ch), ANT_NET_ADDR_LEN);
	cfg.rf_index = api_rf_of(ch);
	cfg.power = api_power_of(ch);
	/*
	 * There is no safe default for the slot bit and ant_transfer.h refuses
	 ...
	 */
	cfg.slot_opener = ant_channel_is_master(ch);
	cfg.channel = ch;
	cfg.ops = &api_xfer_ops;
	cfg.ctx = (void *)(uintptr_t)ch;

	return ant_transfer_init(&api_xfer[ch], &cfg);
}
...
static void api_search_cfg_build(struct ant_search_cfg *cfg)
{
	ant_search_cfg_default(cfg);
	/*
	 ...
	 */
	memcpy(cfg->net_addr, api_net_addr[0], ANT_NET_ADDR_LEN);
}
...
static void api_search_acquired(uint8_t channel,
				const struct ant_search_result *r, void *user)
{
	struct ant_rx_event hal;
	...
	ant_channel_on_acquired(channel, &r->id, r->t_sync);
	api_stats.acquired++;
	 ...
	 * ant_search_result carries the decoded frame and the timing but not
	...
	/*
	 * ANT_SEARCH_MODE_ACQUIRE says the channel leaves the search with an ID
	 * and belongs to the scheduler's tracked set from here on. Scan mode
	 * never leaves, and the state machine says which this is.
	 */
	if (ant_channel_state_get(channel) == ANT_CH_STATE_TRACKING) {
		(void)ant_search_end(&api_search, channel);
	}
}

/*
 * SEARCH TIMEOUT IS OWNED BY ant_channel.c, NOT HERE.
 *
 * It is expressible in both modules - ant_channel_search_deadline() and
 * ant_search_begin(timeout_us) - and two owners of one deadline is how a
 * channel times out twice or not at all. ant_channel.c wins because it already
 ...
 *
 * So every ant_search_begin() below passes ANT_SEARCH_TIMEOUT_NONE and this
...
static const struct ant_search_cbs api_search_cbs = {
	.acquired = api_search_acquired,
	.timeout = api_search_timeout,
};

static void api_search_begin(uint8_t ch, ant_time_t now)
{
	struct ant_search_id_filter want;
	struct ant_channel_id       id;
	enum ant_search_mode        mode;
	...
	mode = ((ant_channel_ext_assign_get(ch) &
		 API_EXT_ASSIGN_BACKGROUND_SCAN) != 0u)
		       ? ANT_SEARCH_MODE_SCAN
		       : ANT_SEARCH_MODE_ACQUIRE;

	(void)ant_search_begin(&api_search, ch, mode, &want, now,
			       ANT_SEARCH_TIMEOUT_NONE);
}
...
static void api_post_search_window(ant_time_t now)
{
	const struct ant_radio_caps *caps = ant_radio_caps_get();
	struct ant_search_window     w;
	struct ant_sched_rx          req;
	uint8_t                      ch;

	/* One sweep at a time - ant_search.c holds exactly one window in
	 * flight, which is what makes its filters[] pointer safe to hand
	 * straight to the HAL. */
	if (api_search_slot != ANT_SCHED_CH_NONE) {
		return;
	}

	/*
	 * ONE SWEEP SERVES EVERY SEARCHING CHANNEL - that is the point of
	 * ant_search.c and the difference between eight simultaneous searches
	...
	for (ch = 0u; ch < API_CHANNELS; ch++) {
		if (ant_search_is_searching(&api_search, ch)) {
			break;
		}
	}
	if (ch >= API_CHANNELS || ant_sched_pending(ch)) {
		return;
	}

	if (ant_search_window(&api_search,
			      now + (ant_time_t)caps->min_arm_lead_us,
			      &w) != ANT_SEARCH_OK) {
		return;
	}
	...
	req.report_crc_fail = (w.flags & ANT_RX_REPORT_CRC_FAIL) != 0u;
	/* Never ANT_RX_STOP_ON_FIRST on a search window: the HAL forbids it on
	 * anything carrying more than one filter, and several masters may
	 * transmit inside one dwell. */
	req.stop_on_first = false;

	if (ant_sched_request_rx(ch, &req) != ANT_RADIO_OK_RC) {
		ant_search_arm_failed(&api_search);
		return;
	}
	...
	/*
	 * ANT_SEARCH_OP_EXTERNAL: the scheduler merges several channels'
	 ...
	 */
	ant_search_armed(&api_search, ANT_SEARCH_OP_EXTERNAL, w.t_open,
			 w.t_close);
}
...
static void api_post_track_rx(uint8_t ch)
{
	struct ant_channel_id id;
	struct ant_sched_rx   req;
	ant_time_t            t = ant_channel_next_slot(ch);
	int                   n;
	...
	n = ant_frame_addr(ANT_FRAME_CFG_TRACKING, api_net_addr_for(ch), &id,
	...
	req.n_filters = 1u;
	req.t_open = (t > (ant_time_t)API_SLOT_GUARD_US)
			     ? (t - (ant_time_t)API_SLOT_GUARD_US)
			     : 0u;
	req.t_close = t + (ant_time_t)API_SLOT_GUARD_US;
	/*
...
static void api_post_master_tx(uint8_t ch)
{
	...
	struct ant_sched_tx    req;
	ant_time_t             t = ant_channel_next_slot(ch);

	if (t == ANT_TIME_NEVER || !api_ch[ch].bcast_valid) {
		return;
	}
	if (ant_channel_id_get(ch, &id) != ANT_CH_OK) {
		return;
	 ...
	 * than written as a literal so that ant_frame_make() gets to refuse a
	 * combination nothing has ever transmitted.
	 */
	memset(&fields, 0, sizeof(fields));
	fields.slot_opener = true;

	if (ant_frame_make(&f, &id, &fields, api_ch[ch].bcast,
			   ANT_FRAME_PAYLOAD_STD) != ANT_FRAME_OK) {
		return;
	}
	if (ant_frame_encode(ANT_FRAME_CFG_TRACKING, api_net_addr_for(ch), &f,
			     &w) != ANT_FRAME_OK) {
		return;
	}
	...
	if (ant_sched_request_tx(ch, &req) != ANT_RADIO_OK_RC) {
		api_bind_rejected(ch);
		return;
	}
...
static void api_pump_locked(void)
{
	...
	for (ch = 0u; ch < API_CHANNELS; ch++) {
		enum ant_channel_state st = ant_channel_state_get(ch);
		bool searching = ant_search_is_searching(&api_search, ch);
		...
		if (st == ANT_CH_STATE_SEARCHING) {
			 ...
			 * alone. ant_search_end() deliberately does not touch
			 ...
			 * set.
			 */
			(void)ant_search_end(&api_search, ch);
		}

		if (ant_sched_pending(ch)) {
			continue;
		}
		/* The transfer engine arms through the same slot, and it has
		 * the tighter deadline: 1.55 ms against a 250 ms period. */
		if (!ant_transfer_is_idle(&api_xfer[ch])) {
			continue;
		}

		if (st != ANT_CH_STATE_TRACKING) {
			continue;
		}
		if (ant_channel_is_master(ch)) {
...
static void api_tracked_frame(uint8_t ch, const struct ant_rx_event *evt)
{
	struct ant_channel_id id;
	struct ant_frame_wire w;
	struct ant_frame      f;
	 ...
	 * identity the match proved. ANT_FRAME_TRUSTED_CRC is right for every
	...
	if (ant_frame_decode(ANT_FRAME_CFG_TRACKING, &w, ANT_FRAME_TRUSTED_CRC,
			     &f) != ANT_FRAME_OK) {
		return;
	}
			 ...
			 const struct ant_rx_event *evt, void *user)
{
	...
	if (api_ch[ch].slot_kind == (uint8_t)API_SLOT_SEARCH) {
		/*
		 * ant_search_on_rx_indexed(), NEVER ant_search_on_rx().
		 *
		 ...
		 */
		ant_search_on_rx_indexed(&api_search, evt, filter_index);
		return;
	}

	if (evt->status != ANT_RADIO_STATUS_OK) {
	 ...
	 *
	 * ant_channel.h asks for ant_channel_op_owner(evt->op). evt->op is the
	 * HAL's id and ant_channel.c holds the surrogate this file minted, so
	 ...
	 * See the report - the substitution is forced by ant_sched.c sitting
	 * between the HAL and ant_channel.c, and it is checked rather than
	 * assumed for the same reason the original was.
	 */
	if (ant_channel_op_owner(api_ch[ch].op) != (int)ch) {
		return;
	}
	...
	if (!ant_transfer_is_idle(&api_xfer[ch])) {
		ant_transfer_on_rx_event(&api_xfer[ch], evt);
		return;
	}
...
static void api_sched_tx(uint8_t ch, const struct ant_tx_event *evt, void *user)
{
	(void)user;
	...
	if (!ant_transfer_is_idle(&api_xfer[ch])) {
		ant_transfer_on_tx_event(&api_xfer[ch], evt);
		return;
	}
	...
	ant_channel_on_slot(ch, evt->t_sync);
	(void)ant_event_post_channel_event(ch, (uint8_t)ANTW_EVENT_TX);
}
 ...
 */
static void api_feed_xfer_terminal(uint8_t ch, enum ant_radio_status st)
{
	switch (ant_transfer_state(&api_xfer[ch])) {
	case ANT_TRANSFER_STATE_TX_DATA:
	case ANT_TRANSFER_STATE_TX_REPLY:
	case ANT_TRANSFER_STATE_ABORTING: {
		struct ant_tx_event e;

		memset(&e, 0, sizeof(e));
		e.op = ANT_TRANSFER_OP_EXTERNAL;
		e.status = st;
		e.t_sync = ant_radio_now();
		e.t_sync_exact = false;
		ant_transfer_on_tx_event(&api_xfer[ch], &e);
		break;
	}
	case ANT_TRANSFER_STATE_WAIT_ACK: {
		 ...
		 * ANT_RADIO_STATUS_TIMEOUT, which the engine reads as
		 * ANT_TRANSFER_FAIL_NO_ACK and does NOT retry - what a data
		...
		e.status = st;
		e.t_sync = ant_radio_now();
		ant_transfer_on_rx_event(&api_xfer[ch], &e);
		break;
	}
	case ANT_TRANSFER_STATE_IDLE:
	case ANT_TRANSFER_STATE_WAIT_BLOCK:
	default:
		break;
	}
}

static void api_sched_done(uint8_t ch, enum ant_sched_done why, void *user)
{
	enum ant_radio_status st;
	ant_time_t            now;
	uint32_t              op;
	...
	if (why == ANT_SCHED_DONE_MISSED) {
		api_stats.sched_missed++;
	} else if (why == ANT_SCHED_DONE_FAILED) {
	...
	if (api_ch[ch].slot_kind == (uint8_t)API_SLOT_SEARCH) {
		...
		ant_search_on_done(&api_search, why == ANT_SCHED_DONE_OK);
		if (api_search_slot == ch) {
			api_search_slot = ANT_SCHED_CH_NONE;
		}
	} else if (!ant_transfer_is_idle(&api_xfer[ch]) &&
		   !ant_sched_pending(ch)) {
		/*
		...
		if (!ant_channel_on_slot_missed(ch, now)) {
			(void)ant_event_post_channel_event(
	 ...
	 */
	(void)ant_channel_on_terminal(op, st, now);
...
static const struct ant_sched_cbs api_sched_cbs = {
	.rx = api_sched_rx,
	.tx = api_sched_tx,
	.done = api_sched_done,
};
...
static void api_housekeep(void)
{
	...
	(void)ant_channel_tick(now);

	/* Seen-cache expiry. The timeout half cannot fire - ant_channel.c owns
	 * the deadline; see api_search_timeout(). */
	ant_search_tick(&api_search, now);
...
static void api_event_thread(void *a, void *b, void *c)
{
	...
	for (;;) {
		...
		/*
		 * The timeout is what drives housekeeping. ant_sched.c owns no
		 * timer, so a request that lost an arm race has nothing else to
		 * retry it, and ant_channel_tick() has no other caller.
		 */
		(void)k_sem_take(&api_event_sem, K_MSEC(ANT_API_HOUSEKEEP_MS));
		 ...
		 * the pump - ant_event.c protects its own ring with
		 * ant_event_crit_enter().
		 */
		n = ant_event_drain(0u);
		if (n != 0u) {
			api_stats.drains++;
...
antr_err_t antr_init(void)
{
	 ...
	 *
	 * ant_event_init() first, and before the radio is enabled: posting
	 * before it returns is ANT_EVENT_ESTATE, which mirrors the rule that
	 ...
	 * this file's. */
	rc = ant_sched_init(&api_sched_cbs, NULL);
	if (rc != ANT_RADIO_OK_RC) {
		LOG_ERR("ant_sched_init: %d", rc);
		return (antr_err_t)ANTW_INVALID_PARAMETER_PROVIDED;
	}

	rc = ant_radio_init(ant_sched_radio_cbs(), NULL);
	if (rc != ANT_RADIO_OK_RC) {
		LOG_ERR("ant_radio_init: %d", rc);
		return (antr_err_t)ANTW_INVALID_PARAMETER_PROVIDED;
	}
	rc = ant_radio_enable();
	if (rc != ANT_RADIO_OK_RC) {
		LOG_ERR("ant_radio_enable: %d", rc);
		return (antr_err_t)ANTW_INVALID_PARAMETER_PROVIDED;
	}

	api_search_cfg_build(&scfg);
	rc = ant_search_init(&api_search, &scfg, &api_search_cbs, NULL);
	if (rc != ANT_SEARCH_OK) {
		/* ANT_SEARCH_ENOTSUP means the backend advertises a wildcard
		 * device-number filter, which struct ant_rx_filter has no field
		 * to ask for. Failing loudly is the correct response to a
		 * capability the contract cannot express. */
		LOG_ERR("ant_search_init: %d", rc);
		return (antr_err_t)ANTW_INVALID_PARAMETER_PROVIDED;
	}
	...
	(void)k_thread_name_set(&api_event_thread_data, "ant_event");

	LOG_INF("ant_core up: %u channels, %u filters/window, %u search sets",
		(unsigned int)API_CHANNELS,
		(unsigned int)ant_search_filters_per_window(&api_search),
		(unsigned int)ant_search_sets(&api_search));
...
antr_err_t antr_stack_reset(void)
{
	...
	/*
	 * ant_radio_abort() BEFORE ant_channel_reset_all(). ant_channel.c has
	 ...
	 * exists. ant_sched_reset() is that abort - it retires the operation
	...
	for (i = 0u; i < API_CHANNELS; i++) {
		(void)ant_transfer_abort(&api_xfer[i]);
		(void)ant_search_end(&api_search, i);
	}
	...
	api_search_cfg_build(&scfg);
	(void)ant_search_init(&api_search, &scfg, &api_search_cbs, NULL);
			       ...
			       uint8_t ext_assign)
{
	...
	k_mutex_lock(&api_lock, K_FOREVER);
	rc = ant_channel_assign(channel, type, network, ext_assign);
	if (rc == ANT_CH_OK) {
...
antr_err_t antr_channel_unassign(uint8_t channel)
{
	...
	k_mutex_lock(&api_lock, K_FOREVER);
	rc = ant_channel_unassign(channel);
	if (rc == ANT_CH_OK) {
		(void)ant_search_end(&api_search, channel);
		(void)ant_sched_cancel(channel);
		memset(&api_ch[channel], 0, sizeof(api_ch[channel]));
...
antr_err_t antr_channel_open_with_offset(uint8_t channel, uint16_t offset)
{
	...
	k_mutex_lock(&api_lock, K_FOREVER);
	rc = ant_channel_open(channel, offset, ant_radio_now());
	if (rc == ANT_CH_OK) {
		int trc = api_xfer_setup(channel);

		if (trc != ANT_TRANSFER_OK) {
			/*
			 * ant_transfer_init() refuses a backend that cannot
			 * turn a frame round inside ANT_TRANSFER_REPLY_US.
...
antr_err_t antr_channel_close(uint8_t channel)
{
	...
	k_mutex_lock(&api_lock, K_FOREVER);
	rc = ant_channel_close(channel, ant_radio_now());
	if (rc == ANT_CH_OK) {
		uint32_t op = api_ch[channel].op;

		(void)ant_transfer_abort(&api_xfer[channel]);
		(void)ant_search_end(&api_search, channel);
		if (api_search_slot == channel) {
			 ...
			 * will tell ant_search.c that its window is over.
			 ...
			 * channel.
			 */
			ant_search_on_done(&api_search, false);
			api_search_slot = ANT_SCHED_CH_NONE;
		}

		/*
		 * ant_sched_cancel() is deliberately SILENT for the channel it
		 * cancels - the caller already knows - so no done() arrives and
		 * the terminal event ant_channel.c is waiting on to complete
		...
		(void)ant_channel_on_terminal(op, ANT_RADIO_STATUS_ABORTED,
					      ant_radio_now());
...
antr_err_t antr_enhanced_channel_spacing_enable(uint8_t enable)
{
	 ...
	 * number, and one no ant_core module models. Accepting enable == 0 -
			    ...
			    uint8_t index)
{
	...
	if (ant_channel_is_open(channel)) {
		return (antr_err_t)ANTW_CHANNEL_IN_WRONG_STATE;
	}
...
antr_err_t antr_network_address_set(uint8_t network, const uint8_t *key)
{
	...
	k_mutex_lock(&api_lock, K_FOREVER);
	api_net_addr[network][0] = ANT_NET_ADDR_ANT_PLUS_0;
	api_net_addr[network][1] = ANT_NET_ADDR_ANT_PLUS_1;

	/*
	 * The sweep's address is fixed when ant_search_init() runs, and
	 ...
	 */
	if (network == 0u && ant_search_n_searching(&api_search) == 0u) {
		struct ant_search_cfg scfg;

		api_search_cfg_build(&scfg);
		(void)ant_search_init(&api_search, &scfg, &api_search_cbs,
				      NULL);
		api_search_slot = ANT_SCHED_CH_NONE;
	}
	k_mutex_unlock(&api_lock);
...
antr_err_t antr_sdu_mask_config(uint8_t channel, uint8_t mask_config)
{
	...
	return ant_channel_sdu_mask_config_set(channel, mask_config);
}
				     ...
				     const uint8_t *data)
{
	 ...
	 * transmit.
	 */
	if ((uint8_t)(ant_channel_type_get(channel) &
		      (uint8_t)ANT_CH_STATUS_TYPE_MASK) ==
	    (uint8_t)ANTW_CHANNEL_TYPE_SLAVE_RX_ONLY) {
		return (antr_err_t)ANTW_CHANNEL_IN_WRONG_STATE;
	}

	k_mutex_lock(&api_lock, K_FOREVER);
	if (!ant_transfer_is_idle(&api_xfer[channel])) {
		k_mutex_unlock(&api_lock);
		return (antr_err_t)ANTW_TRANSFER_IN_PROGRESS;
	}
				       ...
				       const uint8_t *data)
{
	...
	if (!api_ch_valid(channel) || data == NULL ||
	    size != (uint8_t)ANT_TRANSFER_PKT_BYTES) {
		return (antr_err_t)ANTW_INVALID_MESSAGE;
	}
	if (!ant_channel_is_open(channel)) {
		return (antr_err_t)ANTW_CHANNEL_NOT_OPENED;
	...
	k_mutex_lock(&api_lock, K_FOREVER);
	/*
	 * ANT_TIME_NEVER: "as soon as the backend can". The engine resolves it
	 ...
	 * refused with ANT_RADIO_ETIME.
	 */
	rc = ant_transfer_ack_data(&api_xfer[channel], data, size,
				   ANT_TIME_NEVER);
	err = api_xfer_err(rc);
			 ...
			 uint8_t segment)
{
	...
	k_mutex_lock(&api_lock, K_FOREVER);
	/*
	 * The segment byte is FORWARDED UNTOUCHED. ANT_TRANSFER_SEG_* were
	 ...
	 */
	rc = ant_transfer_burst_block(&api_xfer[channel], data, (uint8_t)size,
				      segment, ANT_TIME_NEVER);
	err = api_xfer_err(rc);
	if (rc == ANT_TRANSFER_OK) {
		api_pump_locked();
	}
...
antr_err_t antr_capabilities_get(uint8_t *capabilities)
{
	 ...
	 * count ant_channel.c, ant_sched.c and ant_search.c are all sized for
	...
	capabilities[ANTW_CAPABILITIES_OFFSET_ADVANCED_OPTIONS_2] =
		(uint8_t)(ANTW_CAPABILITIES_EXT_MESSAGE_ENABLED |
			  /* BACKGROUND SCAN. ant_search.c implements scan mode
	...
	capabilities[ANTW_CAPABILITIES_OFFSET_ADVANCED_OPTIONS_3] =
		(uint8_t)(ANTW_CAPABILITIES_ADVANCED_BURST_ENABLED |
			   ...
			   * an A/B diff does not turn on it. What ant_core
...
{
	return ant_channel_status_get(channel, status);
}
			       ...
			       uint8_t *device_type, uint8_t *trans_type)
{
	...
	rc = ant_channel_id_get(channel, &id);
	if (rc != ANT_CH_OK) {
		return rc;
	}
...
const struct ant_api_stats *ant_api_stats_get(void)
{
	return &api_stats;
}

bool ant_api_ready(void)
{
	return api_inited;
}
