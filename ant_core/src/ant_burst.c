...
/*
 * ant_burst.c - the originator's stop-and-wait state machine, and the block
 ...
 * B1-B5 in src/ant_radio.h, and from ant_core/include/ant_radio_hal.h. Nothing
 ...
 *       -I ant_core/include -I ant_core/tests -fsyntax-only ant_core/src/ant_burst.c
 *
 * ---------------------------------------------------------------------------
 ...
 * from inside it.
 *
 * "Arm" means POST TO ant_sched.c, not call the HAL. ant_sched.c is the single
 * arming authority and the only place in ant_core that calls ant_radio_tx() or
 * ant_radio_rx(); this file used to call both, and the two sites - the shared
 * ant_transfer_arm_tx() and the static arm_ack_window() below - were redirected
 * when the two modules first met. See "One radio, one arming authority" in
 * ant_transfer.h for what that costs and what it buys.
 *
 * ---------------------------------------------------------------------------
 ...
 * ant_core/tests/src/test_transfer.c counts released blocks against accepted
...
static void reset_transfer(struct ant_transfer *t)
{
	t->state = ANT_TRANSFER_STATE_IDLE;
	t->op = 0u;
	t->block = NULL;
	t->block_len = 0u;
	t->block_seg = 0u;
 ...
 */
static void release_next(struct ant_transfer *t)
{
	struct ant_transfer_result r;
	const struct ant_transfer_ops *ops = t->cfg.ops;
	void *ctx = t->cfg.ctx;
	...
	memset(&r, 0, sizeof(r));
	r.ev = ANT_TRANSFER_EV_NEXT_BLOCK;
	r.block = t->block;
	r.block_len = t->block_len;
	r.fail = ANT_TRANSFER_FAIL_NONE;
...
static void finish(struct ant_transfer *t, enum ant_transfer_event ev,
		   enum ant_transfer_fail fail)
{
	...
	if (ev == ANT_TRANSFER_EV_TX_COMPLETED) {
			...
			ant_time_t t_sync_at)
{
	...
	if (t_sync_at == ANT_TIME_NEVER) {
		return ANT_TRANSFER_EINVAL;
	}

	n = ant_transfer_build_body(&t->cfg, ctrl, payload, payload_len,
	...
	/*
	 * THE SCHEDULER, NOT THE RADIO. ant_sched.c is the single arming
	 * authority - see "One radio, one arming authority" in ant_transfer.h.
	 ...
	 * ant_sched_tick() the thread-context caller owes.
	 */
	rc = ant_sched_request_tx(t->cfg.channel, &req);
	if (rc != ANT_RADIO_OK_RC) {
		/*
		 * The scheduler's codes are the HAL's, deliberately. A
		 * malformed request is ANT_RADIO_EINVAL and posting before
		 * ant_sched_init() is ANT_RADIO_ESTATE; neither is something a
 ...
 * rather than the standard deviation, and ANT_RX_STOP_ON_FIRST because exactly
 ...
 */
static int arm_ack_window(struct ant_transfer *t, ant_time_t t_data_sync)
{
	...
	req.n_filters = 1u;
	req.t_open = t_data_sync + (ant_time_t)ANT_TRANSFER_REPLY_US -
		     (ant_time_t)ANT_TRANSFER_ACK_GUARD_US;
	req.t_close = t_data_sync + (ant_time_t)ANT_TRANSFER_REPLY_US +
		      (ant_time_t)ANT_TRANSFER_ACK_GUARD_US;
	/*
	...
	rc = ant_sched_request_rx(t->cfg.channel, &req);
	if (rc != ANT_RADIO_OK_RC) {
		return ANT_TRANSFER_EIO;
	}
...
static int try_send(struct ant_transfer *t, uint32_t index, bool last,
		    const uint8_t *payload, ant_time_t at)
{
	uint8_t ctrl;
	int rc;

	rc = ant_transfer_ctrl(index, last, t->cfg.slot_opener, &ctrl);
	if (rc != ANT_TRANSFER_OK) {
		return rc;
	}
	rc = ant_transfer_arm_tx(t, ctrl, payload, ANT_TRANSFER_PKT_BYTES, at);
	if (rc != ANT_TRANSFER_OK) {
		return rc;
	}

	t->cur_ctrl = ctrl;
	t->state = ANT_TRANSFER_STATE_TX_DATA;
	return ANT_TRANSFER_OK;
}
...
{
	const struct ant_radio_caps *caps;
	int n;
	...
	if (cfg->rf_index > ANT_RF_INDEX_MAX) {
		return ANT_TRANSFER_EINVAL;
	}
	/* The engine posts against this slot for the life of the channel, and
	 * ant_sched_request_*() would answer ANT_RADIO_EINVAL for an
	 * out-of-range one on every packet rather than once, here. */
	if (cfg->channel >= (uint8_t)ANT_SCHED_MAX_CHANNELS) {
		return ANT_TRANSFER_EINVAL;
	}
	 ...
	 */
	n = ant_frame_addr(ANT_FRAME_CFG_TRACKING, t->cfg.net_addr, &t->cfg.id,
	 ...
	 */
	if ((uint32_t)caps->min_arm_lead_us + (uint32_t)ANT_TRANSFER_ACK_GUARD_US >
	    (uint32_t)ANT_TRANSFER_REPLY_US) {
		return ANT_TRANSFER_ENOTSUP;
	}
	if ((uint32_t)caps->rx_to_tx_us > (uint32_t)ANT_TRANSFER_REPLY_US) {
		return ANT_TRANSFER_ENOTSUP;
	}
			...
			uint8_t segment, bool owned, ant_time_t t_sync_at)
{
	...
	if ((segment & (uint8_t)~ANT_TRANSFER_SEG_MASK) != 0u) {
		return ANT_TRANSFER_EINVAL;
	}

	n_pkts = ant_transfer_packets_in_block(len);
	if (n_pkts == 0u) {
	...
	if (t->state == ANT_TRANSFER_STATE_IDLE) {
		if ((segment & ANT_TRANSFER_SEG_START) == 0u) {
			return ANT_TRANSFER_ESEQ;
		}
		starting = true;
		index = 0u;
	} else if (t->state == ANT_TRANSFER_STATE_WAIT_BLOCK) {
		if ((segment & ANT_TRANSFER_SEG_START) != 0u) {
			return ANT_TRANSFER_ESEQ;
		}
		...
		return ANT_TRANSFER_EBUSY;
	}

	/* Only the final packet of an END block is the last of the transfer. */
	last = ((segment & ANT_TRANSFER_SEG_END) != 0u) && (n_pkts == 1u);

	if (t_sync_at != ANT_TIME_NEVER) {
		when = t_sync_at;
	} else if (!starting && t->next_t_sync != ANT_TIME_NEVER) {
		/* The continuation instant the acknowledgement already fixed:
		 * ack t_sync + 1550 us. */
		when = t->next_t_sync;
	} else {
		when = ant_radio_now() + (ant_time_t)t->min_lead_us +
		       (ant_time_t)ANT_TRANSFER_ARM_SLACK_US;
	}
	/*
	 ...
	 */
	if (!starting) {
		ant_time_t earliest = ant_radio_now() +
				      (ant_time_t)t->min_lead_us +
				      (ant_time_t)ANT_TRANSFER_ARM_SLACK_US;
...
int ant_transfer_abort(struct ant_transfer *t)
{
	...
	case ANT_TRANSFER_STATE_WAIT_BLOCK:
		...
		finish(t, ANT_TRANSFER_EV_TX_FAILED, ANT_TRANSFER_FAIL_ABORTED);
		return ANT_TRANSFER_OK;
	 ...
	 * handler below finishes it. From thread context ant_radio_abort()
	 ...
	 */
	/*
	 * ant_radio_abort(), NOT ant_sched_cancel(), and the asymmetry with the
	 * two arm sites is deliberate rather than an oversight.
	 *
	 * ant_sched_cancel() is documented as SILENT for the channel it
	 ...
	 * rest of their window and are told ANT_SCHED_DONE_ABORTED. That is
	 ...
	 * ant_sched.h, which this change was not authorised to add.
	 */
	t->state = ANT_TRANSFER_STATE_ABORTING;
 ...
 * proved.
 *
 * ANT_FRAME_TRUSTED_CRC is right for every delivered event, not only on a
 ...
 */
static bool decode_rx(const struct ant_transfer *t,
		      const struct ant_rx_event *e, struct ant_frame *out)
{
	...
	return ant_frame_decode(ANT_FRAME_CFG_TRACKING, &w,
				ANT_FRAME_TRUSTED_CRC, out) == ANT_FRAME_OK;
}

/* The acknowledgement arrived and matched. Move on. */
static void advance(struct ant_transfer *t, ant_time_t ack_t_sync)
{
	int rc;
	bool last;
	...
	if (t->pkt_in_block >= t->n_pkt_in_block) {
		 ...
		 */
		t->state = ANT_TRANSFER_STATE_WAIT_BLOCK;
		t->next_t_sync = ack_t_sync +
				 (ant_time_t)ANT_TRANSFER_NEXT_PACKET_US;
		release_next(t);
		return;
	}
	...
	last = ((t->block_seg & ANT_TRANSFER_SEG_END) != 0u) &&
	       (t->pkt_in_block + 1u == t->n_pkt_in_block);

	rc = try_send(t, t->pkt_index, last,
		      &t->block[(size_t)t->pkt_in_block * ANT_TRANSFER_PKT_BYTES],
		      ack_t_sync + (ant_time_t)ANT_TRANSFER_NEXT_PACKET_US);
	if (rc != ANT_TRANSFER_OK) {
		finish(t, ANT_TRANSFER_EV_TX_FAILED, ANT_TRANSFER_FAIL_RADIO);
	}
}
			      ...
			      const struct ant_tx_event *e)
{
	...
	/*
	 * ANT_TRANSFER_OP_EXTERNAL means the arming authority has no HAL
	 * operation id to compare against and has already routed this event to
	 * the channel that owns it - see ant_transfer.h. Any other non-zero
	 ...
	 */
	if (t->op == 0u ||
	    (t->op != ANT_TRANSFER_OP_EXTERNAL && e->op != t->op)) {
		t->stats.late_events++;
		return;
	}

	if (t->state == ANT_TRANSFER_STATE_ABORTING) {
		finish(t, ANT_TRANSFER_EV_TX_FAILED, ANT_TRANSFER_FAIL_ABORTED);
		return;
	}

	if (t->state == ANT_TRANSFER_STATE_TX_REPLY) {
		/* Our acknowledgement of somebody else's data packet. Nothing
		 * follows it: the peer either sends the next packet or does
		 * not, and ant_sched.c owns the retransmission timer. */
		t->op = 0u;
		t->state = ANT_TRANSFER_STATE_IDLE;
	...
	if (t->state != ANT_TRANSFER_STATE_TX_DATA) {
		t->stats.late_events++;
		return;
	}
	 ...
	 */
	if (arm_ack_window(t, e->t_sync) != ANT_TRANSFER_OK) {
		finish(t, ANT_TRANSFER_EV_TX_FAILED, ANT_TRANSFER_FAIL_RADIO);
		return;
	}
	t->state = ANT_TRANSFER_STATE_WAIT_ACK;
}
			      ...
			      const struct ant_rx_event *e)
{
	...
	/* See the note on the same test in ant_transfer_on_tx_event(). */
	if (t->op == 0u ||
	    (t->op != ANT_TRANSFER_OP_EXTERNAL && e->op != t->op)) {
		t->stats.late_events++;
		return;
	...
	if (t->state != ANT_TRANSFER_STATE_WAIT_ACK) {
		t->stats.late_events++;
		return;
	}

	switch (e->status) {
	case ANT_RADIO_STATUS_OK:
		break;
	case ANT_RADIO_STATUS_CRC_FAIL:
		/* Non-terminal, and only delivered to a window that asked for
		 * it - which this one does not. The window is still open, so
		 * there is nothing to do but wait. */
		return;
	case ANT_RADIO_STATUS_TIMEOUT:
		/*
		 ...
		 * 3143 us) and is implemented, in ant_ack.c, for the direction
		 ...
		 */
		finish(t, ANT_TRANSFER_EV_TX_FAILED, ANT_TRANSFER_FAIL_NO_ACK);
		return;
	default:
		finish(t, ANT_TRANSFER_EV_TX_FAILED, ANT_TRANSFER_FAIL_RADIO);
		return;
	...
	/* ANT_RX_STOP_ON_FIRST makes this event terminal, so the operation slot
	 * is already free and the next packet can be armed from right here. */
	if (!decode_rx(t, e, &f)) {
		finish(t, ANT_TRANSFER_EV_TX_FAILED, ANT_TRANSFER_FAIL_BAD_ACK);
		return;
	}
	if (!ant_transfer_ack_matches(t->cur_ctrl, f.ctrl_byte)) {
		finish(t, ANT_TRANSFER_EV_TX_FAILED, ANT_TRANSFER_FAIL_BAD_ACK);
		return;
	}

	if (ant_ctrl_is_last(t->cur_ctrl)) {
		...
		t->pkt_acked++;
		t->stats.packets_sent++;
		finish(t, ANT_TRANSFER_EV_TX_COMPLETED, ANT_TRANSFER_FAIL_NONE);
		return;
	}
...
{
	return (t == NULL) ? ANT_TRANSFER_STATE_IDLE : t->state;
}
...
{
	return (t == NULL) || (t->state == ANT_TRANSFER_STATE_IDLE);
}

const struct ant_transfer_stats *ant_transfer_stats(const struct ant_transfer *t)
{
	static const struct ant_transfer_stats none;
