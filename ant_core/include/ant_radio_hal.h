...
/*
 * ant_radio_hal.h - the contract between ant_core and a radio backend.
 *
 ...
 * and MPSL-timeslot) and one on EFR32/RAIL; a fourth, ant_core/tests/fake_radio.c,
 ...
 *      was built with (struct ant_radio_caps) and rejects any other. Adding the
 ...
 *      carries its own struct ant_pkt_format. The reason is concrete rather
...
#ifndef ANT_RADIO_HAL_H_
#define ANT_RADIO_HAL_H_
 ...
 * ---------------------------------------------------------------------------
 */
#define ANT_RADIO_OK_RC        0
#define ANT_RADIO_EINVAL     (-1)  /* malformed request */
#define ANT_RADIO_ENOTSUP    (-2)  /* well-formed, but this backend cannot do it */
#define ANT_RADIO_EBUSY      (-3)  /* an operation is already armed or running */
#define ANT_RADIO_ETIME      (-4)  /* the requested instant is already unreachable */
#define ANT_RADIO_ESTATE     (-5)  /* called in the wrong lifecycle state */
#define ANT_RADIO_EIO        (-6)  /* the hardware did not do as it was told */
 ...
 * unspecified origin at ant_radio_init() and never going backwards. 64 bits is
 ...
 * minute 72.
 */
typedef uint64_t ant_time_t;

/* "No deadline" / "not known". Never a valid arm time. */
#define ANT_TIME_NEVER  ((ant_time_t)UINT64_MAX)
...
#define ANT_RF_INDEX_MAX      124
#define ANT_RF_INDEX_ANT_PLUS 57

static inline uint16_t ant_rf_index_to_mhz(uint8_t rf_index)
{
 ...
 * set the backend advertises. ANT_PHY_1M_GFSK is the only one an ANT+
 ...
 * a delivered packet with status ANT_RADIO_STATUS_OK has a verified CRC.
 * ---------------------------------------------------------------------------
 */
struct ant_crc_cfg {
 ...
 * ANT_RADIO_ENOTSUP. It must never transmit or receive something close.
 */
struct ant_pkt_format {
	enum ant_phy       phy;
	uint8_t            addr_len;     /* on-air address bytes, 2..ANT_RADIO_ADDR_MAX */
	enum ant_len_mode  len_mode;
	uint8_t            body_len;     /* ANT_LEN_FIXED only */
	uint8_t            len_offset;   /* ANT_LEN_FROM_BODY only */
	int8_t             len_bias;     /* ANT_LEN_FROM_BODY only */
	uint8_t            max_body_len; /* RX bound; reject anything longer */
	struct ant_crc_cfg crc;
};
 ...
 * and caps.max_filters == 8, ant_search.c enumerates eight concrete addresses
 ...
 * instead of ~8 s.
 */
struct ant_rx_filter {
	uint8_t addr[ANT_RADIO_ADDR_MAX];
 ...
 * have (ant_tx_req.t_sync_at), and the backend works backwards through its own
 ...
 * event, so that ant_event.c can degrade the 0xE0 timestamp's advertised
...
struct ant_tx_req {
	 ...
	 * in the future; otherwise the call fails ANT_RADIO_ETIME. A backend
 ...
 */
#define ANT_RX_STOP_ON_FIRST  (1u << 0)
/* Deliver ANT_RADIO_STATUS_CRC_FAIL events too. Off by default: the core only
 * needs them for the RX_FAIL accounting that makes "unexplained loss" a
 * meaningful gate, and for bench diagnostics.
 */
#define ANT_RX_REPORT_CRC_FAIL (1u << 1)
 ...
 * deciding what to merge is ant_sched.c's.
 *
 ...
 * (ANT_RADIO_STATUS_TIMEOUT if the window closed, ANT_RADIO_STATUS_ABORTED if
 * it was cancelled). With ANT_RX_STOP_ON_FIRST the first accepted frame's event
 * is itself the terminal one.
 */
struct ant_rx_req {
	const struct ant_pkt_format *fmt;
	uint8_t                      rf_index;
	const struct ant_rx_filter  *filters;   /* n_filters entries, core-owned,
						 * must stay valid until the
						 * terminal event */
	uint8_t                      n_filters; /* 1..caps.max_filters */
	ant_time_t                   t_open;    /* earliest acceptable t_sync */
	ant_time_t                   t_close;   /* latest acceptable t_sync */
	uint32_t                     flags;
};
...
enum ant_radio_status {
	ANT_RADIO_STATUS_OK = 0,       /* frame received, CRC verified */
	ANT_RADIO_STATUS_CRC_FAIL,     /* address matched, CRC did not */
	ANT_RADIO_STATUS_TIMEOUT,      /* window closed */
	ANT_RADIO_STATUS_ABORTED,      /* ant_radio_abort() or a lifecycle call */
	ANT_RADIO_STATUS_FAILED        /* the backend could not complete it */
};
...
struct ant_rx_event {
	...
	bool                  t_sync_exact;
	/*
	 * Which filter matched, indexing ant_rx_req.filters.
	 *
 ...
 *   - call ant_radio_tx(), ant_radio_rx(), ant_radio_abort(), ant_radio_now()
 *     and ant_radio_caps_get(),
 ...
 *     reused immediately),
 *   - call ant_radio_init(), ant_radio_enable() or ant_radio_disable(),
 ...
 * may additionally produce zero or more non-terminal ANT_RADIO_STATUS_OK /
 ...
 * resolves that race internally and returns ANT_RADIO_EBUSY rather than
 * corrupting state; the core must handle EBUSY rather than assume it cannot
 * happen.
 */
struct ant_radio_cbs {
	void (*rx)(const struct ant_rx_event *evt, void *user);
	void (*tx)(const struct ant_tx_event *evt, void *user);
};
...
struct ant_radio_caps {
	 ...
	 * struct ant_rx_filter for what the core does about it. */
	 ...
	 * and phys[0] is ANT_PHY_1M_GFSK on any backend claiming ANT+
	 * compatibility. The Phase 7 long-range axis appends to this list. */
	const enum ant_phy *phys;
	uint8_t             n_phys;
	...
	/* Minimum lead time between ant_radio_now() and the earliest instant an
	 * arm call can still honour: software setup, DMA, clock start and
	 * ramp-up. Arming inside this window fails ANT_RADIO_ETIME rather than
	...
	/* True if the CRC in struct ant_crc_cfg is computed by hardware. False
 ...
 * ant_core/tests/fake_radio.c substitutes at link time - which is also what
 * makes the mock cheap.
 *
 * Lifecycle: init -> enable -> (arm/callback)* -> disable. Arm calls outside
 * the enabled state return ANT_RADIO_ESTATE.
 * ---------------------------------------------------------------------------
 */
 ...
 * arm call while one is armed or running returns ANT_RADIO_EBUSY. Chaining is
 ...
 * status ANT_RADIO_STATUS_ABORTED, before this function's caller can observe
 * the slot as free - so the core never has to reason about an operation that
 * ended without an event. Returns ANT_RADIO_OK_RC when there was nothing to do.
 */
