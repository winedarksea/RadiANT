...
/*
 * ant_search.h - wildcard search: the sweep, the seen cache, and scan mode.
 *
 ...
 * window), from ant_core/include/ant_radio_hal.h and ant_core/include/ant_frame.h,
 ...
 * ant_search_filters_per_window() and ant_search_sets() are public so a test
 ...
 *    ANT_RADIO_STATUS_CRC_FAIL event NEVER contributes to an acquisition here.
 ...
 *    recovers it by indexing its OWN filter table with ant_rx_event.filter_index
 ...
 *    channel in ANT_SEARCH_MODE_ACQUIRE or _SCAN. Without this, eight
 ...
 * arming authority, and it is ant_sched.c. ant_search.c is pure policy: it
 * produces a window request, is told what was armed, and consumes the events.
 * The only HAL function it calls is ant_radio_caps_get(), once, in
 * ant_search_init(). That is what lets the scheduler merge, delay, truncate or
 * refuse a search window without this module knowing or caring, and it is why
 * a search suite needs no scheduler.
 *
 * A consequence worth stating: there is no open-ended window. ANT_TIME_NEVER is
...
#ifndef ANT_SEARCH_H_
#define ANT_SEARCH_H_
 ...
 * Return codes
 *
 * Our own, for the reason the HAL's and ant_frame.h's are their own: this
 ...
 * ---------------------------------------------------------------------------
 */
#define ANT_SEARCH_OK        0
#define ANT_SEARCH_EINVAL  (-1)  /* null pointer or an out-of-range argument */
#define ANT_SEARCH_ENOSPC  (-2)  /* no free searching-channel slot */
#define ANT_SEARCH_ESTATE  (-3)  /* not initialised, or a window is already
				  * armed and has not terminated */
#define ANT_SEARCH_ENOENT  (-4)  /* nothing is searching, so no window is
				  * wanted. Not an error - the scheduler asks
				  * this question on every pass */
#define ANT_SEARCH_ENOTSUP (-5)  /* the backend advertises something this
 ...
 * Sizes
 *
 * All state is caller-provided (struct ant_search below) and every array in it
...
#ifndef ANT_SEARCH_MAX_CHANNELS
#define ANT_SEARCH_MAX_CHANNELS 32
#endif
 ...
 */
#ifndef ANT_SEARCH_MAX_FILTERS
#define ANT_SEARCH_MAX_FILTERS 8
#endif
...
#ifndef ANT_SEARCH_SEEN_ENTRIES
#define ANT_SEARCH_SEEN_ENTRIES 16
#endif
 ...
 * than as a magic 256. */
#define ANT_SEARCH_LO_VALUES 256u
...
#define ANT_SEARCH_SEEN_LIFETIME_US 60000000u
...
#define ANT_SEARCH_DWELL_DEFAULT_US 260000u

/* ANT's default search timeout: 10 counts of 2.5 s [rev5.1]. */
#define ANT_SEARCH_TIMEOUT_DEFAULT_US 25000000u

/* Passed as timeout_us to ant_search_begin() for a search that never expires.
 * What background scan mode uses, and what ANT calls an infinite search. */
#define ANT_SEARCH_TIMEOUT_NONE 0u
...
enum ant_search_mode {
	 ...
	 * ant_search_begin() again if it loses the sensor, which is the path
	 * the seen cache makes fast.
	 */
	ANT_SEARCH_MODE_ACQUIRE = 0,
	/*
	 ...
	 * advertises this mode and never implemented it; ant_core implements it,
 ...
 * ant_search_on_rx(), which the scheduler calls from the HAL's rx callback, and
 * it therefore inherits every MUST NOT in the callback contract in
 * ant_radio_hal.h: it must not block, must not take a mutex, must not retain
 * anything past its return, and must not do work proportional to anything.
 * Queue it and return. timeout() is called from ant_search_tick() and is
 ...
 */
struct ant_search_cbs {
	void (*acquired)(uint8_t channel, const struct ant_search_result *r,
			 void *user);
...
struct ant_search_cfg {
	...
	/* 2400 + N MHz. ANT+ is ANT_RF_INDEX_ANT_PLUS, and that every channel
	...
	/*
	 * Ask the backend for ANT_RADIO_STATUS_CRC_FAIL events too. Off by
...
void ant_search_cfg_default(struct ant_search_cfg *cfg);

/* ---------------------------------------------------------------------------
 * The window this module wants the scheduler to arm
 * ---------------------------------------------------------------------------
 */
struct ant_search_window {
	const struct ant_pkt_format *fmt;
	uint8_t                      rf_index;

	/*
	 * Points INTO the struct ant_search that produced it, which is what the
	 * HAL requires: filters must stay valid until the terminal event. It is
	 * invalidated by the next ant_search_window() call on the same
	...
	/*
	 * ANT_RX_REPORT_CRC_FAIL if cfg.report_crc_fail. Never
	 * ANT_RX_STOP_ON_FIRST: the HAL forbids it on a search window and the
 ...
 * ant_search_stats(); everything else in here is private and may be
 * rearranged.
 * ---------------------------------------------------------------------------
 */

struct ant_search_chan {
	bool                        active;
	enum ant_search_mode        mode;
	struct ant_search_id_filter want;
	ant_time_t                  deadline; /* ANT_TIME_NEVER = no timeout */
};

struct ant_search_seen {
	bool                  valid;
	struct ant_channel_id id;
	ant_time_t            t_seen;
};
...
struct ant_search {
	...
	struct ant_rx_filter filters[ANT_SEARCH_MAX_FILTERS];
	uint8_t              filter_lo[ANT_SEARCH_MAX_FILTERS];
	...
	uint16_t next_set;
	uint16_t steer[ANT_SEARCH_SEEN_ENTRIES];
	uint8_t  n_steer;

	struct ant_search_chan chans[ANT_SEARCH_MAX_CHANNELS];
	struct ant_search_seen seen[ANT_SEARCH_SEEN_ENTRIES];
	uint8_t                seen_next;

	struct ant_search_stats stats;
};
...
/*
 * Zero the instance and compute the sweep geometry from ant_radio_caps_get().
 *
 ...
 *
 * ANT_SEARCH_ENOTSUP if caps.filter_wildcard_dev is set. That flag says the
 * backend can match "any device number" in one filter, which would collapse the
 * sweep to a single window - but struct ant_rx_filter has no field in which to
 ...
 */
int ant_search_init(struct ant_search *s, const struct ant_search_cfg *cfg,
		    const struct ant_search_cbs *cbs, void *user);

/* Filters per window: caps.max_filters, clamped to ANT_SEARCH_MAX_FILTERS. */
uint8_t ant_search_filters_per_window(const struct ant_search *s);
...
uint16_t ant_search_sets(const struct ant_search *s);
...
uint64_t ant_search_sweep_us(const struct ant_search *s);
 ...
 *
 * timeout_us is ANT_SEARCH_TIMEOUT_NONE for no expiry, or a duration from now;
 * ANT_SEARCH_TIMEOUT_DEFAULT_US is the 25 s the protocol defaults to. The
 * deadline is only examined by ant_search_tick().
 *
 ...
 *
 * ANT_SEARCH_EINVAL for a null filter or an unknown mode; ANT_SEARCH_ENOSPC for
 * a channel number this build was not sized for (see ANT_SEARCH_MAX_CHANNELS).
 *
 ...
 * below is for.
 */
int ant_search_begin(struct ant_search *s, uint8_t channel,
		     enum ant_search_mode mode,
		     const struct ant_search_id_filter *want, ant_time_t now,
		     uint32_t timeout_us);

/* Take a channel out of the search. Idempotent; ANT_SEARCH_ENOENT if it was not
 ...
 * wants the radio. */
int ant_search_end(struct ant_search *s, uint8_t channel);

bool    ant_search_is_searching(const struct ant_search *s, uint8_t channel);
uint8_t ant_search_n_searching(const struct ant_search *s);
 ...
 * ANT_RADIO_ETIME would be this module's fault under any other convention.
 *
 * Returns ANT_SEARCH_OK and fills out, or ANT_SEARCH_ENOENT if nothing is
 * searching, or ANT_SEARCH_ESTATE if a window is already in flight.
 *
 ...
 */
int ant_search_window(struct ant_search *s, ant_time_t t_earliest,
		      struct ant_search_window *out);

/*
 * Passed as op to ant_search_armed() by an arming authority that does not
 * expose a HAL operation id - which ant_sched.c does not, because it merges
 ...
 */
#define ANT_SEARCH_OP_EXTERNAL ((uint32_t)0xFFFFFFFFu)

/*
 * The window was armed: op is what ant_radio_rx() returned, or
 * ANT_SEARCH_OP_EXTERNAL, and t_open / t_close are what was actually asked for,
 * which need not be what ant_search_window() proposed. The dwell is credited
 ...
 */
void ant_search_armed(struct ant_search *s, uint32_t op, ant_time_t t_open,
		      ant_time_t t_close);
...
void ant_search_arm_failed(struct ant_search *s);
...
bool ant_search_owns_op(const struct ant_search *s, uint32_t op);

/*
 * One radio event. RUNS IN THE RADIO INTERRUPT; see struct ant_search_cbs.
 *
 ...
 *
 * ANT_RADIO_STATUS_CRC_FAIL is counted and DISCARDED. It is never evidence of
 * anything but the noise floor.
 */
void ant_search_on_rx(struct ant_search *s, const struct ant_rx_event *evt);
 ...
 * transmitted. Use this from ant_sched.c; use ant_search_on_rx() when the
 * module has the radio to itself.
 */
void ant_search_on_rx_indexed(struct ant_search *s, const struct ant_rx_event *evt,
			      uint8_t filter_index);
 ...
 * same set is listened to again. Called for you by ant_search_on_rx() on a
 * terminal event; called directly by a scheduler whose completion notification
 * is not a HAL event.
 */
void ant_search_on_done(struct ant_search *s, bool ran_to_close);
 ...
 */
void ant_search_tick(struct ant_search *s, ant_time_t now);
 ...
 * ant_channel.c can also note a sensor it is tracking, which is what makes the
 * cache useful at the moment that sensor drops. */
void ant_search_seen_note(struct ant_search *s, const struct ant_channel_id *id,
			  ant_time_t now);

/* The most recently seen unexpired device matching want, if any. */
bool ant_search_seen_lookup(const struct ant_search *s,
			    const struct ant_search_id_filter *want,
			    ant_time_t now, struct ant_channel_id *out);

/* Unexpired entries at time now. */
uint8_t ant_search_seen_count(const struct ant_search *s, ant_time_t now);

/* The set that would catch a given device number, i.e. the one holding its low
 * byte. ANT_SEARCH_SETS_NONE if the instance is not initialised. */
#define ANT_SEARCH_SETS_NONE ((uint16_t)0xFFFFu)
uint16_t ant_search_set_for_devnum(const struct ant_search *s, uint16_t devnum);

const struct ant_search_stats *ant_search_get_stats(const struct ant_search *s);
