...
/*
 * ant_transfer.h - acknowledged data and burst: the one engine both of them are.
 *
 ...
 * ant_core/include/ant_frame.h, ant_core/include/ant_radio_hal.h and the
 ...
 *
 * So ant_ack.c and ant_burst.c share one encoder and one state machine.
 * ant_transfer_ack_data() is ant_transfer_submit() with a copied eight-byte
 ...
 * ANT_TRANSFER_REPLY_US is, it is what ant_transfer_on_data() spends its budget
 ...
 *
 * This module expresses those three events as ant_transfer_event and guarantees
 * the invariant that makes B1-B5 hold:
 *
 *   EXACTLY ONE ant_transfer_ops::done() CALL CARRIES ANY GIVEN BLOCK POINTER,
 *   AND EXACTLY ONE TERMINAL EVENT (TX_COMPLETED or TX_FAILED) ENDS ANY
 *   TRANSFER.
 *
 * A block is released with ANT_TRANSFER_EV_NEXT_BLOCK when the acknowledgement
 ...
 * why ant_transfer_submit() arms the radio before it records the block as
 ...
 *      with ANT_TRANSFER_FAIL_NO_ACK and does not retransmit. Inventing a retry
 *      here would put an unmeasured frame on the air at an unmeasured instant.
 *   2. ACKNOWLEDGING A SLOT OPENER. ant_ctrl_reply_for() returns 0 for 0x8A and
 ...
 *      does not guess one: ant_transfer_on_data() refuses, counts it in
 *      stats.unackable_openers, and returns ANT_TRANSFER_ENOTSUP.
 *   3. AN ACKNOWLEDGEMENT CARRYING ANYTHING BUT THE BROADCAST BUFFER. Every one
 *      observed carried it. ant_transfer_on_data() therefore requires
 ...
 *      ANT_TRANSFER_PKT_BYTES is not a tunable.
 */

#ifndef ANT_TRANSFER_H_
#define ANT_TRANSFER_H_
 ...
 * ant_radio_tx() or ant_radio_rx(). See "One radio, one arming authority"
 * below.
 */
#include "ant_sched.h"
 ...
 * ---------------------------------------------------------------------------
 * This module was written before ant_sched.c existed and armed the radio
 * directly, at two sites: ant_transfer_arm_tx() called ant_radio_tx() and
 * arm_ack_window() in ant_burst.c called ant_radio_rx(). Both now post to
 * ant_sched.c instead.
 *
 * That is not tidiness. The HAL permits exactly one operation in flight and
 * ant_sched.h says in its first paragraph that it "is the only place in
 * ant_core that calls ant_radio_tx() or ant_radio_rx()". Two schedulers racing
 ...
 * tracked channels were sharing, or lose to it with ANT_RADIO_EBUSY and fail a
 ...
 *
 *   - struct ant_transfer_cfg carries a CHANNEL NUMBER. The scheduler's slots
 *     are per channel and the engine has to name its own.
 *   - the engine no longer holds a HAL operation id, because ant_sched.c does
 ...
 *     ANT_TRANSFER_OP_EXTERNAL and the engine trusts its caller's routing,
 *     which is correct precisely because the caller is the thing that did the
 *     routing. ant_search.h reached the same conclusion first and spells it
 *     ANT_SEARCH_OP_EXTERNAL.
 *
 * ant_transfer_abort() still calls ant_radio_abort() directly and is
 ...
 * operation id to report. With this token ant_transfer_on_tx_event() and
 * ant_transfer_on_rx_event() stop filtering by op id; every other value keeps
 ...
 */
#define ANT_TRANSFER_OP_EXTERNAL ((uint32_t)0xFFFFFFFFu)
 ...
 *
 * ant_api.c maps them onto the serial protocol's wire codes, which is where the
 ...
 *
 *   ANT_TRANSFER_EINVAL  -> ANTW_INVALID_MESSAGE
 *   ANT_TRANSFER_EBUSY   -> ANTW_TRANSFER_IN_PROGRESS
 *   ANT_TRANSFER_ESTATE  -> ANTW_CHANNEL_IN_WRONG_STATE
 *   ANT_TRANSFER_ESEQ    -> ANTW_TRANSFER_SEQUENCE_NUMBER_ERROR
 *   ANT_TRANSFER_ENOTSUP -> ANTW_INVALID_PARAMETER_PROVIDED
 *   ANT_TRANSFER_EIO     -> ANTW_TRANSFER_IN_ERROR
 * ---------------------------------------------------------------------------
 */
#define ANT_TRANSFER_OK        0
#define ANT_TRANSFER_EINVAL  (-1)  /* null pointer, bad block size, bad flags */
#define ANT_TRANSFER_EBUSY   (-2)  /* a transfer already owns this engine */
#define ANT_TRANSFER_ESTATE  (-3)  /* not initialised, or wrong state for this */
#define ANT_TRANSFER_ESEQ    (-4)  /* a segment flag that does not follow */
#define ANT_TRANSFER_ENOTSUP (-5)  /* well formed, but not measured / not
				    * reachable on this backend's timings */
#define ANT_TRANSFER_EIO     (-6)  /* the radio refused the operation */
...
#define ANT_TRANSFER_PKT_BYTES  ANT_FRAME_PAYLOAD_STD
 ...
 * one that says no. */
#define ANT_TRANSFER_BLOCK_MAX  24u
#define ANT_TRANSFER_PKTS_MAX   (ANT_TRANSFER_BLOCK_MAX / ANT_TRANSFER_PKT_BYTES)
 ...
 * ant_api.c forwards the byte untouched instead of remapping it.
 *
 ...
 * checked it. ant_api.c must therefore carry a BUILD_ASSERT that the three
 ...
 */
#define ANT_TRANSFER_SEG_CONTINUE 0x00u
#define ANT_TRANSFER_SEG_START    0x01u
#define ANT_TRANSFER_SEG_END      0x02u
#define ANT_TRANSFER_SEG_MASK     (ANT_TRANSFER_SEG_START | ANT_TRANSFER_SEG_END)
...
#define ANT_TRANSFER_REPLY_US        1560u

/* Acknowledgement's t_sync -> the next data packet's t_sync. 1546 / 1550. */
#define ANT_TRANSFER_NEXT_PACKET_US  1550u

/* Master's broadcast t_sync -> a slave's reply t_sync. 2186 / 2191. Exported
 * because ant_sched.c needs it to place the first packet of a slave-originated
 * exchange, which this module does not choose for itself. */
#define ANT_TRANSFER_SLOT_REPLY_US   2190u
...
#define ANT_TRANSFER_PACKET_US \
	(ANT_TRANSFER_REPLY_US + ANT_TRANSFER_NEXT_PACKET_US)
 ...
 */
#define ANT_TRANSFER_ACK_GUARD_US    250u
 ...
 *
 * This module implements the retransmission as ant_transfer_reply_retransmit()
 * and enforces the limit, but does NOT own the timer: deciding when a channel
 * is idle enough to spend a slot on a repeat is ant_sched.c's call, and a timer
 * in here would be a second scheduler.
 */
#define ANT_TRANSFER_REPLY_RETRY_US  3143u
...
#define ANT_TRANSFER_REPLY_ATTEMPTS_MAX 21u

/* Added to caps.min_arm_lead_us when a caller says "as soon as possible"
 * (ANT_TIME_NEVER). Small, because the point of the arm-lead figure is that it
 * is already the whole cost of getting a frame out. */
#define ANT_TRANSFER_ARM_SLACK_US    50u

/* ---------------------------------------------------------------------------
 * The pure encoder - the shared half of ant_ack.c and ant_burst.c
 * ---------------------------------------------------------------------------
 */
 ...
 *
 * Returns ANT_TRANSFER_OK, or ANT_TRANSFER_EINVAL for a null argument.
 */
 ...
 * measured, so the ant_ctrl_observed() check can never fire. It is here anyway:
 * it is the assertion that a future edit to the field rules did not invent a
 * seventh.
 *
 * Returns ANT_TRANSFER_OK, or ANT_TRANSFER_EINVAL.
 */
 ...
 *
 * Thin over ant_ctrl_reply_for(), and named here so that the transfer layer's
 ...
 * measured pairs this predicate is exactly ant_ctrl_reply_for().
 */
 ...
 * frame.
 *
 * Goes through ant_frame_make(), which refuses any field combination outside
 * the measured eleven, and then ant_frame_encode(). It does NOT hand-set
 * ctrl_byte and pass it to ant_frame_encode() directly: that path exists so a
 ...
 * ever transmitted.
 *
 * Returns the body length written, or ANT_TRANSFER_EINVAL.
 */
struct ant_transfer_cfg;
 ...
 * with the same meaning as the ANTW_EVENT_TRANSFER_* codes ant_api.c turns them
 * into. There is no fourth: an event that does not release something has no
 * business on this path.
 */
enum ant_transfer_event {
...
struct ant_transfer_result {
	 ...
	 * Never NULL on ANT_TRANSFER_EV_NEXT_BLOCK.
	 *
	...
	uint8_t *block;
	uint8_t  block_len;
	enum ant_transfer_fail fail;   /* NONE unless ev is TX_FAILED */
 ...
 */
struct ant_transfer_ops {
	/* Required. See struct ant_transfer_result. */
	void (*done)(void *ctx, const struct ant_transfer_result *r);
	...
	bool (*broadcast_payload)(void *ctx,
				  uint8_t out[ANT_TRANSFER_PKT_BYTES]);
};
...
struct ant_transfer_cfg {
	...
	uint8_t               net_addr[ANT_NET_ADDR_LEN];
	uint8_t               rf_index;   /* 57 for ANT+ */
	struct ant_tx_power   power;

	/*
	 * The ANT channel this engine belongs to, 0 .. ANT_SCHED_MAX_CHANNELS-1.
	 *
	 * The engine posts its packets and its reply windows to ant_sched.c
	...
	const struct ant_transfer_ops *ops;
	void                          *ctx;
};
 ...
 * ---------------------------------------------------------------------------
 */
enum ant_transfer_state {
	ANT_TRANSFER_STATE_IDLE = 0,
	/* A data packet of an outbound transfer is armed or on the air. */
	ANT_TRANSFER_STATE_TX_DATA,
	/* The reply window is open. */
	ANT_TRANSFER_STATE_WAIT_ACK,
	...
	ANT_TRANSFER_STATE_WAIT_BLOCK,
	/* We are the acknowledger and our reply is armed or on the air. */
	ANT_TRANSFER_STATE_TX_REPLY,
	/* ant_transfer_abort() has been called and the terminal event of the
	...
	ANT_TRANSFER_STATE_ABORTING
};

struct ant_transfer_stats {
...
/*
 * One channel's transfer engine. Embed one per channel in ant_channel.c; there
 * is no allocation anywhere in ant_core.
 *
 * THE FIELDS ARE PRIVATE. They are in the header because the struct has to be
 * embeddable by value, not because anything outside ant_ack.c and ant_burst.c
 * may read them. Use ant_transfer_state(), ant_transfer_op(),
 * ant_transfer_is_idle() and ant_transfer_stats().
 */
struct ant_transfer {
	...
	uint8_t ack_payload[ANT_TRANSFER_PKT_BYTES];
	...
	ant_time_t next_t_sync;    /* when the next packet is due */

	/* The acknowledger's side. */
	uint8_t reply_ctrl;
	uint8_t reply_payload[ANT_TRANSFER_PKT_BYTES];
	...
	struct ant_transfer_stats stats;
};
 ...
 * because it reads ant_radio_caps_get() and refuses a backend that cannot meet
 * the turnaround:
 *
 *   caps.min_arm_lead_us + ANT_TRANSFER_ACK_GUARD_US must be <=
 *   ANT_TRANSFER_REPLY_US, or the reply window cannot be armed in time;
 *   caps.rx_to_tx_us must be <= ANT_TRANSFER_REPLY_US, which is the check the
 *   HAL's own comment on that field asks for.
 *
 * Returns ANT_TRANSFER_OK, ANT_TRANSFER_EINVAL, or ANT_TRANSFER_ENOTSUP for a
 ...
 */
int ant_transfer_init(struct ant_transfer *t, const struct ant_transfer_cfg *cfg);
 ...
 * @param segment    ANT_TRANSFER_SEG_* flags. A single-block transfer carries
 *                   START|END.
 ...
 * @param t_sync_at  Absolute t_sync for the first packet, or ANT_TIME_NEVER for
 *                   "as soon as the backend can". A slave answering its master
 *                   passes master_t_sync + ANT_TRANSFER_SLOT_REPLY_US; a master
 *                   passes its own slot instant. Ignored on a continuation
 *                   block, which is already scheduled at
 *                   ack_t_sync + ANT_TRANSFER_NEXT_PACKET_US.
 *
 ...
 */
int ant_transfer_submit(struct ant_transfer *t, uint8_t *block, uint8_t len,
			uint8_t segment, bool owned, ant_time_t t_sync_at);

/* Burst: ownership transfers, per B1-B5. */
static inline int ant_transfer_burst_block(struct ant_transfer *t,
					   uint8_t *block, uint8_t len,
					   uint8_t segment,
					   ant_time_t t_sync_at)
{
	return ant_transfer_submit(t, block, len, segment, true, t_sync_at);
}
 ...
 * inside one - and it runs through ant_transfer_submit() precisely so that
 * nothing can drift between the two. `len` must be ANT_TRANSFER_PKT_BYTES.
 *
 ...
 * synchronously (called from thread context, where ant_radio_abort() delivers
 ...
 * ANT_TRANSFER_FAIL_ABORTED ends the transfer, and any block still held is
 * released with it.
 *
 * Returns ANT_TRANSFER_OK, including when there was nothing to cancel.
 */
int ant_transfer_abort(struct ant_transfer *t);
 ...
 * ant_frame_decode() and delivered from the channel's tracked window.
 *
 ...
 * ANT_TRANSFER_REPLY_RETRY_US and ANT_TRANSFER_REPLY_ATTEMPTS_MAX, and this
 * function enforces the limit.
 *
 * The TIMER IS NOT HERE. ant_sched.c decides when a channel can spend a slot on
 ...
 *
 * Returns ANT_TRANSFER_OK, ANT_TRANSFER_ESTATE (nothing to repeat, engine busy,
 * or the limit reached) or ANT_TRANSFER_EIO.
 */
 ...
 * Radio events
 *
 * The HAL has one global callback pair for the whole image, so ant_channel.c
 ...
 * ---------------------------------------------------------------------------
 */
void ant_transfer_on_tx_event(struct ant_transfer *t,
			      const struct ant_tx_event *e);
void ant_transfer_on_rx_event(struct ant_transfer *t,
			      const struct ant_rx_event *e);

/* ---------------------------------------------------------------------------
 * Shared between ant_ack.c and ant_burst.c
 *
 ...
 * ANT_TRANSFER_OP_EXTERNAL. Does NOT touch t->state: the caller knows whether
 ...
 * only path that works - ant_sched.c runs its commit on the way out and the
 * frame is armed with no thread wakeup in between. From thread context the
 * caller owes one ant_sched_tick(); ant_api.c does that at the end of the
 ...
 * turn a loud ANT_RADIO_ETIME into a silent one.
 *
 * Returns ANT_TRANSFER_OK, ANT_TRANSFER_EINVAL or ANT_TRANSFER_EIO.
 */
int ant_transfer_arm_tx(struct ant_transfer *t, uint8_t ctrl,
			const uint8_t *payload, uint8_t payload_len,
			ant_time_t t_sync_at);
 ...
 * ---------------------------------------------------------------------------
 */
enum ant_transfer_state ant_transfer_state(const struct ant_transfer *t);
uint32_t                ant_transfer_op(const struct ant_transfer *t);
bool                    ant_transfer_is_idle(const struct ant_transfer *t);
/* Never NULL for a non-NULL engine. */
const struct ant_transfer_stats *ant_transfer_stats(const struct ant_transfer *t);

#ifdef __cplusplus
}
#endif

#endif /* ANT_TRANSFER_H_ */
