...
/*
 * ant_api.h - what ant_core presents to the rest of the image, and the two
 ...
 * from the six ant_core module headers it composes. Nothing here derives from
 ...
 * ---------------------------------------------------------------------------
 * ant_api.c's real interface is src/ant_radio.h: it defines the fifty antr_*
 ...
 *
 *   ant_channel_event_out()   ant_channel.c raises channel events through it;
 *                             ant_event.c spells the same thing
 *                             ant_event_post_channel_event(). One adapter.
 *   ant_event_crit_enter()    the ring's critical section - irq_lock() here,
 *   ant_event_crit_exit()     because the producer is a radio ISR and a mutex
 *                             would be wrong.
 *   ant_event_wakeup()        makes the drain run. A semaphore give, O(1) and
 ...
 * anything.
 *
 * The one hard rule this file exists to write down: ant_event_drain() is
 ...
 * inside a call the bridge is still executing. ant_api.c therefore owns a
 ...
 *
 * ant_sched.c owns no timer: it arms for absolute instants and lets the backend
 * hold them, so a request that lost an arm race with ANT_RADIO_EBUSY or was
 * posted before the radio was enabled (ANT_RADIO_ESTATE) has nothing scheduled
 * to retry it. ant_sched_tick() is the only recovery path, and this is what
 * drives it. ant_channel_tick() - search deadlines - and ant_search_tick() -
 ...
 * (ANT_CHANNEL_PERIOD_MIN_COUNTS is 8.3 ms, but no ANT+ profile is faster than
 ...
 */
#define ANT_API_HOUSEKEEP_MS 50u
 ...
 * radio backend under ant_core cannot yet put anything on the air.
 */
struct ant_api_stats {
	uint32_t drains;          /* ant_event_drain() calls that delivered */
	...
	uint32_t sched_missed;    /* ANT_SCHED_DONE_MISSED reports */
	uint32_t sched_failed;    /* ANT_SCHED_DONE_FAILED reports */
