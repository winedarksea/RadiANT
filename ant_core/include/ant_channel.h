...
/*
 * ant_channel.h - the per-channel state machine, its configuration, and the
 ...
 *
 * It calls no HAL function. Not one. Arming the radio is ant_sched.c's job,
 ...
 * single highest-value item in the scheduler (see struct ant_rx_req in
 * ant_radio_hal.h). So this module *decides* and ant_sched.c *acts*, and the
 * seam between them is the handful of ant_channel_on_*() / ant_channel_next_*
 * functions at the bottom of this file.
 *
 * The practical consequence, which is worth knowing before reading
 * ant_core/tests/src/test_channel.c: this module is testable with no radio at
 ...
 * ---------------------------------------------------------------------------
 * Every function here that ant_api.c forwards returns ant_channel_err_t, which
 ...
 * ant_core/include - the gate is
 *
 *   arm-zephyr-eabi-gcc -c -std=c11 -Wall -Wextra -Werror \
 *       -I ant_core/include -I ant_core/tests -fsyntax-only \
 *       ant_core/src/ant_channel.c
 *
 ...
 * translation unit that has already included src/ant_wire.h. ant_api.c
...
#include "ant_frame.h"     /* struct ant_channel_id, and the frame geometry */
#include "ant_radio_hal.h" /* ant_time_t, enum ant_radio_status */
 ...
 * lookup happens.
 */
#define ANT_CHANNEL_NETWORK_COUNT 3u
 ...
 * ---------------------------------------------------------------------------
 */
typedef uint8_t ant_channel_err_t;

#define ANT_CH_OK                 0x00u /* ANTW_RESPONSE_NO_ERROR */
#define ANT_CH_ERR_WRONG_STATE    0x15u /* ANTW_CHANNEL_IN_WRONG_STATE */
#define ANT_CH_ERR_NOT_OPENED     0x16u /* ANTW_CHANNEL_NOT_OPENED */
#define ANT_CH_ERR_ID_NOT_SET     0x18u /* ANTW_CHANNEL_ID_NOT_SET */
#define ANT_CH_ERR_INVALID_MESSAGE 0x28u /* ANTW_INVALID_MESSAGE */
#define ANT_CH_ERR_INVALID_NETWORK 0x29u /* ANTW_INVALID_NETWORK_NUMBER */
#define ANT_CH_ERR_INVALID_PARAM  0x33u /* ANTW_INVALID_PARAMETER_PROVIDED */
 ...
 *
 *   - a channel NUMBER out of range is ANT_CH_ERR_INVALID_MESSAGE; a bad
 *     parameter value is ANT_CH_ERR_INVALID_PARAM;
 *   - ANT_CH_ERR_WRONG_STATE is a channel that exists in the wrong state;
 *     ANT_CH_ERR_NOT_OPENED is a data-transfer call on a channel that is not
 ...
 *
 * These are ANTW_EVENT_* codes and go to the host through ant_event.c. They
 ...
 * ---------------------------------------------------------------------------
 */
#define ANT_CH_EVENT_RX_SEARCH_TIMEOUT   0x01u /* ANTW_EVENT_RX_SEARCH_TIMEOUT */
#define ANT_CH_EVENT_CHANNEL_CLOSED      0x07u /* ANTW_EVENT_CHANNEL_CLOSED */
#define ANT_CH_EVENT_RX_FAIL_GO_TO_SEARCH 0x08u /* ANTW_EVENT_RX_FAIL_GO_TO_SEARCH */
...
#define ANT_CH_STATUS_UNASSIGNED 0x00u /* ANTW_STATUS_UNASSIGNED_CHANNEL */
#define ANT_CH_STATUS_ASSIGNED   0x01u /* ANTW_STATUS_ASSIGNED_CHANNEL */
#define ANT_CH_STATUS_SEARCHING  0x02u /* ANTW_STATUS_SEARCHING_CHANNEL */
#define ANT_CH_STATUS_TRACKING   0x03u /* ANTW_STATUS_TRACKING_CHANNEL */
#define ANT_CH_STATUS_STATE_MASK 0x03u /* ANTW_STATUS_CHANNEL_STATE_MASK */
 ...
 */
#define ANT_CH_STATUS_NETWORK_SHIFT 2u
#define ANT_CH_STATUS_NETWORK_MASK  0x0Cu
#define ANT_CH_STATUS_TYPE_MASK     0xF0u
 ...
 */
#define ANT_CH_TYPE_MASTER_BIT 0x10u /* ANTW_CHANNEL_TYPE_MASTER */
 ...
 */
#define ANT_CHANNEL_PERIOD_HZ         32768u
#define ANT_CHANNEL_PERIOD_ANT_PLUS   8182u  /* 4.005 Hz, measured */
 ...
 * counts is 8.331 ms.
 *
 * ant_sched.c may raise the floor at init through
 * ant_channel_period_floor_set() once it has read caps.min_arm_lead_us from
 * the backend, because the real bound is a backend property and this one is
 * only the protocol's.
 */
#define ANT_CHANNEL_PERIOD_MIN_COUNTS 273u
#define ANT_CHANNEL_PERIOD_MAX_COUNTS 65535u
...
#define ANT_CHANNEL_SEARCH_TICK_US    2500000u
#define ANT_CHANNEL_SEARCH_INFINITE   255u
 ...
 * to spare - which is why ant_search.c may size its sweep without negotiating
 * with this module. */
#define ANT_CHANNEL_SEARCH_TIMEOUT_DEFAULT 10u
...
#define ANT_CHANNEL_LP_SEARCH_TIMEOUT_DEFAULT 0u

/*
 * Consecutive missed slots before a TRACKING channel gives up and searches
 * again, raising ANT_CH_EVENT_RX_FAIL_GO_TO_SEARCH.
 *
 ...
 * than the dropout. ant_sched.c may override it per build if a measurement
 * ever supersedes the guess.
 */
#define ANT_CHANNEL_RX_FAIL_TO_SEARCH 8u
 ...
 *
 * ant_channel_status_get() reports CLOSING as whatever it was closing FROM -
 * SEARCHING or TRACKING - not as ASSIGNED. Reporting ASSIGNED early would tell
 * a host it may unassign a channel that ant_channel_unassign() will then
 * refuse, and a status byte that disagrees with the next call's return code is
 * worse than one that lags by a millisecond.
 */
enum ant_channel_state {
	ANT_CH_STATE_UNASSIGNED = 0,
	ANT_CH_STATE_ASSIGNED,
	ANT_CH_STATE_SEARCHING,
	ANT_CH_STATE_TRACKING,
	ANT_CH_STATE_CLOSING,
	ANT_CH_STATE_COUNT
};
 ...
 * raises ANT_CH_EVENT_RX_SEARCH_TIMEOUT immediately and only the trailing
 * ANT_CH_EVENT_CHANNEL_CLOSED is deferred - so by the time the terminal event
 ...
 *
 * IMPLEMENTED BY ant_event.c, NOT BY ant_channel.c, and by a test double in
 * ant_core/tests/src/test_channel.c. This mirrors the inversion src/ant_radio.h
 ...
 *
 * @param channel     0 .. ANT_CHANNEL_COUNT-1.
 * @param event_code  An ANT_CH_EVENT_* value, which is an ANTW_EVENT_* wire
 *                    byte. ant_event.c wraps it in a channel-response message.
 *
 * Called from whatever context provoked the transition: the bridge's parser
 * thread for a host-initiated close, and ant_sched.c's context - which may be
 ...
 * block. It must not call back into ant_channel.
 */
 ...
 * no way to abort one, so ant_sched.c must call ant_radio_abort() before this
 ...
 */
void ant_channel_reset_all(void);

/* Raise the schedulable period floor, in 32768 Hz counts. For ant_sched.c at
 * init, once it has read caps.min_arm_lead_us. Values below
 * ANT_CHANNEL_PERIOD_MIN_COUNTS are ignored: the protocol floor is a floor. */
void ant_channel_period_floor_set(uint16_t counts);

/* ---------------------------------------------------------------------------
 * Lifecycle - what ant_api.c forwards from the four antr_channel_* calls
 * ---------------------------------------------------------------------------
 */
 ...
 * the wrong sensor instead of getting ANT_CH_ERR_ID_NOT_SET.
 *
 * Returns ANT_CH_OK, ANT_CH_ERR_INVALID_MESSAGE (channel out of range),
 * ANT_CH_ERR_WRONG_STATE (already assigned) or ANT_CH_ERR_INVALID_NETWORK.
 */
ant_channel_err_t ant_channel_assign(uint8_t channel, uint8_t type,
				     uint8_t network, uint8_t ext_assign);

/*
 * antr_channel_unassign(). The channel must be ASSIGNED and closed.
 *
 * Returns ANT_CH_OK, ANT_CH_ERR_INVALID_MESSAGE or ANT_CH_ERR_WRONG_STATE
 ...
 */
ant_channel_err_t ant_channel_unassign(uint8_t channel);
 ...
 *                period uses.
 * @param now     ant_radio_now(). Passed in rather than read here, because
 ...
 *
 * Returns ANT_CH_OK, ANT_CH_ERR_INVALID_MESSAGE (channel out of range),
 * ANT_CH_ERR_WRONG_STATE (unassigned, or already open or closing) or
 * ANT_CH_ERR_ID_NOT_SET.
 */
ant_channel_err_t ant_channel_open(uint8_t channel, uint16_t offset,
				   ant_time_t now);
 ...
 * ANT_CH_EVENT_CHANNEL_CLOSED is raised before this returns. If one is bound,
 * the channel enters ANT_CH_STATE_CLOSING and the event is raised when
 * ant_channel_on_terminal() delivers that operation's terminal event - which
 ...
 *
 * Returns ANT_CH_OK, ANT_CH_ERR_INVALID_MESSAGE or ANT_CH_ERR_WRONG_STATE
 * (not open). Calling it twice returns ANT_CH_ERR_WRONG_STATE the second time
 * rather than raising a second event.
 */
ant_channel_err_t ant_channel_close(uint8_t channel, ant_time_t now);
...
ant_channel_err_t ant_channel_id_set(uint8_t channel, uint16_t device_number,
 ...
 */
ant_channel_err_t ant_channel_id_get(uint8_t channel,
				     struct ant_channel_id *out);

/* antr_channel_period_set(), in 32768 Hz counts.
 * Returns OK, INVALID_MESSAGE, WRONG_STATE, or INVALID_PARAM (outside
 * [floor, ANT_CHANNEL_PERIOD_MAX_COUNTS]). */
ant_channel_err_t ant_channel_period_set(uint8_t channel, uint16_t period);
ant_channel_err_t ant_channel_period_get(uint8_t channel, uint16_t *out);

/* antr_channel_radio_freq_set(): an index, 0..124, meaning 2400+N MHz.
 * Returns OK, INVALID_MESSAGE, WRONG_STATE, or INVALID_PARAM (> 124). */
ant_channel_err_t ant_channel_rf_freq_set(uint8_t channel, uint8_t freq);
ant_channel_err_t ant_channel_rf_freq_get(uint8_t channel, uint8_t *out);
 ...
 * part can produce it is a backend question that ant_sched.c answers against
 ...
 */
ant_channel_err_t ant_channel_tx_power_set(uint8_t channel, uint8_t level,
					   uint8_t custom);
ant_channel_err_t ant_channel_tx_power_get(uint8_t channel, uint8_t *level,
					   uint8_t *custom);
...
ant_channel_err_t ant_channel_search_timeout_set(uint8_t channel,
						 uint8_t timeout);
ant_channel_err_t ant_channel_lp_search_timeout_set(uint8_t channel,
						    uint8_t timeout);
 ...
 * exactly {ANT_CH_CRC_MODE_DEFAULT}. A backend that accepted and ignored a
 ...
 */
#define ANT_CH_CRC_MODE_DEFAULT 0x00u
ant_channel_err_t ant_channel_crc_mode_set(uint8_t channel, uint8_t mode);
ant_channel_err_t ant_channel_crc_mode_get(uint8_t channel, uint8_t *out);
...
ant_channel_err_t ant_channel_freq_hop_table_set(uint8_t channel, uint8_t freq0,
						 uint8_t freq1, uint8_t freq2);
ant_channel_err_t ant_channel_freq_hop_table_get(uint8_t channel,
						 uint8_t out[3]);

/* ---------------------------------------------------------------------------
 * Search configuration held per channel
 *
 * ant_search.c owns the sweep, the seen-cache and scan mode; it does not own
 ...
 * Returns OK, INVALID_MESSAGE or WRONG_STATE (unassigned). */
#define ANT_CH_SEARCH_WAVEFORM_DEFAULT 316u
ant_channel_err_t ant_channel_search_waveform_set(uint8_t channel,
						  uint16_t waveform);
ant_channel_err_t ant_channel_search_waveform_get(uint8_t channel,
						  uint16_t *out);
...
#define ANT_CH_PROX_THRESHOLD_MAX 10u
ant_channel_err_t ant_channel_prox_search_set(uint8_t channel,
					      uint8_t threshold,
					      uint8_t custom);
ant_channel_err_t ant_channel_prox_search_get(uint8_t channel,
					      uint8_t *threshold,
					      uint8_t *custom);
...
ant_channel_err_t ant_channel_search_priority_set(uint8_t channel,
						  uint8_t priority);
ant_channel_err_t ant_channel_search_priority_get(uint8_t channel,
						  uint8_t *out);
...
ant_channel_err_t ant_channel_sharing_cycles_set(uint8_t channel,
						 uint8_t cycles);
ant_channel_err_t ant_channel_sharing_cycles_get(uint8_t channel,
						 uint8_t *out);
 ...
 */
#define ANT_CH_SDU_MASK_OFF 0xFFu /* ANTW_INVALID_SDU_MASK */
ant_channel_err_t ant_channel_sdu_mask_config_set(uint8_t channel,
						  uint8_t mask_config);
ant_channel_err_t ant_channel_sdu_mask_config_get(uint8_t channel,
						  uint8_t *out);
 ...
 * Returns OK or INVALID_MESSAGE.
 */
ant_channel_err_t ant_channel_status_get(uint8_t channel, uint8_t *out);

/* The state itself, for the other core modules. Out-of-range answers
 * ANT_CH_STATE_UNASSIGNED, so a caller that forgot to range-check gets the
 * least dangerous answer rather than a read past the array. */
enum ant_channel_state ant_channel_state_get(uint8_t channel);

/* True for the three master channel types. False for an unassigned channel. */
bool ant_channel_is_master(uint8_t channel);

/* True while the channel is on air: SEARCHING, TRACKING or CLOSING. This is
 * the predicate behind ANT_CH_ERR_NOT_OPENED, and it deliberately includes
...
bool ant_channel_is_open(uint8_t channel);

/* The channel type and the extended assignment flags as assigned. Zero on an
 * unassigned channel. */
uint8_t ant_channel_type_get(uint8_t channel);
uint8_t ant_channel_ext_assign_get(uint8_t channel);
uint8_t ant_channel_network_get(uint8_t channel);

/* ---------------------------------------------------------------------------
 * The scheduler seam
 *
 * Everything below is called by ant_sched.c and by nothing else. None of it
 ...
 * ANT_TIME_NEVER when the channel is not on air.
 *
 * ant_sched.c adds its own guard either side and subtracts caps.ramp_up_us and
 ...
 */
ant_time_t ant_channel_next_slot(uint8_t channel);

/* The absolute instant this channel's search gives up, or ANT_TIME_NEVER for
 * an infinite timeout or a channel that is not searching. */
ant_time_t ant_channel_search_deadline(uint8_t channel);

/* The earliest next_slot across all channels, and which channel it belongs to.
 * *channel_out is untouched when the answer is ANT_TIME_NEVER. */
ant_time_t ant_channel_earliest_slot(uint8_t *channel_out);

/*
 * Attribute a radio operation to a channel.
 *
 * `op` is the non-zero id ant_radio_tx()/ant_radio_rx() returned. Binding it
 ...
 * ant_channel_stale_op_count() is NOT incremented, because that counter means
 ...
 */
void ant_channel_bind_op(uint8_t channel, uint32_t op);

/* Which channel owns `op`, or -1. Never matches op == 0. */
int ant_channel_op_owner(uint32_t op);
 ...
 * ant_channel_on_acquired(); a CLOSING channel is leaving and must not have
 * its clock re-anchored on the way out.
 */
void ant_channel_on_slot(uint8_t channel, ant_time_t t_sync);
 ...
 */
void ant_channel_on_acquired(uint8_t channel, const struct ant_channel_id *id,
			     ant_time_t t_sync);
 ...
 * tracking - and counts the miss. After ANT_CHANNEL_RX_FAIL_TO_SEARCH
 * consecutive misses a TRACKING channel returns to SEARCHING and
 * ANT_CH_EVENT_RX_FAIL_GO_TO_SEARCH is raised. Returns true if that happened.
 */
bool ant_channel_on_slot_missed(uint8_t channel, ant_time_t now);
 ...
 * ANT_CH_EVENT_CHANNEL_CLOSED. Every other state simply drops the binding -
 * the window closed, and choosing the next one is ant_sched.c's business.
 *
 * Returns the channel the event was attributed to, or -1 if the op was stale.
 */
int ant_channel_on_terminal(uint32_t op, enum ant_radio_status status,
			    ant_time_t now);
 ...
 * ANT_CH_EVENT_RX_SEARCH_TIMEOUT and then closes, raising
 * ANT_CH_EVENT_CHANNEL_CLOSED as well - both, in that order, because that is
 ...
 */
uint32_t ant_channel_tick(ant_time_t now);
...
uint32_t ant_channel_stale_op_count(void);

/* Channels closed by a search timeout since init. */
uint32_t ant_channel_search_timeout_count(void);

/* ---------------------------------------------------------------------------
 * Unit conversion, exposed because ant_sched.c and the tests both need it and
 ...
 */
static inline ant_time_t ant_channel_counts_to_us(uint32_t counts)
{
	return (((ant_time_t)counts * 15625u) + 256u) / 512u;
}

/* Search timeout ticks (2.5 s each) to microseconds. Not defined for
 * ANT_CHANNEL_SEARCH_INFINITE - callers test for that first. */
static inline ant_time_t ant_channel_search_ticks_to_us(uint8_t ticks)
{
	return (ant_time_t)ticks * ANT_CHANNEL_SEARCH_TICK_US;
}
 ...
 * this header - ant_api.c, in the firmware build, which is required to. That
 * is deliberate: the check runs everywhere both headers exist, and this header
 * still compiles standalone against nothing but ant_core/include, which is
 * what the module's own gate needs.
 * ---------------------------------------------------------------------------
 */
#ifdef ANTW_RESPONSE_NO_ERROR
_Static_assert(ANT_CH_OK == ANTW_RESPONSE_NO_ERROR, "wire code drift: OK");
_Static_assert(ANT_CH_ERR_WRONG_STATE == ANTW_CHANNEL_IN_WRONG_STATE,
	       "wire code drift: CHANNEL_IN_WRONG_STATE");
_Static_assert(ANT_CH_ERR_NOT_OPENED == ANTW_CHANNEL_NOT_OPENED,
	       "wire code drift: CHANNEL_NOT_OPENED");
_Static_assert(ANT_CH_ERR_ID_NOT_SET == ANTW_CHANNEL_ID_NOT_SET,
	       "wire code drift: CHANNEL_ID_NOT_SET");
_Static_assert(ANT_CH_ERR_INVALID_MESSAGE == ANTW_INVALID_MESSAGE,
	       "wire code drift: INVALID_MESSAGE");
_Static_assert(ANT_CH_ERR_INVALID_NETWORK == ANTW_INVALID_NETWORK_NUMBER,
	       "wire code drift: INVALID_NETWORK_NUMBER");
_Static_assert(ANT_CH_ERR_INVALID_PARAM == ANTW_INVALID_PARAMETER_PROVIDED,
	       "wire code drift: INVALID_PARAMETER_PROVIDED");
_Static_assert(ANT_CH_EVENT_RX_SEARCH_TIMEOUT == ANTW_EVENT_RX_SEARCH_TIMEOUT,
	       "wire code drift: EVENT_RX_SEARCH_TIMEOUT");
_Static_assert(ANT_CH_EVENT_CHANNEL_CLOSED == ANTW_EVENT_CHANNEL_CLOSED,
	       "wire code drift: EVENT_CHANNEL_CLOSED");
_Static_assert(ANT_CH_EVENT_RX_FAIL_GO_TO_SEARCH ==
		       ANTW_EVENT_RX_FAIL_GO_TO_SEARCH,
	       "wire code drift: EVENT_RX_FAIL_GO_TO_SEARCH");
_Static_assert(ANT_CH_STATUS_UNASSIGNED == ANTW_STATUS_UNASSIGNED_CHANNEL,
	       "wire code drift: STATUS_UNASSIGNED_CHANNEL");
_Static_assert(ANT_CH_STATUS_ASSIGNED == ANTW_STATUS_ASSIGNED_CHANNEL,
	       "wire code drift: STATUS_ASSIGNED_CHANNEL");
_Static_assert(ANT_CH_STATUS_SEARCHING == ANTW_STATUS_SEARCHING_CHANNEL,
	       "wire code drift: STATUS_SEARCHING_CHANNEL");
_Static_assert(ANT_CH_STATUS_TRACKING == ANTW_STATUS_TRACKING_CHANNEL,
	       "wire code drift: STATUS_TRACKING_CHANNEL");
_Static_assert(ANT_CH_STATUS_STATE_MASK == ANTW_STATUS_CHANNEL_STATE_MASK,
	       "wire code drift: STATUS_CHANNEL_STATE_MASK");
_Static_assert(ANT_CH_TYPE_MASTER_BIT == ANTW_CHANNEL_TYPE_MASTER,
