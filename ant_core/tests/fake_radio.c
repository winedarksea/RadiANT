...
/*
 * fake_radio.c - an in-memory backend for ant_core/include/ant_radio_hal.h.
 *
 * Provenance: original clean-room work. Written against
 * ant_core/include/ant_radio_hal.h (keystone K3), docs/backends.md,
 ...
 * The shape of it
 * ---------------------------------------------------------------------------
 * This file defines every function ant_radio_hal.h declares, so the unit-test
 ...
 *    behaviour ant_search.c depends on when it reads devnum_hi and the device
 *    type out of the body of a frame whose devnum_lo it recovered from the
 *    filter index.
 *
 * 3. NO WALL CLOCK, ANYWHERE. ant_radio_now() reads a variable. It changes
...
enum fake_lifecycle {
	LC_DOWN = 0,   /* before ant_radio_init() */
...
struct cur_op {
	...
	/* receive */
	struct ant_rx_filter filters[FAKE_RADIO_MAX_FILTERS];
	uint8_t              n_filters;
	uint32_t             flags;
	ant_time_t           t_open;
	ant_time_t           t_close;
	...
	/* transmit */
	ant_time_t t_sync_req;
	ant_time_t t_fire;

	bool                  force_terminal_set;
	enum ant_radio_status force_terminal;
};

struct fake_state {
	enum fake_lifecycle  lc;
	struct ant_radio_cbs cbs;
	void                *user;

	ant_time_t now;
	uint32_t   next_op;

	struct ant_radio_caps caps;
	...
	enum ant_radio_status pending_force;
	...
	uint8_t body_buf[ANT_RADIO_BODY_MAX];
};
...
static const enum ant_phy fake_phys_1m[] = { ANT_PHY_1M_GFSK };

static const struct ant_radio_cbs fake_cbs_none = { NULL, NULL };
 ...
 * callback that loops over 32 channels calling ant_radio_now() is doing
...
{
	struct ant_radio_caps *c = &g.caps;
	...
	c->max_body_len = ANT_RADIO_BODY_MAX;
	c->phys = fake_phys_1m;
	c->n_phys = 1u;
	c->phy_switch_us = 0u;
	c->ramp_up_us = 40u;
...
struct ant_radio_caps *fake_radio_caps_mut(void)
{
	return &g.caps;
}

void fake_radio_set_phys(const enum ant_phy *phys, uint8_t n_phys)
{
	g.caps.phys = phys;
	g.caps.n_phys = n_phys;
}

const struct ant_radio_caps *ant_radio_caps_get(void)
{
	isr_tick();
	return &g.caps;
}

static bool phy_supported(enum ant_phy phy)
{
	uint8_t i;
...
static bool caps_modellable(void)
{
	...
	if (g.caps.max_body_len == 0u ||
	    g.caps.max_body_len > ANT_RADIO_BODY_MAX) {
		record_viol(FAKE_RADIO_VIOL_CAPS_UNMODELLABLE,
			    "max_body_len outside 1..ANT_RADIO_BODY_MAX");
		return false;
	}
	return true;
}
...
static void count_status(enum ant_radio_status st, bool is_tx)
{
	if (is_tx) {
		g.stats.ev_tx++;
	}
...
static bool has_frame_fields(enum ant_radio_status st)
{
	return st == ANT_RADIO_STATUS_OK || st == ANT_RADIO_STATUS_CRC_FAIL;
}

static void deliver_rx(uint32_t op, enum ant_radio_status status, bool terminal,
		       ant_time_t t_sync, uint8_t filter_index,
...
static void deliver_tx(uint32_t op, enum ant_radio_status status,
		       ant_time_t t_sync, bool late)
{
	struct ant_tx_event evt;
	struct fake_radio_event *rec;
	...
	evt.op = op;
	evt.status = status;
	evt.t_sync = (status == ANT_RADIO_STATUS_OK) ? t_sync : 0u;
	evt.t_sync_exact = (status == ANT_RADIO_STATUS_OK) &&
...
static void mark_terminal(int32_t arm_idx, enum ant_radio_status st)
{
 ...
 * return ANT_RADIO_EBUSY for the one call pattern the contract exists to
 * permit.
 */
static void end_op(enum ant_radio_status status, ant_time_t t_sync)
{
...
static void run_deferred(void)
{
	...
	while (g.defer_abort_op != 0u) {
		...
		if (kind == FAKE_RADIO_ARM_TX) {
			deliver_tx(op, ANT_RADIO_STATUS_ABORTED, 0u, false);
		} else {
			deliver_rx(op, ANT_RADIO_STATUS_ABORTED, true, 0u, 0u,
				   NULL, 0u, 0, false);
		}
	}
	g.draining = false;
...
int fake_radio_air_add(const struct fake_radio_air *f)
{
	...
	for (i = 0u; i < FAKE_RADIO_AIR_MAX; i++) {
		if (!g.air[i].used) {
			...
			return ANT_RADIO_OK_RC;
		}
	}
	record_viol(FAKE_RADIO_VIOL_QUEUE_FULL, "air queue full");
	return ANT_RADIO_EBUSY;
}

int fake_radio_air_frame(ant_time_t t_sync, const uint8_t *bytes, uint8_t len)
{
	struct fake_radio_air f;
...
uint32_t fake_radio_air_master(ant_time_t t_first, uint32_t period_us,
			       ...
			       uint8_t len)
{
	...
	for (i = 0u; i < count; i++) {
		ant_time_t t = t_first + (ant_time_t)period_us * (ant_time_t)i;

		if (fake_radio_air_frame(t, bytes, len) != ANT_RADIO_OK_RC) {
			break;
		}
		queued++;
	}
...
uint32_t fake_radio_air_noise(ant_time_t t0, ant_time_t t1, uint32_t count,
			      int8_t rssi_dbm)
{
	...
	for (i = 0u; i < count; i++) {
		...
		f.t_sync = t0;
		if (count > 1u && span > 0u) {
			f.t_sync = t0 + (ant_time_t)((span * (uint64_t)i) /
						     (uint64_t)(count - 1u));
		}
		f.crc_bad = true;
		f.rssi_set = true;
		...
		if (fake_radio_air_add(&f) != ANT_RADIO_OK_RC) {
			break;
		}
		queued++;
	}
...
{
	const struct ant_pkt_format *fmt = &g.cur.fmt;
	uint8_t avail;
	int32_t need;
...
static void mark_missed(ant_time_t before)
{
	uint32_t i;
...
static void clock_to(ant_time_t t)
{
	if (t <= g.now) {
		return;
	}
...
static bool next_due(ant_time_t *t_out, enum due_kind *kind_out,
		     int32_t *air_out)
{
	uint32_t i;
...
static void dispatch_air(int32_t idx)
{
	...
	if (!crc_ok && (g.cur.flags & ANT_RX_REPORT_CRC_FAIL) == 0u) {
	...
	status = crc_ok ? ANT_RADIO_STATUS_OK : ANT_RADIO_STATUS_CRC_FAIL;
	rssi = s->f.rssi_set ? s->f.rssi_dbm : g.default_rssi;

	/*
	 * ANT_RX_STOP_ON_FIRST closes the window on the first *accepted* frame,
	 ...
	 */
	terminal = crc_ok && ((g.cur.flags & ANT_RX_STOP_ON_FIRST) != 0u);
...
static void dispatch(enum due_kind kind, int32_t air_idx)
{
	...
	case DUE_AIR:
		...
		break;
	case DUE_CLOSE: {
		enum ant_radio_status st = g.cur.force_terminal_set
						   ? g.cur.force_terminal
						   : ANT_RADIO_STATUS_TIMEOUT;
		end_op(st, 0u);
		break;
	}
	case DUE_TX: {
...
static void run_until(ant_time_t target)
{
	uint32_t guard = 0u;

	for (;;) {
		ant_time_t due = ANT_TIME_NEVER;
...
void fake_radio_set_origin(ant_time_t t)
{
	if (reject_from_callback()) {
		return;
	}
...
void fake_radio_advance_to(ant_time_t t)
{
	if (reject_from_callback()) {
		return;
	}
...
{
	fake_radio_advance_to(g.now + (ant_time_t)us);
}

ant_time_t fake_radio_next_due(void)
{
	ant_time_t due = ANT_TIME_NEVER;
	enum due_kind kind = DUE_NONE;
	int32_t air_idx = -1;
...
{
	switch (rc) {
	case ANT_RADIO_EINVAL:
		g.stats.rc_einval++;
		break;
	case ANT_RADIO_ENOTSUP:
		g.stats.rc_enotsup++;
		break;
	case ANT_RADIO_EBUSY:
		g.stats.rc_ebusy++;
		break;
	case ANT_RADIO_ETIME:
		g.stats.rc_etime++;
		break;
	case ANT_RADIO_ESTATE:
		g.stats.rc_estate++;
		break;
	default:
		break;
 ...
 * at the arm call.
 */
static int check_fmt(const struct ant_pkt_format *fmt)
{
	if (fmt == NULL) {
		return ANT_RADIO_EINVAL;
	}
	if ((unsigned int)fmt->phy >= (unsigned int)ANT_PHY_COUNT) {
		return ANT_RADIO_EINVAL;
	}
	if (!phy_supported(fmt->phy)) {
		return ANT_RADIO_ENOTSUP;
	}
	if (fmt->addr_len < 2u || fmt->addr_len > ANT_RADIO_ADDR_MAX) {
		return ANT_RADIO_EINVAL;
	}
	if (fmt->max_body_len == 0u || fmt->max_body_len > ANT_RADIO_BODY_MAX) {
		return ANT_RADIO_EINVAL;
	}
...
static bool too_late(ant_time_t t)
{
	return t < g.now + (ant_time_t)g.caps.min_arm_lead_us;
}
...
int ant_radio_init(const struct ant_radio_cbs *cbs, void *user)
{
	isr_tick();
	if (g.in_cb) {
		record_viol(FAKE_RADIO_VIOL_ISR_LIFECYCLE,
			    "ant_radio_init() from a callback");
		return ANT_RADIO_ESTATE;
	}
	if (cbs == NULL) {
		return ANT_RADIO_EINVAL;
...
int ant_radio_enable(void)
{
	isr_tick();
	if (g.in_cb) {
		record_viol(FAKE_RADIO_VIOL_ISR_LIFECYCLE,
			    "ant_radio_enable() from a callback");
		return ANT_RADIO_ESTATE;
	}
	if (g.lc == LC_DOWN) {
		return ANT_RADIO_ESTATE;
...
int ant_radio_disable(void)
{
	isr_tick();
	if (g.in_cb) {
		record_viol(FAKE_RADIO_VIOL_ISR_LIFECYCLE,
			    "ant_radio_disable() from a callback");
		return ANT_RADIO_ESTATE;
	}
	if (g.lc == LC_DOWN) {
		return ANT_RADIO_ESTATE;
	...
	if (g.cur.kind != FAKE_RADIO_ARM_NONE) {
		end_op(ANT_RADIO_STATUS_ABORTED, 0u);
	}
	g.lc = LC_INIT;
	return ANT_RADIO_OK_RC;
}

ant_time_t ant_radio_now(void)
{
	isr_tick();
	return g.now;
}
...
static int arm_preflight(void)
{
	...
	if (g.force_arm_set) {
		...
		g.force_arm_rc = ANT_RADIO_OK_RC;
		if (rc != ANT_RADIO_OK_RC) {
			g.stats.rc_forced++;
			return rc;
		}
...
int ant_radio_tx(const struct ant_tx_req *req, uint32_t *op)
{
	...
	if (req->body != NULL && req->body_len > 0u &&
	    req->body_len <= ANT_RADIO_BODY_MAX) {
	...
	if (rc == ANT_RADIO_OK_RC && req->rf_index > ANT_RF_INDEX_MAX) {
		rc = ANT_RADIO_EINVAL;
	}
	if (rc == ANT_RADIO_OK_RC) {
		if (req->body == NULL || req->body_len == 0u ||
	...
	if (rc == ANT_RADIO_OK_RC) {
		...
		if (req->fmt->len_mode == ANT_LEN_FIXED) {
	...
	if (rc == ANT_RADIO_OK_RC) {
		if (req->t_sync_at == ANT_TIME_NEVER) {
			rc = ANT_RADIO_EINVAL;
	...
	if (g.tx_offset_us > 0) {
		g.cur.t_fire += (ant_time_t)g.tx_offset_us;
	} else if (g.tx_offset_us < 0) {
		ant_time_t back = (ant_time_t)(-(int64_t)g.tx_offset_us);
...
int ant_radio_rx(const struct ant_rx_req *req, uint32_t *op)
{
	struct fake_radio_arm *rec;
	int32_t rec_idx;
	int rc;
	...
	if (req->filters != NULL && req->n_filters > 0u &&
		...
		memcpy(rec->filters, req->filters,
		       sizeof(struct ant_rx_filter) * req->n_filters);
	}
	...
	if (rc == ANT_RADIO_OK_RC) {
		const uint32_t known = ANT_RX_STOP_ON_FIRST |
				       ANT_RX_REPORT_CRC_FAIL;

		if ((req->flags & ~known) != 0u) {
			rc = ANT_RADIO_EINVAL;
		} else if ((req->flags & ANT_RX_STOP_ON_FIRST) != 0u &&
			   req->n_filters > 1u) {
	...
	if (rc == ANT_RADIO_OK_RC) {
		if (req->t_open == ANT_TIME_NEVER ||
		    req->t_close == ANT_TIME_NEVER) {
			/* ANT_TIME_NEVER is never a valid arm time, so an
...
int ant_radio_abort(void)
{
	...
	if (g.in_cb) {
		 ...
		 * unambiguous.
		 */
		mark_terminal(g.cur.arm_idx, ANT_RADIO_STATUS_ABORTED);
...
{
	g.force_arm_rc = rc;
	g.force_arm_set = (rc != ANT_RADIO_OK_RC);
}

void fake_radio_force_next_terminal(enum ant_radio_status status)
{
	/*
	 ...
	 * ended by ANT_RX_STOP_ON_FIRST or by abort keeps the status that
			      ...
			      uint8_t filter_index)
{
	...
	body = f->bytes;
	body_len = f->len;
	if (body_len > ANT_RADIO_BODY_MAX) {
		body_len = ANT_RADIO_BODY_MAX;
	}
	deliver_rx(op,
		   f->crc_bad ? ANT_RADIO_STATUS_CRC_FAIL : ANT_RADIO_STATUS_OK,
		   false, f->t_sync, filter_index, body, body_len,
		   f->rssi_set ? f->rssi_dbm : g.default_rssi, true);
	return ANT_RADIO_OK_RC;
}

int fake_radio_inject_late_tx(uint32_t op, enum ant_radio_status status)
{
	if (reject_from_callback()) {
		return ANT_RADIO_ESTATE;
	}
...
{
	fake_radio_reset();
	(void)ant_radio_init(&fake_cbs_none, NULL);
	(void)ant_radio_enable();
}
