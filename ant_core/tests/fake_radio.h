 ...
 * ant_core/include/ant_radio_hal.h, docs/backends.md, docs/ant-radio-link.md
 ...
 * What this is
 * ---------------------------------------------------------------------------
 * ant_core/tests/fake_radio.c defines every function in ant_radio_hal.h. The
 ...
 *     module that forgets a field gets ANT_RADIO_EINVAL rather than silently
 ...
 *
 *           ant_time_t t = ant_radio_now() + 100000u;
 *           fake_radio_air_frame(t, frame, n);
 *
 *           struct ant_rx_req req = { ... .t_open = t - 500, .t_close = t + 500 };
 *           uint32_t op;
 *           zassert_ok(ant_radio_rx(&req, &op));
...
#ifndef ANT_CORE_TESTS_FAKE_RADIO_H_
#define ANT_CORE_TESTS_FAKE_RADIO_H_
...
#define FAKE_RADIO_AIR_FRAME_MAX   (ANT_RADIO_ADDR_MAX + ANT_RADIO_BODY_MAX)
 ...
 */
#define FAKE_RADIO_T_ORIGIN        ((ant_time_t)0xFFF0BDC0ULL)
 ...
 *   ant_search.c must rank and gate on CRC and never on match count, and
 ...
 * ant_search.c's recovery of devnum_hi and the device type out of the *body*
...
enum fake_radio_match {
	 ...
	 * how ant_search.c recovers devnum_lo from filter_index. */
	 ...
	 * a frame. Delivered as ANT_RADIO_STATUS_CRC_FAIL with filter_index
...
struct fake_radio_air {
	 ...
	 * ant_radio_hal.h - this is the same instant, from the transmitter's
	 * side. */
	ant_time_t t_sync;
...
struct fake_radio_arm {
	...
	ant_time_t               t_arm;   /* ant_radio_now() at the call */
	...
	bool                  terminated;
	enum ant_radio_status terminal_status;
	ant_time_t            t_terminal;
...
struct fake_radio_event {
	...
	bool                     terminal;
	ant_time_t               t;      /* ant_radio_now() at delivery */
	ant_time_t               t_sync;
	bool                     t_sync_exact;
...
/* reset() + ant_radio_init() with both callbacks NULL + ant_radio_enable().
 ...
 * ant_radio_now() trips it. Default 16; 0 disables the check. */
 ...
 * with ant_radio_now() reading the event's own instant while its callback
...
/* The instant of the next event the mock would fire, or ANT_TIME_NEVER if
 * nothing is pending. */
 ...
 *
 * ant_sched.c must never hardcode 8 and ant_search.c must never hardcode a
...
/* Returns ANT_RADIO_OK_RC, or ANT_RADIO_EINVAL / a full queue. */
 ...
 * ANT_RX_REPORT_CRC_FAIL flag clear these are invisible to the core and only
 ...
 * recorded. Chiefly for ANT_RADIO_EBUSY: the HAL says a thread-context arm
...
#endif /* ANT_CORE_TESTS_FAKE_RADIO_H_ */
