 ...
 * ant_core/include/ant_search.h, ant_core/include/ant_frame.h,
 ...
 * ---------------------------------------------------------------------------
 * ant_search.c is policy, not plumbing, so every test here is about a decision
...
struct capture {
	...
	struct ant_search_result res[CAP_MAX];
...
static void on_acquired(uint8_t channel, const struct ant_search_result *r, void *user)
{
	ARG_UNUSED(user);
...
static const struct ant_search_cbs search_cbs = {
 ...
 * ant_sched.c will do. Nothing in this path calls the HAL, so the mock's ISR
 * call budget is never at risk and a violation from this suite means a real
 * contract breach rather than a noisy fixture.
 */
static void rx_cb(const struct ant_rx_event *evt, void *user)
{
	ARG_UNUSED(user);
	ant_search_on_rx(&g_s, evt);
}

static const struct ant_radio_cbs radio_cbs = {
	.rx = rx_cb,
	.tx = NULL,
};
...
static const struct ant_search_id_filter want_any = { 0u, 0u, 0u };

static void bring_up(bool report_crc_fail)
{
	struct ant_search_cfg cfg;
	...
	zassert_ok(ant_search_init(&g_s, &cfg, &search_cbs, NULL),
		   "ant_search_init failed");
	zassert_ok(ant_radio_init(&radio_cbs, NULL));
...
ZTEST_SUITE(ant_search, NULL, NULL, setup, NULL, NULL);

/* ---------------------------------------------------------------------------
 * The pump - a two-line stand-in for ant_sched.c
 *
...
static struct ant_search_window g_w;
static uint32_t g_op;

/* Ask for a window and arm it. Returns the ant_search_window() code. */
static int win_open(void)
{
	 ...
	 * now + min_arm_lead_us is the convention ant_search_window() documents,
	 * and getting it wrong here would show up as ANT_RADIO_ETIME below.
	 */
	earliest = ant_radio_now() + ant_radio_caps_get()->min_arm_lead_us + 100u;

	rc = ant_search_window(&g_s, earliest, &g_w);
	if (rc != ANT_SEARCH_OK) {
		return rc;
	}
	...
	zassert_ok(ant_radio_rx(&req, &g_op), "arming the search window failed");
	ant_search_armed(&g_s, g_op, req.t_open, req.t_close);
	return ANT_SEARCH_OK;
}
...
static int win_open_truncated(uint32_t len_us)
{
	...
	zassert_ok(ant_radio_rx(&req, &g_op));
	ant_search_armed(&g_s, g_op, req.t_open, req.t_close);
	return ANT_SEARCH_OK;
}
...
static void win_run(void)
{
	...
	g_windows_run++;
	ant_search_tick(&g_s, ant_radio_now());
}
...
ZTEST(ant_search, test_sweep_geometry_nrf)
{
	bring_up(false);

	zassert_equal(8u, ant_search_filters_per_window(&g_s));
	zassert_equal(32u, ant_search_sets(&g_s),
	...
	zassert_equal(32ULL * ANT_SEARCH_DWELL_DEFAULT_US, ant_search_sweep_us(&g_s));

	end_of_test();
}

ZTEST(ant_search, test_sweep_geometry_rail)
{
	fake_radio_caps_preset_rail();
	bring_up(false);

	zassert_equal(2u, ant_search_filters_per_window(&g_s));
	zassert_equal(128u, ant_search_sets(&g_s),
	...
	zassert_true(ant_search_sweep_us(&g_s) > 25000000ULL);
...
ZTEST(ant_search, test_wildcard_capability_is_refused)
{
	struct ant_search_cfg cfg;
	...
	zassert_equal(ANT_SEARCH_ENOTSUP,
		      ant_search_init(&g_s, &cfg, &search_cbs, NULL),
		      "struct ant_rx_filter has no wildcard field, so a backend "
...
ZTEST(ant_search, test_no_channel_means_no_window)
{
	struct ant_search_window w;

	bring_up(false);

	zassert_equal(ANT_SEARCH_ENOENT,
		      ant_search_window(&g_s, ant_radio_now() + 1000u, &w),
		      "an idle search must not ask for the radio");
	zassert_equal(0u, ant_search_n_searching(&g_s));

	end_of_test();
}

ZTEST(ant_search, test_window_shape_and_filter_addresses)
{
	bring_up(false);
	zassert_ok(ant_search_begin(&g_s, 0u, ANT_SEARCH_MODE_ACQUIRE, &want_any,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));

	zassert_equal(ANT_SEARCH_OK, win_open());

	zassert_equal_ptr(ant_frame_format(ANT_FRAME_CFG_SEARCH), g_w.fmt,
			  "search must use the measured PCNF0=0 / BALEN=2 / "
			  "STATLEN=12 geometry");
	zassert_equal(ANT_RF_INDEX_ANT_PLUS, g_w.rf_index);
	zassert_equal(8u, g_w.n_filters);
	zassert_equal(0u, g_w.set_index);
	zassert_false(g_w.from_cache);
	zassert_equal(ANT_SEARCH_DWELL_DEFAULT_US, (uint32_t)(g_w.t_close - g_w.t_open),
		      "dwell is one channel period; a shorter one loses the "
		      "within-one-sweep guarantee for nothing");
	zassert_equal(0u, g_w.flags & ANT_RX_STOP_ON_FIRST,
	...
	for (uint8_t j = 0; j < g_w.n_filters; j++) {
		...
		zassert_equal(ANT_NET_ADDR_ANT_PLUS_0, g_w.filters[j].addr[0]);
		zassert_equal(ANT_NET_ADDR_ANT_PLUS_1, g_w.filters[j].addr[1]);
		zassert_equal(j, g_w.filters[j].addr[2]);
	}

	win_run();
	zassert_ok(ant_search_end(&g_s, 0u));
	end_of_test();
}

ZTEST(ant_search, test_window_shape_at_two_filters)
{
	fake_radio_caps_preset_rail();
	bring_up(false);
	zassert_ok(ant_search_begin(&g_s, 0u, ANT_SEARCH_MODE_ACQUIRE, &want_any,
 ...
 * than on the air. */
ZTEST(ant_search, test_crc_coverage_is_fifteen_either_way)
{
	zassert_equal(15, ant_frame_covered_len(ANT_FRAME_CFG_SEARCH,
						ANT_FRAME_PAYLOAD_STD));
	zassert_equal(15, ant_frame_covered_len(ANT_FRAME_CFG_TRACKING,
						ANT_FRAME_PAYLOAD_STD));
	end_of_test();
}
...
ZTEST(ant_search, test_acquires_within_one_sweep)
{
	uint8_t frame[FAKE_RADIO_AIR_FRAME_MAX];
	uint8_t len;
	uint32_t windows;
	const struct ant_search_stats *st;
	...
	(void)fake_radio_air_master(ant_radio_now() + 1000u, FAKE_RADIO_ANT_PERIOD_US,
				    45u, frame, len);

	zassert_ok(ant_search_begin(&g_s, 3u, ANT_SEARCH_MODE_ACQUIRE, &want_any,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));

	windows = pump(ant_search_sets(&g_s) + 1u);

	zassert_equal(1u, g_cap.n_acquired, "not acquired within one sweep");
	zassert_true(windows <= ant_search_sets(&g_s),
	...
	zassert_equal(ANT_FRAME_PAYLOAD_STD, g_cap.res[0].frame.payload_len);
	zassert_mem_equal(payload8, g_cap.res[0].frame.payload, sizeof(payload8));
	zassert_equal(ANT_CTRL_BROADCAST, g_cap.res[0].frame.ctrl_byte);

	/* Acquire mode: the channel has left the search. */
	zassert_false(ant_search_is_searching(&g_s, 3u));
	zassert_equal(0u, ant_search_n_searching(&g_s));

	st = ant_search_get_stats(&g_s);
 ...
 * pins separately.
 */
ZTEST(ant_search, test_acquires_at_two_filters)
{
	uint8_t frame[FAKE_RADIO_AIR_FRAME_MAX];
	uint8_t len;
...
ZTEST(ant_search, test_devnum_lo_from_filter_index_not_body)
{
	...
	bring_up(false);
	zassert_ok(ant_search_begin(&g_s, 1u, ANT_SEARCH_MODE_ACQUIRE, &want_any,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
	 ...
	 *
	 * If ant_search.c ever parses the low byte out of the frame, this reads
 ...
 *
 * ant_sched.c combines several channels' requests into one hardware window, so
 ...
 */
ZTEST(ant_search, test_merged_window_uses_the_supplied_filter_index)
{
	struct ant_rx_event evt;
	struct ant_search_window w;
	uint8_t body[ANT_FRAME_HDR_LEN_SEARCH + ANT_FRAME_PAYLOAD_STD];

	bring_up(false);
	zassert_ok(ant_search_begin(&g_s, 2u, ANT_SEARCH_MODE_ACQUIRE, &want_any,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));

	zassert_ok(ant_search_window(&g_s, ant_radio_now() + 1000u, &w));
	...
	ant_search_on_rx_indexed(&g_s, &evt, 6u);
	...
	zassert_equal(0u, ant_search_get_stats(&g_s)->late_events,
		      "the EXTERNAL sentinel means the caller owns the routing");

	ant_search_on_done(&g_s, true);
	end_of_test();
}
 ...
 * ant_search.c because indexing filter_lo[] with an unchecked value from a
...
ZTEST(ant_search, test_noise_with_bad_crc_never_acquires)
{
	const struct ant_search_stats *st;
	uint32_t queued;
	 ...
	 */
	queued = fake_radio_air_noise(ant_radio_now() + 1000u,
				      ant_radio_now() + 3u * ANT_SEARCH_DWELL_DEFAULT_US,
	...
	zassert_equal(0u, g_cap.n_acquired,
		      ...
		      "nothing else");
	zassert_equal(0u, ant_search_seen_count(&g_s, ant_radio_now()),
		      "noise must not populate the seen cache either");
	zassert_true(ant_search_is_searching(&g_s, 0u),
...
ZTEST(ant_search, test_noise_is_suppressed_by_default)
{
	const struct ant_search_stats *st;
	...
	(void)fake_radio_air_noise(ant_radio_now() + 1000u,
				   ant_radio_now() + 2u * ANT_SEARCH_DWELL_DEFAULT_US,
...
ZTEST(ant_search, test_signal_is_found_through_noise)
{
	uint8_t frame[FAKE_RADIO_AIR_FRAME_MAX];
	uint8_t len;
	...
	(void)fake_radio_air_master(ant_radio_now() + 1000u, FAKE_RADIO_ANT_PERIOD_US,
				    4u, frame, len);
	(void)fake_radio_air_noise(ant_radio_now() + 1500u,
				   ant_radio_now() + ANT_SEARCH_DWELL_DEFAULT_US,
	...
	zassert_true(ant_search_get_stats(&g_s)->crc_fail > 0u);
...
ZTEST(ant_search, test_one_window_serves_several_channels)
{
	struct ant_search_id_filter want_pwr = { 0u, 0x0Bu, 0u };
	struct ant_search_id_filter want_hrm = { 0u, 0x78u, 0u };
	...
	zassert_ok(ant_search_begin(&g_s, 0u, ANT_SEARCH_MODE_ACQUIRE, &want_pwr,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
	zassert_ok(ant_search_begin(&g_s, 1u, ANT_SEARCH_MODE_ACQUIRE, &want_hrm,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
	zassert_equal(2u, ant_search_n_searching(&g_s));
...
ZTEST(ant_search, test_a_channel_only_gets_what_it_asked_for)
{
	struct ant_search_id_filter want_hrm = { 0u, 0x78u, 0u };
	uint8_t frame[FAKE_RADIO_AIR_FRAME_MAX];
	uint8_t len;
	...
	bring_up(false);
	zassert_ok(ant_search_begin(&g_s, 0u, ANT_SEARCH_MODE_ACQUIRE, &want_hrm,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
	...
	zassert_equal(1u, ant_search_get_stats(&g_s)->frames_ok);
	zassert_equal(1u, ant_search_seen_count(&g_s, ant_radio_now()));
	zassert_true(ant_search_is_searching(&g_s, 0u));
...
ZTEST(ant_search, test_pairing_bit_matches_and_survives)
{
	struct ant_search_id_filter want_pwr = { 0u, 0x0Bu, 0u };
	uint8_t frame[FAKE_RADIO_AIR_FRAME_MAX];
	uint8_t len;
	...
	len = fake_radio_build_ant_frame(frame, 0x0804u,
					 (uint8_t)(0x0Bu | ANT_DEVICE_TYPE_PAIRING_BIT),
					 5u, payload8);
	zassert_ok(fake_radio_air_frame(g_w.t_open + 1000u, frame, len));
	win_run();
	...
	zassert_equal((uint8_t)(0x0Bu | ANT_DEVICE_TYPE_PAIRING_BIT),
...
ZTEST(ant_search, test_seen_cache_reacquires_in_one_window)
{
	struct ant_search_id_filter want_pwr = { 0u, 0x0Bu, 0u };
	uint8_t frame[FAKE_RADIO_AIR_FRAME_MAX];
	uint8_t len;
	...
	zassert_ok(ant_search_begin(&g_s, 0u, ANT_SEARCH_MODE_ACQUIRE, &want_pwr,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
	first_windows = pump(ant_search_sets(&g_s) + 1u);
...
ZTEST(ant_search, test_named_device_number_steers_directly)
{
	struct ant_search_id_filter want_one = { 0x09A1u, 0u, 0u };

	bring_up(false);
	zassert_ok(ant_search_begin(&g_s, 0u, ANT_SEARCH_MODE_ACQUIRE, &want_one,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
...
ZTEST(ant_search, test_seen_cache_expires_at_sixty_seconds)
{
	struct ant_search_id_filter want_pwr = { 0u, 0x0Bu, 0u };
	struct ant_channel_id id = { 0x09A1u, 0x0Bu, 5u };
	ant_time_t t0;

	bring_up(false);

	t0 = ant_radio_now();
	ant_search_seen_note(&g_s, &id, t0);
	zassert_equal(1u, ant_search_seen_count(&g_s, t0));
	zassert_true(ant_search_seen_lookup(&g_s, &want_pwr, t0, NULL));

	/* One microsecond inside the lifetime. */
	zassert_true(ant_search_seen_lookup(&g_s, &want_pwr,
					    t0 + ANT_SEARCH_SEEN_LIFETIME_US - 1u,
					    NULL));
	/* One microsecond outside it. */
	zassert_false(ant_search_seen_lookup(&g_s, &want_pwr,
					     t0 + ANT_SEARCH_SEEN_LIFETIME_US + 1u,
					     NULL));
	zassert_equal(0u, ant_search_seen_count(&g_s,
						t0 + ANT_SEARCH_SEEN_LIFETIME_US + 1u));

	/* And an expired entry must not steer: the sweep starts where the round
	 * robin left it. */
	fake_radio_advance(ANT_SEARCH_SEEN_LIFETIME_US + 1000000u);
	ant_search_tick(&g_s, ant_radio_now());
	zassert_equal(0u, ant_search_seen_count(&g_s, ant_radio_now()));
...
ZTEST(ant_search, test_seen_cache_refreshes_in_place)
{
	struct ant_channel_id a = { 0x1111u, 0x0Bu, 5u };
	struct ant_channel_id b = { 0x2222u, 0x78u, 1u };
	ant_time_t t = ant_radio_now();

	bring_up(false);

	ant_search_seen_note(&g_s, &b, t);
	for (uint32_t i = 0; i < 64u; i++) {
		ant_search_seen_note(&g_s, &a, t + i * 1000u);
	}
	zassert_equal(2u, ant_search_seen_count(&g_s, t + 64000u),
...
ZTEST(ant_search, test_scan_mode_reports_every_frame_and_never_leaves)
{
	...
	zassert_ok(ant_search_begin(&g_s, 0u, ANT_SEARCH_MODE_SCAN, &want_any,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
	...
	/*
	 * And the sweep keeps re-arming bounded windows, because ANT_TIME_NEVER
	 * is not a legal window edge - there is no open-ended receive to leave
	 * running.
	 */
	zassert_equal(ANT_SEARCH_OK, win_open());
	zassert_equal(1u, g_w.set_index);
	zassert_true(g_w.t_close != ANT_TIME_NEVER);
	win_run();
...
ZTEST(ant_search, test_truncated_windows_do_not_skip_a_set)
{
	const struct ant_search_stats *st;
	...
	for (uint32_t i = 0; i < 4u; i++) {
		zassert_equal(ANT_SEARCH_OK,
			      win_open_truncated(ANT_SEARCH_DWELL_DEFAULT_US / 4u));
		zassert_equal(0u, g_w.set_index,
...
ZTEST(ant_search, test_aborted_window_credits_nothing)
{
	bring_up(false);
	zassert_ok(ant_search_begin(&g_s, 0u, ANT_SEARCH_MODE_ACQUIRE, &want_any,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
...
ZTEST(ant_search, test_arm_failure_leaves_the_set_selected)
{
	struct ant_search_window w;
...
ZTEST(ant_search, test_second_window_while_one_is_armed_is_refused)
{
	struct ant_search_window w;
	...
	zassert_equal(ANT_SEARCH_OK, win_open());
	zassert_equal(ANT_SEARCH_ESTATE,
		      ant_search_window(&g_s, ant_radio_now() + 1000u, &w));
	win_run();
...
ZTEST(ant_search, test_search_timeout_fires_and_the_channel_leaves)
{
	bring_up(false);

	/* Two dwells' worth, so the timeout lands mid-sweep with nothing heard. */
	zassert_ok(ant_search_begin(&g_s, 7u, ANT_SEARCH_MODE_ACQUIRE, &want_any,
				    ant_radio_now(), 2u * ANT_SEARCH_DWELL_DEFAULT_US));
	...
	zassert_false(ant_search_is_searching(&g_s, 7u));
	zassert_equal(0u, ant_search_n_searching(&g_s));
	zassert_equal(1u, ant_search_get_stats(&g_s)->timeouts);

	/* And with nothing searching the module stops asking for the radio. */
	zassert_equal(ANT_SEARCH_ENOENT,
		      ant_search_window(&g_s, ant_radio_now() + 1000u, &g_w));

	end_of_test();
}

/* Scan mode's infinite search does not expire. */
ZTEST(ant_search, test_scan_mode_has_no_timeout)
{
	bring_up(false);
	zassert_ok(ant_search_begin(&g_s, 0u, ANT_SEARCH_MODE_SCAN, &want_any,
				    ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
	...
	(void)pump(4u);
	ant_search_tick(&g_s, ant_radio_now() + 10u * ANT_SEARCH_TIMEOUT_DEFAULT_US);
 ...
 */
ZTEST(ant_search, test_late_event_never_acquires)
{
	struct fake_radio_air air;
	uint8_t frame[FAKE_RADIO_AIR_FRAME_MAX];
	uint8_t len;
...
ZTEST(ant_search, test_rejects_bad_arguments)
{
	struct ant_search_cfg cfg;
	struct ant_search_window w;

	ant_search_cfg_default(&cfg);
	zassert_equal(ANT_SEARCH_EINVAL, ant_search_init(NULL, &cfg, NULL, NULL));
	zassert_equal(ANT_SEARCH_EINVAL, ant_search_init(&g_s, NULL, NULL, NULL));

	cfg.rf_index = (uint8_t)(ANT_RF_INDEX_MAX + 1u);
	zassert_equal(ANT_SEARCH_EINVAL, ant_search_init(&g_s, &cfg, NULL, NULL));
	...
	bring_up(false);
	zassert_equal(ANT_SEARCH_EINVAL,
		      ant_search_begin(&g_s, 0u, ANT_SEARCH_MODE_ACQUIRE, NULL,
				       ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
	zassert_equal(ANT_SEARCH_ENOSPC,
		      ant_search_begin(&g_s, (uint8_t)ANT_SEARCH_MAX_CHANNELS,
				       ANT_SEARCH_MODE_ACQUIRE, &want_any,
				       ant_radio_now(), ANT_SEARCH_TIMEOUT_NONE));
	zassert_equal(ANT_SEARCH_ENOENT, ant_search_end(&g_s, 0u));
	zassert_equal(ANT_SEARCH_EINVAL,
		      ant_search_window(&g_s, ant_radio_now(), NULL));
	zassert_equal(ANT_SEARCH_ENOENT,
		      ant_search_window(&g_s, ant_radio_now() + 1000u, &w));

	/* Nothing armed, so nothing owns any operation. */
	zassert_false(ant_search_owns_op(&g_s, 0u));
	zassert_false(ant_search_owns_op(&g_s, 1u));
 ...
 * 31 silently. */
ZTEST(ant_search, test_all_channel_slots_can_search)
{
	bring_up(false);

	for (uint8_t c = 0; c < (uint8_t)ANT_SEARCH_MAX_CHANNELS; c++) {
		zassert_ok(ant_search_begin(&g_s, c, ANT_SEARCH_MODE_ACQUIRE,
					    &want_any, ant_radio_now(),
					    ANT_SEARCH_TIMEOUT_NONE));
	}
	zassert_equal((uint8_t)ANT_SEARCH_MAX_CHANNELS, ant_search_n_searching(&g_s));
	...
	for (uint8_t c = 0; c < (uint8_t)ANT_SEARCH_MAX_CHANNELS; c++) {
		zassert_ok(ant_search_end(&g_s, c));
	}
	end_of_test();
}
