 ...
 * ant_core/include/ant_transfer.h, ant_core/include/ant_frame.h,
 * ant_core/tests/fake_radio.h and the measurements in
...
static struct ant_transfer xfer;
...
static uint8_t  rx_payload[ANT_TRANSFER_PKT_BYTES];
 ...
 * captured carried. */
static uint8_t bcast_payload[ANT_TRANSFER_PKT_BYTES] = {
	0x10u, 0xBDu, 0xFFu, 0x50u, 0xDEu, 0x11u, 0x64u, 0x00u
};
static bool bcast_available;

static void on_done(void *ctx, const struct ant_transfer_result *r)
{
	ARG_UNUSED(ctx);
...
static bool on_broadcast_payload(void *ctx, uint8_t out[ANT_TRANSFER_PKT_BYTES])
{
	ARG_UNUSED(ctx);
	...
	memcpy(out, bcast_payload, ANT_TRANSFER_PKT_BYTES);
	return true;
}

static const struct ant_transfer_ops ops = {
	.done = on_done,
	.rx_data = on_rx_data,
...
/* ---------------------------------------------------------------------------
 * Routing radio events, exactly as ant_channel.c will
 *
...
static struct ant_transfer *engine_b;

static void rx_cb(const struct ant_rx_event *e, void *user)
{
	ARG_UNUSED(user);
	ant_transfer_on_rx_event(&xfer, e);
	if (engine_b != NULL) {
		ant_transfer_on_rx_event(engine_b, e);
	}
}
...
{
	ARG_UNUSED(user);
	ant_transfer_on_tx_event(&xfer, e);
	if (engine_b != NULL) {
		ant_transfer_on_tx_event(engine_b, e);
	}
}

static const struct ant_radio_cbs radio_cbs = { .rx = rx_cb, .tx = tx_cb };
...
static struct ant_transfer_cfg cfg_slave;

static void make_cfg(struct ant_transfer_cfg *c, bool slot_opener)
{
	memset(c, 0, sizeof(*c));
	c->id.device_number = DEVNUM;
	c->id.device_type = DEVTYPE;
	c->id.trans_type = TRANSTYPE;
	c->net_addr[0] = ANT_NET_ADDR_ANT_PLUS_0;
	c->net_addr[1] = ANT_NET_ADDR_ANT_PLUS_1;
	c->rf_index = ANT_RF_INDEX_ANT_PLUS;
	c->power.dbm = 0;
...
static void before(void *f)
{
	...
	fake_radio_reset();
	zassert_equal(ANT_RADIO_OK_RC, ant_radio_init(&radio_cbs, NULL));
	...
	zassert_equal(ANT_TRANSFER_OK, ant_transfer_init(&xfer, &cfg_slave));
}
...
static const struct fake_radio_arm *last_arm_of(enum fake_radio_arm_kind kind)
{
	...
	while (n > 0u) {
		...
		if (a == NULL || a->kind != kind || a->rc != ANT_RADIO_OK_RC) {
			continue;
		}
		return a;
	}
...
static uint32_t collect_tx_bodies(uint8_t out[][ANT_RADIO_BODY_MAX], uint32_t cap)
{
	uint32_t n = fake_radio_arm_count();
	uint32_t found = 0u;
	uint32_t i;
...
/*
 * The peer's frame, hand-assembled rather than run through ant_frame_encode().
 *
...
{
	out[0] = ANT_NET_ADDR_ANT_PLUS_0;
	out[1] = ANT_NET_ADDR_ANT_PLUS_1;
	out[2] = DEVNUM_LO;
	out[3] = DEVNUM_HI;
	out[4] = DEVTYPE;
	out[5] = (uint8_t)TRANSTYPE;
	out[6] = ctrl;
	memcpy(&out[7], payload8, ANT_TRANSFER_PKT_BYTES);
	return AIR_LEN;
}

static const uint8_t peer_payload[ANT_TRANSFER_PKT_BYTES] = {
 ...
 * packet's t_sync + ANT_TRANSFER_REPLY_US.
 *
...
static void air_the_ack(void)
{
	...
	reply = ant_ctrl_reply_for(tx->body[BODY_CTRL]);
	...
	zassert_equal(ANT_RADIO_OK_RC,
		      fake_radio_air_frame(rx->t_open +
					   (ant_time_t)ANT_TRANSFER_ACK_GUARD_US,
					   frame, AIR_LEN));
}
...
static uint8_t  blocks[BLOCKS_MAX][ANT_TRANSFER_BLOCK_MAX];
...
static bool feed_next_block(void)
{
	...
	rc = ant_transfer_burst_block(&xfer, blocks[next_block],
				      block_len[next_block],
				      block_seg[next_block], ANT_TIME_NEVER);
...
static void run_one_packet_exchange(void)
{
	...
	zassert_equal(ANT_TRANSFER_STATE_WAIT_ACK, ant_transfer_state(&xfer),
...
static void drive(void)
{
	...
	for (guard = 0u; guard < 400u; guard++) {
		enum ant_transfer_state st = ant_transfer_state(&xfer);

		if (st == ANT_TRANSFER_STATE_IDLE) {
			return;
		}
		if (st == ANT_TRANSFER_STATE_WAIT_BLOCK) {
			if (!feed_next_block()) {
				...
				(void)ant_transfer_abort(&xfer);
				return;
			}
			continue;
		}
		if (st == ANT_TRANSFER_STATE_WAIT_ACK &&
		    ant_transfer_op(&xfer) != acked_op) {
			acked_op = ant_transfer_op(&xfer);
			if (acks_to_air > 0u) {
				acks_to_air--;
				air_the_ack();
			}
 ...
 * 0, last packet, opening the slot". There is no ANT_MSG_ACKNOWLEDGED in
 * ant_frame.h and its absence is the finding.
 */
ZTEST(transfer, test_acknowledged_data_is_a_one_packet_burst)
{
	...
	zassert_equal(ANT_TRANSFER_OK,
		      ant_transfer_ctrl(0u, true, true, &opener));
	zassert_equal(ANT_CTRL_ACK_DATA_OPEN, opener,
		      "a one-packet slot-opening transfer must be 0xAA, got 0x%02X",
		      opener);

	zassert_equal(ANT_TRANSFER_OK,
		      ant_transfer_ctrl(0u, true, false, &in_slot));
	zassert_equal(ANT_CTRL_BURST_LAST_SEQ0, in_slot,
	...
	zassert_equal((uint8_t)(opener ^ in_slot), ANT_CTRL_SLOT,
	...
	zassert_equal(ANT_MSG_BURST_LAST, ant_frame_msg_type(opener));
	zassert_equal(ANT_MSG_BURST_LAST, ant_frame_msg_type(in_slot));
	zassert_true(ant_ctrl_is_last(opener));
	zassert_equal(0u, ant_ctrl_seq(opener), "0xAA is sequence 0");
	zassert_true(ant_ctrl_is_slot_opener(opener));
	zassert_false(ant_ctrl_is_slot_opener(in_slot));
...
ZTEST(transfer, test_the_sequence_is_one_bit_and_alternates_per_packet)
{
	...
	for (i = 0u; i < 51u; i++) {
		...
		zassert_equal(ANT_TRANSFER_OK,
			      ant_transfer_ctrl(i, false, false, &mid));
		zassert_equal(ANT_TRANSFER_OK,
			      ant_transfer_ctrl(i, true, false, &last));

		if ((i & 1u) == 0u) {
			zassert_equal(ANT_CTRL_BURST_SEQ0, mid,
				      "packet %u: expected 0x82, got 0x%02X",
				      (unsigned)i, mid);
			zassert_equal(ANT_CTRL_BURST_LAST_SEQ0, last,
				      "packet %u last: expected 0xA2, got 0x%02X",
				      (unsigned)i, last);
		} else {
			zassert_equal(ANT_CTRL_BURST_SEQ1, mid,
				      "packet %u: expected 0x92, got 0x%02X",
				      (unsigned)i, mid);
			zassert_equal(ANT_CTRL_BURST_LAST_SEQ1, last,
				      "packet %u last: expected 0xB2, got 0x%02X",
				      (unsigned)i, last);
		}
		zassert_equal((uint8_t)(i & 1u), ant_ctrl_seq(mid));
	...
	/* Only packet 0 of a slot-opening transfer carries bit 3. */
	{
		...
		zassert_equal(ANT_TRANSFER_OK,
			      ant_transfer_ctrl(0u, false, true, &p0));
		zassert_equal(ANT_TRANSFER_OK,
			      ant_transfer_ctrl(1u, false, true, &p1));
		zassert_equal(ANT_CTRL_BURST_OPEN_SEQ0, p0,
			      "a master's first burst packet must be 0x8A");
		zassert_equal(ANT_CTRL_BURST_SEQ1, p1,
...
ZTEST(transfer, test_the_reply_relation_on_the_four_measured_pairs)
{
	...
	for (i = 0u; i < ARRAY_SIZE(pairs); i++) {
		...
		uint8_t a = ant_transfer_reply_ctrl(d);

		zassert_equal(pairs[i].reply, a,
			      "0x%02X should be acknowledged by 0x%02X, got 0x%02X",
			      d, pairs[i].reply, a);

		zassert_true(ant_ctrl_is_ack(a), "bit 6 must be set on a reply");
		zassert_false(ant_ctrl_is_ack(d), "bit 6 must be clear on data");
		zassert_equal(ant_ctrl_is_last(d), ant_ctrl_is_last(a),
			      "bit 5 is echoed, not changed");
		zassert_not_equal(ant_ctrl_seq(d), ant_ctrl_seq(a),
				  "bit 4 is COMPLEMENTED, not echoed: 0x%02X -> 0x%02X",
				  d, a);
		zassert_true(ant_transfer_ack_matches(d, a));
		zassert_true(ant_ctrl_observed(a),
	...
	for (i = 0u; i < ARRAY_SIZE(pairs); i++) {
		for (j = 0u; j < ARRAY_SIZE(pairs); j++) {
			...
			zassert_false(ant_transfer_ack_matches(pairs[i].data,
							       pairs[j].reply),
	 ...
	 * visible - see gap 2 in ant_transfer.h.
	 */
	zassert_equal(0u, ant_transfer_reply_ctrl(ANT_CTRL_BURST_OPEN_SEQ0));
	zassert_equal(0u, ant_transfer_reply_ctrl(ANT_CTRL_ACK_DATA_OPEN));
	zassert_equal(0u, ant_transfer_reply_ctrl(ANT_CTRL_BROADCAST));
...
{
	const struct ant_transfer_stats *st;
	uint8_t bodies[6][ANT_RADIO_BODY_MAX];
	uint32_t n_bodies;
	uint32_t i;

	reset_blocks();
	queue_block(0u, 8u, ANT_TRANSFER_SEG_START, 0x10u);
	queue_block(1u, 8u, ANT_TRANSFER_SEG_CONTINUE, 0x20u);
	queue_block(2u, 8u, ANT_TRANSFER_SEG_END, 0x30u);

	zassert_true(feed_next_block(), "the first block was refused");
	drive();

	zassert_equal(3u, n_ev, "expected 3 events, got %u", (unsigned)n_ev);
	zassert_equal(ANT_TRANSFER_EV_NEXT_BLOCK, evlog[0].ev);
	zassert_equal_ptr(blocks[0], evlog[0].block,
			  "block 0 was not the one released");
	zassert_equal(ANT_TRANSFER_EV_NEXT_BLOCK, evlog[1].ev);
	zassert_equal_ptr(blocks[1], evlog[1].block);
	zassert_equal(ANT_TRANSFER_EV_TX_COMPLETED, evlog[2].ev,
		      "the END block must be released by TX_COMPLETED (B4)");
	zassert_equal_ptr(blocks[2], evlog[2].block);
	zassert_equal(ANT_TRANSFER_FAIL_NONE, evlog[2].fail);
	zassert_equal(3u, evlog[2].packets);

	st = ant_transfer_stats(&xfer);
	...
	zassert_equal(3u, n_bodies, "expected 3 transmits, saw %u",
		      (unsigned)n_bodies);
	zassert_equal(ANT_CTRL_BURST_SEQ0, bodies[0][BODY_CTRL]);
	zassert_equal(ANT_CTRL_BURST_SEQ1, bodies[1][BODY_CTRL]);
	zassert_equal(ANT_CTRL_BURST_LAST_SEQ0, bodies[2][BODY_CTRL]);
	for (i = 0u; i < 3u; i++) {
		zassert_equal((uint8_t)TRANSTYPE, bodies[i][BODY_TTYPE]);
		zassert_mem_equal(&bodies[i][BODY_D0], blocks[i],
				  ANT_TRANSFER_PKT_BYTES,
...
{
	const struct ant_transfer_stats *st;
	uint8_t bodies[4][ANT_RADIO_BODY_MAX];
	uint32_t n_bodies;

	reset_blocks();
	queue_block(0u, 24u,
		    ANT_TRANSFER_SEG_START | ANT_TRANSFER_SEG_END, 0x40u);

	zassert_equal(3u, ant_transfer_packets_in_block(24u));
	zassert_equal(2u, ant_transfer_packets_in_block(16u));
	zassert_equal(1u, ant_transfer_packets_in_block(8u));
	zassert_equal(0u, ant_transfer_packets_in_block(12u),
	...
	zassert_equal(ANT_TRANSFER_EV_TX_COMPLETED, evlog[0].ev);
	zassert_equal_ptr(blocks[0], evlog[0].block);
...
{
	const struct ant_transfer_stats *st;
	uint8_t stray[ANT_TRANSFER_BLOCK_MAX];

	reset_blocks();
	memset(stray, 0xEEu, sizeof(stray));
	queue_block(0u, 8u, ANT_TRANSFER_SEG_START, 0x50u);

	/* A length no ANT stack fragments into eight-byte packets. */
	zassert_equal(ANT_TRANSFER_EINVAL,
		      ant_transfer_burst_block(&xfer, stray, 12u,
					       ANT_TRANSFER_SEG_START,
					       ANT_TIME_NEVER));
	/* A continuation with no transfer open. */
	zassert_equal(ANT_TRANSFER_ESEQ,
		      ant_transfer_burst_block(&xfer, stray, 8u,
					       ANT_TRANSFER_SEG_CONTINUE,
					       ANT_TIME_NEVER));
	/* An undefined segment bit. */
	zassert_equal(ANT_TRANSFER_EINVAL,
		      ant_transfer_burst_block(&xfer, stray, 8u, 0x04u,
					       ANT_TIME_NEVER));
	zassert_equal(0u, n_ev, "a rejected block raised %u events", (unsigned)n_ev);
	zassert_equal(0u, ant_transfer_stats(&xfer)->blocks_accepted);

	/* And mid-packet, which is the ANTW_TRANSFER_IN_PROGRESS the bridge
	 * already knows how to answer with. */
	zassert_true(feed_next_block());
	zassert_equal(ANT_TRANSFER_STATE_TX_DATA, ant_transfer_state(&xfer));
	zassert_equal(ANT_TRANSFER_EBUSY,
		      ant_transfer_burst_block(&xfer, stray, 8u,
					       ANT_TRANSFER_SEG_CONTINUE,
					       ANT_TIME_NEVER));
	...
	zassert_equal(ANT_TRANSFER_OK, ant_transfer_abort(&xfer));
...
ZTEST(transfer, test_a_missing_acknowledgement_fails_once_and_releases_the_block)
{
	...
	reset_blocks();
	queue_block(0u, 8u, ANT_TRANSFER_SEG_START, 0x60u);
	queue_block(1u, 8u, ANT_TRANSFER_SEG_END, 0x70u);
	...
	zassert_equal(ANT_TRANSFER_EV_NEXT_BLOCK, evlog[0].ev);
	zassert_equal_ptr(blocks[0], evlog[0].block);
	zassert_equal(ANT_TRANSFER_EV_TX_FAILED, evlog[1].ev);
	zassert_equal(ANT_TRANSFER_FAIL_NO_ACK, evlog[1].fail);
	zassert_equal_ptr(blocks[1], evlog[1].block,
...
ZTEST(transfer, test_a_burst_aborted_midflight_and_its_late_terminal_event)
{
	...
	reset_blocks();
	queue_block(0u, 16u, ANT_TRANSFER_SEG_START, 0x80u);
	queue_block(1u, 8u, ANT_TRANSFER_SEG_END, 0x90u);
	...
	dead_op = ant_transfer_op(&xfer);
	zassert_not_equal(0u, dead_op);

	zassert_equal(ANT_TRANSFER_OK, ant_transfer_abort(&xfer));

	zassert_equal(ANT_TRANSFER_STATE_IDLE, ant_transfer_state(&xfer),
		      "the abort's terminal event should already have arrived");
	zassert_equal(1u, n_ev, "abort must raise exactly one event, got %u",
		      (unsigned)n_ev);
	zassert_equal(ANT_TRANSFER_EV_TX_FAILED, evlog[0].ev);
	zassert_equal(ANT_TRANSFER_FAIL_ABORTED, evlog[0].fail);
	zassert_equal_ptr(blocks[0], evlog[0].block,
	 ...
	 */
	zassert_equal(ANT_RADIO_OK_RC,
		      fake_radio_inject_late_tx(dead_op, ANT_RADIO_STATUS_ABORTED));
	{
		...
		f.len = build_peer_frame(frame, ANT_CTRL_ACK_SEQ1, peer_payload);
	...
	zassert_true(ant_transfer_stats(&xfer)->late_events >= 2u,
		     "late events must be counted, not silently dropped");
	zassert_equal(1u, ant_transfer_stats(&xfer)->blocks_released);
...
ZTEST(transfer, test_the_receive_path_meets_the_turnaround_deadline)
{
	...
	zassert_equal(ANT_TRANSFER_OK,
		      ant_transfer_ctrl_fields(0u, false, false, &fields));
	zassert_equal(ANT_FRAME_OK,
		      ant_frame_make(&in, &cfg_slave.id, &fields, peer_payload,
				     ANT_TRANSFER_PKT_BYTES));
	zassert_equal(ANT_CTRL_BURST_SEQ0, in.ctrl_byte);

	t_sync = ant_radio_now() + 10000u;

	zassert_equal(ANT_TRANSFER_OK, ant_transfer_on_data(&xfer, &in, t_sync));
	zassert_equal(ANT_TRANSFER_STATE_TX_REPLY, ant_transfer_state(&xfer));
	...
	zassert_equal((uint64_t)ANT_TRANSFER_REPLY_US, turnaround,
	...
	zassert_equal(ANT_CTRL_ACK_SEQ1, arm->body[BODY_CTRL],
	...
	zassert_equal(ANT_TRANSFER_PKT_BYTES, rx_len);
	zassert_false(rx_last, "0x82 is not the last packet of a transfer");
	zassert_mem_equal(rx_payload, peer_payload, ANT_TRANSFER_PKT_BYTES);

	fake_radio_advance_to(arm->t_sync_at + 1u);
	zassert_equal(ANT_TRANSFER_STATE_IDLE, ant_transfer_state(&xfer));
	zassert_equal(1u, ant_transfer_stats(&xfer)->acks_sent);
...
ZTEST(transfer, test_received_acknowledged_data_is_answered_with_transfer_ack)
{
	...
	zassert_equal(ANT_TRANSFER_OK,
		      ant_transfer_ctrl_fields(0u, true, false, &fields));
	zassert_equal(ANT_FRAME_OK,
		      ant_frame_make(&in, &cfg_slave.id, &fields, peer_payload,
				     ANT_TRANSFER_PKT_BYTES));
	zassert_equal(ANT_CTRL_BURST_LAST_SEQ0, in.ctrl_byte);

	zassert_equal(ANT_TRANSFER_OK,
		      ant_transfer_on_data(&xfer, &in, ant_radio_now() + 10000u));

	arm = last_arm_of(FAKE_RADIO_ARM_TX);
	zassert_not_null(arm);
	zassert_equal(ANT_CTRL_ACK_LAST_SEQ0, arm->body[BODY_CTRL],
 ...
 *
 * ant_ctrl_reply_for() returns 0 for 0x8A and 0xAA and this module refuses
...
ZTEST(transfer, test_a_slot_opening_packet_has_no_measured_acknowledgement)
{
	...
	zassert_equal(ANT_TRANSFER_OK,
		      ant_transfer_ctrl_fields(0u, false, true, &fields));
	zassert_equal(ANT_FRAME_OK,
		      ant_frame_make(&in, &cfg_slave.id, &fields, peer_payload,
				     ANT_TRANSFER_PKT_BYTES));
	zassert_equal(ANT_CTRL_BURST_OPEN_SEQ0, in.ctrl_byte);

	zassert_equal(ANT_TRANSFER_ENOTSUP,
		      ant_transfer_on_data(&xfer, &in, ant_radio_now() + 10000u));
	zassert_equal(1u, ant_transfer_stats(&xfer)->unackable_openers);
	zassert_equal(ANT_TRANSFER_STATE_IDLE, ant_transfer_state(&xfer));
	...
	zassert_equal(ANT_TRANSFER_OK,
		      ant_transfer_ctrl_fields(0u, true, true, &fields));
	zassert_equal(ANT_FRAME_OK,
		      ant_frame_make(&in, &cfg_slave.id, &fields, peer_payload,
				     ANT_TRANSFER_PKT_BYTES));
	zassert_equal(ANT_CTRL_ACK_DATA_OPEN, in.ctrl_byte);
	zassert_equal(ANT_TRANSFER_ENOTSUP,
		      ant_transfer_on_data(&xfer, &in, ant_radio_now() + 10000u));
	zassert_equal(2u, ant_transfer_stats(&xfer)->unackable_openers);
...
ZTEST(transfer, test_a_channel_with_no_broadcast_buffer_is_not_answered)
{
	...
	zassert_equal(ANT_TRANSFER_ESTATE,
		      ant_transfer_on_data(&xfer, &in, ant_radio_now() + 10000u));
	zassert_equal(1u, ant_transfer_stats(&xfer)->no_broadcast);
	zassert_equal(0u, fake_radio_stats()->arms_tx);
...
ZTEST(transfer, test_the_acknowledgement_retransmission_limit)
{
	...
	for (i = 1u; i < ANT_TRANSFER_REPLY_ATTEMPTS_MAX; i++) {
		ant_time_t at = ant_radio_now() +
				(ant_time_t)ANT_TRANSFER_REPLY_RETRY_US;

		zassert_equal(ANT_TRANSFER_OK,
			      ant_transfer_reply_retransmit(&xfer, at),
	...
	zassert_equal(ANT_TRANSFER_ESTATE,
		      ant_transfer_reply_retransmit(&xfer,
						    ant_radio_now() + 5000u),
		      "the 22nd attempt must be refused; 21 is what was measured");
	zassert_equal(ANT_TRANSFER_REPLY_ATTEMPTS_MAX - 1u,
		      ant_transfer_stats(&xfer)->ack_retransmits);
...
{
	const struct fake_radio_arm *arm;
	ant_time_t master_t_sync = ant_radio_now() + 100000u;
	uint8_t payload[ANT_TRANSFER_PKT_BYTES];

	memset(payload, 0xA5u, sizeof(payload));

	zassert_equal(ANT_TRANSFER_OK,
		      ant_transfer_ack_data(&xfer, payload,
					    ANT_TRANSFER_PKT_BYTES,
					    master_t_sync +
					    (ant_time_t)ANT_TRANSFER_SLOT_REPLY_US));

	arm = last_arm_of(FAKE_RADIO_ARM_TX);
	zassert_not_null(arm);
	zassert_equal(master_t_sync + (ant_time_t)ANT_TRANSFER_SLOT_REPLY_US,
		      arm->t_sync_at);
	/* In-slot acknowledged data is 0xA2 - one packet, sequence 0, last. */
	zassert_equal(ANT_CTRL_BURST_LAST_SEQ0, arm->body[BODY_CTRL]);
	zassert_mem_equal(&arm->body[BODY_D0], payload, ANT_TRANSFER_PKT_BYTES);
	...
	zassert_equal(0u, ant_transfer_stats(&xfer)->blocks_released);
 ...
 * resistance gets set - so this has to be a loud ANT_TRANSFER_ENOTSUP here
...
ZTEST(transfer, test_a_backend_that_cannot_turn_a_frame_round_is_refused)
{
	...
	zassert_equal(ANT_TRANSFER_OK, ant_transfer_init(&slow, &cfg));

	fake_radio_caps_mut()->min_arm_lead_us =
		(uint16_t)(ANT_TRANSFER_REPLY_US - ANT_TRANSFER_ACK_GUARD_US + 1u);
	zassert_equal(ANT_TRANSFER_ENOTSUP, ant_transfer_init(&slow, &cfg));

	fake_radio_caps_preset_nrf();
	fake_radio_caps_mut()->rx_to_tx_us = (uint16_t)(ANT_TRANSFER_REPLY_US + 1u);
	zassert_equal(ANT_TRANSFER_ENOTSUP, ant_transfer_init(&slow, &cfg));
...
{
	struct ant_transfer master;
	struct ant_transfer_cfg cfg;
	const struct fake_radio_arm *arm;
	uint8_t payload[ANT_TRANSFER_PKT_BYTES];

	make_cfg(&cfg, true);
	zassert_equal(ANT_TRANSFER_OK, ant_transfer_init(&master, &cfg));
	engine_b = &master;

	memset(payload, 0x5Au, sizeof(payload));
	zassert_equal(ANT_TRANSFER_OK,
		      ant_transfer_ack_data(&master, payload,
					    ANT_TRANSFER_PKT_BYTES,
					    ANT_TIME_NEVER));

	arm = last_arm_of(FAKE_RADIO_ARM_TX);
	zassert_not_null(arm);
	zassert_equal(ANT_CTRL_ACK_DATA_OPEN, arm->body[BODY_CTRL],
		      "a master's acknowledged data is 0xAA, not 0xA2");

	zassert_equal(ANT_TRANSFER_OK, ant_transfer_abort(&master));
	zassert_true(ant_transfer_is_idle(&master));
	/* The other engine saw the same event and correctly disowned it. */
	zassert_true(ant_transfer_stats(&xfer)->late_events >= 1u);
	zassert_equal(0u, ant_transfer_stats(&xfer)->blocks_released);
