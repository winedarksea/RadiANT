 ...
 * ant_core/include/ant_sched.h, ant_core/include/ant_radio_hal.h,
 * ant_core/tests/fake_radio.h and the measurements in docs/spike-a-results.md
 ...
 * ---------------------------------------------------------------------------
 * ant_sched.c is pure logic over ant_radio_hal.h, so everything about it is
 ...
 * check on "a pass is one ant_radio_now() and one arm call").
 */
...
static struct ant_rx_filter track_filter[ANT_SCHED_MAX_CHANNELS];
static struct ant_rx_filter search_filter[ANT_SCHED_MAX_FILTERS];

static const uint8_t test_payload[8] = {
	0x10u, 0x00u, 0xFFu, 0xFFu, 0x00u, 0x00u, 0x00u, 0x00u
};

/* Ten bytes, which is exactly ant_frame_format(TRACKING)->body_len. The mock
...
static const uint8_t test_tx_body[10] = {
	TEST_TRANS_TYPE, ANT_CTRL_BROADCAST,
	0x10u, 0x00u, 0xFFu, 0xFFu, 0x00u, 0x00u, 0x00u, 0x00u
};
...
static struct {
	...
	struct {
		uint8_t             ch;
		enum ant_sched_done why;
	} done[TEST_LOG_MAX];

	uint32_t ok_count[ANT_SCHED_MAX_CHANNELS];
	uint32_t missed_count[ANT_SCHED_MAX_CHANNELS];
	uint32_t aborted_count[ANT_SCHED_MAX_CHANNELS];

	/* Steady-state driver: re-post the next window from done(), which is
	 * what ant_channel.c will do and the only way the round-robin fairness
	 * of the scheduler is observable at all. */
	bool       repost;
	ant_time_t next_open[ANT_SCHED_MAX_CHANNELS];
...
static void rx_req_init(struct ant_sched_rx *r, const struct ant_pkt_format *fmt,
			const struct ant_rx_filter *filters, uint8_t n,
			ant_time_t t_open, ant_time_t t_close)
{
	memset(r, 0, sizeof(*r));
	r->fmt = fmt;
	r->rf_index = ANT_RF_INDEX_ANT_PLUS;
	r->filters = filters;
	r->n_filters = n;
	r->t_open = t_open;
	r->t_close = t_close;
...
static void post_track(uint8_t ch, ant_time_t t_open, ant_time_t t_close)
{
	struct ant_sched_rx r;

	rx_req_init(&r, ant_frame_format(ANT_FRAME_CFG_TRACKING),
		    &track_filter[ch], 1u, t_open, t_close);
	zassert_ok(ant_sched_request_rx(ch, &r), "channel %u refused", ch);
}
		  ...
		  const struct ant_rx_event *evt, void *user)
{
	...
	if (log.reply_after_us != 0u && evt->status == ANT_RADIO_STATUS_OK) {
		struct ant_sched_tx t;

		memset(&t, 0, sizeof(t));
		t.fmt = ant_frame_format(ANT_FRAME_CFG_TRACKING);
		t.rf_index = ANT_RF_INDEX_ANT_PLUS;
		t.power.dbm = 0;
		t.body = test_tx_body;
		t.body_len = (uint8_t)sizeof(test_tx_body);
		t.t_sync_at = evt->t_sync + (ant_time_t)log.reply_after_us;
		(void)ant_sched_request_tx(log.reply_ch, &t);
		log.reply_after_us = 0u; /* one reply per test */
	}
}

static void on_tx(uint8_t ch, const struct ant_tx_event *evt, void *user)
{
	ARG_UNUSED(user);
...
static void on_done(uint8_t ch, enum ant_sched_done why, void *user)
{
	ARG_UNUSED(user);
	...
	if (log.repost) {
		log.next_open[ch] += (ant_time_t)TEST_PERIOD_US;
		post_track(ch, log.next_open[ch],
			   log.next_open[ch] + (ant_time_t)TEST_WINDOW_US);
	}
}

static const struct ant_sched_cbs test_cbs = {
	.rx = on_rx,
	.tx = on_tx,
	.done = on_done,
};
...
static void build_filters(void)
{
	...
	for (i = 0u; i < ANT_SCHED_MAX_CHANNELS; i++) {
		...
		zassert_equal(5, ant_frame_addr(ANT_FRAME_CFG_TRACKING,
						ant_net_addr_ant_plus, &id,
						track_filter[i].addr,
	...
	for (i = 0u; i < ANT_SCHED_MAX_FILTERS; i++) {
		struct ant_channel_id id = {
			.device_number = (uint16_t)(TEST_DEVNUM_BASE + i),
			.device_type = TEST_DEV_TYPE,
			.trans_type = TEST_TRANS_TYPE,
		...
		zassert_equal(3, ant_frame_addr(ANT_FRAME_CFG_SEARCH,
						ant_net_addr_ant_plus, &id,
 ...
 * byte stream. */
static void air_frame_for(uint8_t ch, ant_time_t t_sync)
{
...
{
	zassert_ok(ant_sched_init(&test_cbs, NULL));
	zassert_ok(ant_radio_init(ant_sched_radio_cbs(), NULL));
 ...
 * for more filters than caps.max_filters is exactly ANT_RADIO_EINVAL.
 */
static void finish(void)
{
	...
	for (ch = 0u; ch < (uint8_t)ANT_SCHED_MAX_CHANNELS; ch++) {
		zassert_ok(ant_sched_cancel(ch));
	}
...
/* Every accepted window must be finite and well ordered. ANT_TIME_NEVER is
...
static void assert_every_window_bounded(void)
{
	...
	for (i = 0u; i < fake_radio_arm_count(); i++) {
		...
		zassert_not_equal(ANT_TIME_NEVER, a->t_close,
...
ZTEST_SUITE(ant_sched, NULL, NULL, sched_before, NULL, NULL);
 ...
 */
ZTEST(ant_sched, test_a_window_is_armed_at_the_requested_absolute_microsecond)
{
	...
	zassert_ok(ant_sched_tick());
	zassert_equal(1u, fake_radio_arm_count());

	a = fake_radio_arm(0u);
	zassert_not_null(a);
	zassert_equal(FAKE_RADIO_ARM_RX, a->kind);
	zassert_equal(ANT_RADIO_OK_RC, a->rc);
	zassert_true(a->t_open > (ant_time_t)0xFFFFFFFFu,
	...
	zassert_equal(ANT_RF_INDEX_ANT_PLUS, a->rf_index);
	...
	zassert_equal(ANT_SCHED_DONE_OK, log.done[0].why);
 ...
 */
ZTEST(ant_sched, test_overlapping_windows_collapse_into_one_hardware_window)
{
	const struct fake_radio_arm  *a;
	const struct ant_sched_stats *st;
	ant_time_t                    t0;
	ant_time_t                    open;
	ant_time_t                    close;
	uint8_t                       ch;
	 ...
	 * it here either. */
	zassert_equal(0u, a->flags & ANT_RX_STOP_ON_FIRST);
	for (ch = 0u; ch < 3u; ch++) {
	...
	zassert_equal(ANT_RADIO_STATUS_OK, log.rx[0].status);
 ...
 * coverage.
 */
ZTEST(ant_sched, test_disjoint_windows_are_not_merged)
{
	ant_time_t t0;
	ant_time_t open;
	...
	zassert_equal(0u, ant_sched_stats_get()->windows_merged);
 ...
 *
 * The four that do not are reported ANT_SCHED_DONE_MISSED rather than dropped,
 ...
 */
ZTEST(ant_sched, test_merging_is_bounded_by_max_filters_at_eight)
{
	ant_time_t t0;
	ant_time_t open;
	uint8_t    ch;
	...
	zassert_equal(8u, ant_radio_caps_get()->max_filters);
	...
	zassert_equal(4u, ant_sched_stats_get()->missed);
 ...
 * ANT_RADIO_EINVAL, so a module that ignored the capability would fail here
 * even if this test's own assertion were deleted.
 */
ZTEST(ant_sched, test_merging_is_bounded_by_max_filters_at_two)
{
	ant_time_t t0;
	ant_time_t open;
	uint8_t    ch;
	...
	bring_up();
	zassert_equal(2u, ant_radio_caps_get()->max_filters);
	...
	zassert_equal(2u, fake_radio_arm(0u)->n_filters,
		      ...
		      fake_radio_arm(0u)->n_filters);
	zassert_equal(ANT_RADIO_OK_RC, fake_radio_arm(0u)->rc);
 ...
 *
 * ant_search.c's sweep asks for caps.max_filters addresses at a time, so on nRF
 ...
 */
ZTEST(ant_sched, test_a_request_for_more_filters_than_exist_is_reported_once)
{
	struct ant_sched_rx r;
	ant_time_t          t0;
	...
	t0 = ant_radio_now();
	rx_req_init(&r, ant_frame_format(ANT_FRAME_CFG_SEARCH), search_filter,
		    8u, t0 + 100000u, t0 + 100000u + 250000u);
	zassert_ok(ant_sched_request_rx(4u, &r));
	zassert_ok(ant_sched_tick());

	zassert_equal(0u, fake_radio_arm_count(),
		      "the scheduler offered the backend a window it must refuse");
	zassert_equal(1u, log.n_done);
	zassert_equal(ANT_SCHED_DONE_FAILED, log.done[0].why);
	zassert_equal(4u, log.done[0].ch);
	zassert_false(ant_sched_pending(4u), "the failed request was not consumed");
 ...
 * of c's own array".
 *
 * This is what ant_search.c's recovery of devnum_lo rests on: with a 3-byte
 ...
 */
ZTEST(ant_sched, test_filter_index_is_translated_to_the_channels_own_filter)
{
	struct ant_sched_rx r;
	ant_time_t          t0;
	ant_time_t          open;
	...
	rx_req_init(&r, ant_frame_format(ANT_FRAME_CFG_SEARCH), &search_filter[0],
		    4u, open, close);
	zassert_ok(ant_sched_request_rx(0u, &r));
	rx_req_init(&r, ant_frame_format(ANT_FRAME_CFG_SEARCH), &search_filter[4],
		    2u, open, close);
	zassert_ok(ant_sched_request_rx(1u, &r));
	zassert_ok(ant_sched_tick());
 ...
 */
ZTEST(ant_sched, test_a_late_channel_joins_a_window_that_has_not_opened_yet)
{
	ant_time_t t0;
	ant_time_t open;
	...
	zassert_equal(1u, ant_sched_stats_get()->replans);
	zassert_equal(0u, ant_sched_stats_get()->preempted);
 ...
 * which is what ant_channel.c will do, and it is the only way the rotation is
 * observable.
 */
ZTEST(ant_sched, test_thirty_two_contending_channels_are_all_served)
{
	ant_time_t base;
	uint32_t   cycles = 19u;
	uint8_t    ch;
	...
	fake_radio_advance_to(base + (ant_time_t)cycles * TEST_PERIOD_US + 1000u);
	...
	zassert_equal(2u * ant_sched_stats_get()->windows_armed,
		      ant_sched_stats_get()->channels_windowed,
		      "some window carried other than two channels");
	zassert_equal(ant_sched_stats_get()->windows_armed,
		      ant_sched_stats_get()->windows_merged,
 ...
 * nothing else does.
 */
ZTEST(ant_sched, test_thirty_two_channels_merge_eight_at_a_time_on_nrf)
{
	ant_time_t base;
	uint8_t    ch;
	...
	fake_radio_advance_to(base + 6u * (ant_time_t)TEST_PERIOD_US + 1000u);

	for (ch = 0u; ch < (uint8_t)ANT_SCHED_MAX_CHANNELS; ch++) {
		zassert_true(log.ok_count[ch] >= 1u, "channel %u was never armed",
			     ch);
	}
	zassert_equal(8u * ant_sched_stats_get()->windows_armed,
		      ant_sched_stats_get()->channels_windowed,
 ...
 * backend never silently runs late - it returns ANT_RADIO_ETIME. So the correct
 ...
 */
ZTEST(ant_sched, test_an_unreachable_deadline_is_refused_rather_than_run_late)
{
	struct ant_sched_tx t;
	ant_time_t          t0;
	uint16_t            lead;
	...
	zassert_false(ant_sched_pending(0u));
	...
	t.t_sync_at = t0 + (ant_time_t)lead - 1u;
	zassert_ok(ant_sched_request_tx(1u, &t));
	zassert_ok(ant_sched_tick());
	zassert_equal(0u, fake_radio_arm_count(),
	...
	t.t_sync_at = t0 + (ant_time_t)lead;
	zassert_ok(ant_sched_request_tx(2u, &t));
	zassert_ok(ant_sched_tick());
	zassert_equal(1u, fake_radio_arm_count());
 ...
 */
ZTEST(ant_sched, test_a_window_is_truncated_to_clear_a_transmit_deadline)
{
	struct ant_sched_tx t;
	ant_time_t          t0;
	ant_time_t          open;
	...
	zassert_equal(tx_at - (ant_time_t)lead, fake_radio_arm(0u)->t_close,
 ...
 * aborted window's ANT_RADIO_STATUS_ABORTED arrives after the transmit has been
 ...
 */
ZTEST(ant_sched, test_a_reply_preempts_the_window_it_was_heard_in)
{
	ant_time_t t0;
	ant_time_t open;
	ant_time_t heard_at;
	...
	zassert_equal(2u, fake_radio_arm_count());
	zassert_equal(FAKE_RADIO_ARM_TX, fake_radio_arm(1u)->kind);
	zassert_equal(ANT_RADIO_OK_RC, fake_radio_arm(1u)->rc);
	...
	zassert_equal(1u, ant_sched_stats_get()->stale_events,
		      "the aborted window's late terminal event was not seen as stale");
	zassert_equal(1u, ant_sched_stats_get()->tx_armed);
 ...
 */
ZTEST(ant_sched, test_a_transmit_deadline_displaces_an_armed_window)
{
	struct ant_sched_tx t;
	ant_time_t          t0;
	ant_time_t          open;
	...
	zassert_equal(2u, fake_radio_arm_count());
	zassert_equal(FAKE_RADIO_ARM_TX, fake_radio_arm(1u)->kind);
	zassert_equal(1u, ant_sched_stats_get()->preempted,
		      "an open window was cut short but not counted as a preemption");
	zassert_equal(0u, ant_sched_stats_get()->replans,
 ...
 */
ZTEST(ant_sched, test_a_dead_operations_late_event_changes_nothing)
{
	...
	zassert_ok(fake_radio_inject_late_tx(dead_op, ANT_RADIO_STATUS_OK));
	...
	zassert_equal(2u, ant_sched_stats_get()->stale_events);
...
/*
 * ANT_TIME_NEVER is not a valid window edge, so "receive until I say stop" has
 * to be a chain of bounded windows that something re-arms. That something is
 * this module, and the translation lives nowhere else in ant_core.
 *
 ...
 */
ZTEST(ant_sched, test_a_continuous_request_becomes_a_chain_of_bounded_windows)
{
	struct ant_sched_rx r;
	ant_time_t          t0;
	ant_time_t          tracked_at;
	uint16_t            lead;
	uint32_t            i;
	...
	rx_req_init(&r, ant_frame_format(ANT_FRAME_CFG_SEARCH), search_filter,
		    8u, t0, ANT_TIME_NEVER);
	r.chunk_us = 50000u;
	zassert_ok(ant_sched_request_rx(31u, &r));
	zassert_ok(ant_sched_tick());

	zassert_equal(1u, fake_radio_arm_count());
	zassert_equal(t0 + (ant_time_t)lead, fake_radio_arm(0u)->t_open,
		      "a scan asking for 'now' must be armed exactly one arming "
		      "lead from now");
	zassert_equal(t0 + (ant_time_t)lead + 50000u, fake_radio_arm(0u)->t_close);
	zassert_equal(8u, fake_radio_arm(0u)->n_filters);
	zassert_equal(0u, fake_radio_arm(0u)->flags & ANT_RX_STOP_ON_FIRST,
	...
	zassert_equal(fake_radio_arm_count(),
		      ant_sched_stats_get()->scan_chunks);
	zassert_true(ant_sched_pending(31u), "the continuous request was consumed");
	for (i = 0u; i < fake_radio_arm_count(); i++) {
		zassert_equal(ANT_RADIO_OK_RC, fake_radio_arm(i)->rc);
	}

	/* Now a tracked channel wants the radio. The scan must not be cut short
	 * for a window that is still a long way off... */
	tracked_at = ant_radio_now() + 200000u;
	...
	zassert_true(ant_sched_stats_get()->scan_chunks > chunks_before,
 ...
 */
ZTEST(ant_sched, test_cancelling_a_scan_leaves_the_radio_idle)
{
	struct ant_sched_rx r;
	ant_time_t          t0;
	...
	rx_req_init(&r, ant_frame_format(ANT_FRAME_CFG_SEARCH), search_filter,
		    4u, t0, ANT_TIME_NEVER);
	zassert_ok(ant_sched_request_rx(9u, &r));
	zassert_ok(ant_sched_tick());
	zassert_equal(1u, fake_radio_arm_count());
	zassert_false(fake_radio_is_idle());

	zassert_ok(ant_sched_cancel(9u));
	zassert_true(fake_radio_is_idle(), "%s", fake_radio_busy_reason());
	zassert_false(ant_sched_pending(9u));
...
/*
 * ANT_RX_STOP_ON_FIRST survives an unmerged window and is dropped from a merged
 * one.
 *
 ...
 * ANT_RADIO_EINVAL, which finish() would also catch. The interesting half is
 ...
 */
ZTEST(ant_sched, test_stop_on_first_survives_only_an_unmerged_window)
{
	struct ant_sched_rx r;
	ant_time_t          t0;
	ant_time_t          open;
	...
	rx_req_init(&r, ant_frame_format(ANT_FRAME_CFG_TRACKING), &track_filter[0],
		    1u, open, open + 50000u);
	...
	zassert_equal(1u, fake_radio_arm_count());
	zassert_true((fake_radio_arm(0u)->flags & ANT_RX_STOP_ON_FIRST) != 0u,
	...
	open = ant_radio_now() + 100000u;
	rx_req_init(&r, ant_frame_format(ANT_FRAME_CFG_TRACKING), &track_filter[0],
		    1u, open, open + TEST_WINDOW_US);
	r.stop_on_first = true;
	zassert_ok(ant_sched_request_rx(0u, &r));
	rx_req_init(&r, ant_frame_format(ANT_FRAME_CFG_TRACKING), &track_filter[1],
		    1u, open, open + TEST_WINDOW_US);
	r.stop_on_first = true;
	zassert_ok(ant_sched_request_rx(1u, &r));
	zassert_ok(ant_sched_tick());
	...
	zassert_equal(2u, fake_radio_arm_count());
	zassert_equal(2u, fake_radio_arm(1u)->n_filters);
	zassert_equal(0u, fake_radio_arm(1u)->flags & ANT_RX_STOP_ON_FIRST,
...
/*
 * ANT_RADIO_EBUSY leaves the request pending.
 *
 ...
 */
ZTEST(ant_sched, test_ebusy_leaves_the_request_for_the_next_commit)
{
	ant_time_t t0;
	ant_time_t open;
	...
	zassert_equal(1u, ant_sched_stats_get()->arm_ebusy);
	zassert_true(ant_sched_pending(0u), "the request was thrown away on EBUSY");
 ...
 * rather than leaving it in flight for ever. ANT_RADIO_STATUS_FAILED is the one
 ...
 */
ZTEST(ant_sched, test_a_failed_operation_ends_the_request)
{
	ant_time_t t0;
	ant_time_t open;
 ...
 * ant_search.c able to rank and gate on CRC, and it is why the flag is off by
 * default.
 */
ZTEST(ant_sched, test_crc_failures_are_delivered_only_when_asked_for)
{
	struct ant_sched_rx r;
	ant_time_t          t0;
	ant_time_t          open;
	...
	zassert_equal(6u, log.n_rx, "the channel asked for CRC failures and got %u",
		      log.n_rx);
	zassert_equal(ANT_RADIO_STATUS_CRC_FAIL, log.rx[0].status);
	zassert_equal(0u, log.rx[0].ch);
...
ZTEST(ant_sched, test_malformed_requests_are_rejected_at_the_door)
{
	struct ant_sched_rx r;
	struct ant_sched_tx t;
	ant_time_t          t0;
	...
	zassert_equal(ANT_RADIO_EINVAL,
		      ant_sched_request_rx((uint8_t)ANT_SCHED_MAX_CHANNELS, &r),
		      "a channel past the ceiling was accepted");
	zassert_equal(ANT_RADIO_EINVAL, ant_sched_request_rx(0u, NULL));

	r.fmt = NULL;
	zassert_equal(ANT_RADIO_EINVAL, ant_sched_request_rx(0u, &r));
	...
	rx_req_init(&r, ant_frame_format(ANT_FRAME_CFG_TRACKING), &track_filter[0],
		    (uint8_t)(ANT_SCHED_MAX_FILTERS + 1u), t0 + 1000u, t0 + 2000u);
	zassert_equal(ANT_RADIO_EINVAL, ant_sched_request_rx(0u, &r));
	...
	zassert_equal(ANT_RADIO_EINVAL, ant_sched_request_tx(0u, &t),
		      "a transmit with no body was accepted");
	t.body = test_tx_body;
	t.body_len = (uint8_t)sizeof(test_tx_body);
	t.t_sync_at = ANT_TIME_NEVER;
	zassert_equal(ANT_RADIO_EINVAL, ant_sched_request_tx(0u, &t),
		      "ANT_TIME_NEVER was accepted as a transmit instant");
