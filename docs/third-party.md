...
| NCS Matter bridge core (`apps/dongle_thread/src/matter/core/`) | `LicenseRef-Nordic-5-Clause` | Yes — vendored verbatim | Yes, freely |
| NCS Matter bridge ZAP output (`.../matter/default_zap/zap-generated/`) | Apache-2.0 (long form, no SPDX line) | Yes — vendored verbatim | Yes, freely |
| NCS Matter bridge ZAP source (`bridge.zap`, `bridge.matter`) | Unmarked upstream — see note below | Yes — vendored verbatim | Yes, freely |
...
## NCS Matter bridge core, vendored (`apps/dongle_thread/src/matter/`)

**What it is.** The bridge-manager, bridged-device and storage-manager
sources and the committed ZAP data model from
`nrf/applications/matter_bridge/src/`, copied into this repository so
RadiANT's Matter bridge (package E3 of the Matter plan) has an Aggregator
endpoint and dynamic-endpoint machinery to drive. Copied from **NCS v3.4.0**,
toolchain bundle `dcbdc366a1`, on 2026-08-15. The provenance table, the
verbatim copy list and everything deliberately left behind — the
BLE-central provider (`src/ble/`), the simulated providers, all five bridged
device types, `app_task.cpp`/`main.cpp`/`bridge_shell.cpp`/
`zcl_callbacks.cpp`, and the `onoff_plug_zap/` alternative data model — are
recorded in `apps/dongle_thread/src/matter/README.md` and
`docs/matter-e3-vendoring.md`; this entry only tracks the licence
obligation.

**Licence — two, by file group, and one group unmarked.**

- `core/bridge_manager.{h,cpp}`, `core/matter_bridged_device.{h,cpp}`,
  `core/bridged_device_data_provider.{h,cpp}`,
  `core/bridge_storage_manager.{h,cpp}`, `core/util/bridge_util.h` — nine
  files, each headed `Copyright (c) 2023 Nordic Semiconductor ASA` /
  `SPDX-License-Identifier: LicenseRef-Nordic-5-Clause`. The full licence
  text is the `LicenseRef-Nordic-5-Clause` entry in
  `C:\ncs\v3.4.0\nrf\LICENSE` — the same 5-clause BSD-style text already
  summarised in the nRF Connect SDK entry above, with clause 4 restricting
  use to Nordic silicon and clause 5 forbidding reverse engineering of
  anything shipped in binary form. It is not reproduced a second time here,
  matching how this document treats the GPL and BSD texts elsewhere: the
  vendored files carry it verbatim in their own headers, which is where it
  ships with the source.
- `default_zap/zap-generated/access.h`, `CodeDrivenCallback.h`,
  `endpoint_config.h`, `gen_config.h`, `PluginApplicationCallbacks.h`,
  `callback-stub.cpp`, `CodeDrivenInitShutdown.cpp`,
  `IMClusterCommandHandler.cpp` — eight files, each headed
  `Copyright (c) 2022 Project CHIP Authors` and the full Apache-2.0 licence
  text in long form. This is Project CHIP's code, generated by the ZAP tool
  and committed because the build runs with `BYPASS_IDL` (see the README):
  this directory is the data model itself, not a regenerable cache of one.
- `default_zap/bridge.zap` and `default_zap/bridge.matter` carry **no
  licence header at all, upstream or here** — `bridge.zap` is ZAP's JSON
  save file (JSON has no comment syntax to carry one) and `bridge.matter`
  opens with only `// This IDL was generated automatically by ZAP.` /
  `// It is for view/code review purposes only.`. Both sit in the same
  upstream directory as the Apache-2.0 `zap-generated/` output and describe
  the same data model, which is why the table above lists them the same
  way, but neither file states its own licence and none was found stated
  elsewhere in the `nrf` or `modules/lib/matter` trees for these two paths
  specifically. **Flagged rather than asserted.**

**Posture.** *Vendored, source form, unmodified.* This is the second
component in this repository redistributed under a licence other than its
own — the first, `radiant/coex154_ti/ieee802154_cc13xx_cc26xx.c`, is itself
Apache-2.0 and needed no entry here, which is why this is the first "yes" in
the Redistributed column. Every file above is byte-for-byte identical to its
NCS v3.4.0 source; none was reformatted, retagged, or had its header
touched — retagging a vendored file to force an Apache-2.0 SPDX line onto it
would be a false licence claim, which is exactly the failure
`scripts/check_license.py` exists to catch. Its `ALLOWLIST` instead carries
the ten `.h` files above by name, each with the licence it actually carries;
the `.cpp` files fall outside that checker's `SOURCE_SUFFIXES` and need no
entry, a pre-existing gap in the checker recorded in its own comments rather
than a decision about these files.
