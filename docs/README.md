...
| [`backends.md`](backends.md) | The `core` (shipping) / `sdk_ant` (A/B reference) / `stub` radio seam, the two `radiant` radio backends, the USB and UART transports, the per-board build targets, and the optional ANT features each backend can do |
...
| ADR | Records |
|---|---|
| [`0001`](decisions/0001-backend-selection-and-release-default.md) | When `radiant` becomes the release default, and the Tier 3 evidence required first. **Resolved 2026-08-20**: the acceptance passed, release artifacts are cut from `-DANT_RADIO=core`, and the sdk-ant CI job is gone — the switchover was a decision recorded here, not a drift. sdk-ant remains the A/B comparison reference |
