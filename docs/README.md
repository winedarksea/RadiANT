...
| [`sdk-ant-comparison.md`](sdk-ant-comparison.md) | `radiant` against `libant.a` behaviour by behaviour — where the rebuild matches, where it deliberately differs, and the defensive invariants cross-checked against sdk-ant's own burst handler |
