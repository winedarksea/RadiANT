...
| **ANT+ device profile documents** | **Not redistributed, and not committed even as mirrors** | Not freely redistributable, Garmin copyright. Committing a mirror would combine redistribution risk with clean-room contamination in one act: it would put non-redistributable material in the same tree as `radiant_core/**`, whose whole defence is that it never touched it. Listed in `archive/specs/README.md` as references only. |
...
- A `core` or `stub` build carries no `libant.a` and has none of this problem.
  Once `radiant_core` can produce an equivalent readback, that is the image this
