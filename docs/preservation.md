...
| **ANT+ device profile documents** | **Not redistributed, and not committed even as mirrors** | Adopter-gated, Garmin copyright. Committing a mirror would combine redistribution risk with clean-room contamination in one act: it would put adopter-gated material in the same tree as `radiant_core/**`, whose whole defence is that it never touched it. Listed in `archive/specs/README.md` as references only. |
