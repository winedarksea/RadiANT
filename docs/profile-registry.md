...
| `0x14` | Light Electric Vehicle (LEV) | ant-plus-reserved | Garmin / ANT+ | 2025-06-30 | 8192 | no | no | **Implemented** in `src/profiles/profile_lev.c` — pages `0x01`, `0x02`, `0x22`, `0x03`, `0x04`, `0x05`, `0x10` and the request page `0x46`, encode and decode, plus the Table 6-1 travel-mode mapping. Not in `tools/ant_pages.py`. **The common pair rides every 20th channel period, not the 119/120-of-121 cadence** `profile_sched.c` implements, so it takes the client seam as FE-C does. `docs/device-profiles.md` 3.6. **This is why no e-bike type is claimed** |
...
| `0x14` | `0x01` | Speed & System Information 1 | `src/profiles/profile_lev.c` | Main page, 1 Hz. Temperature state, travel mode, system state, gear state, error code, and the 12-bit speed split byte `[6]` plus byte `[7]`'s low nibble |
| `0x14` | `0x02` | Speed & Distance Information | `src/profiles/profile_lev.c` | Main page, 1 Hz. u24 odometer in 0.01 km, 12-bit remaining range in km |
| `0x14` | `0x03` | Speed & System Information 2 | `src/profiles/profile_lev.c` | Main page, 1 Hz. Battery state of charge with an empty-warning bit 7, and % assist whose unknown value is `0xFF` rather than 0 |
| `0x14` | `0x04` | Battery Information | `src/profiles/profile_lev.c` | Optional. **Byte `[3]` finishes two unrelated fields** — bits 3..0 the charge-cycle count, bits 7..4 the fuel consumption |
| `0x14` | `0x05` | LEV Capabilities | `src/profiles/profile_lev.c` | Required of every LEV, sent in the fourth rotation slot or on request. Travel modes supported and wheel circumference |
| `0x14` | `0x06-0x0F` | Reserved | — | The spec's own "reserved for future main data page definitions" |
| `0x14` | `0x10` | Display Data | `src/profiles/profile_lev.c` | **Display → LEV, acknowledged.** Optional to send, mandatory to decode. Gear and light state in a 16-bit command field, travel mode in its own byte |
| `0x14` | `0x22` | Alternative Speed & Distance | `src/profiles/profile_lev.c` | Page 34. Page `0x02` with fuel consumption where the range is; a node sending it must still send `0x02` at least once every 30 s |
| `0x14` | `0x46` | Request Data Page | `src/profiles/profile_lev.c` | Common page 70, display → LEV only, acknowledged. The reply goes out on the next channel period and **resets the 4-page rotation** |
| `0x14` | `0x50` | Common page 80, manufacturer | `ant_pages.encode_common_80` | Byte-exact ANT+; alternates with 81 roughly every 20th channel period |
| `0x14` | `0x51` | Common page 81, product | `ant_pages.encode_common_81` | The other half of the alternation |
