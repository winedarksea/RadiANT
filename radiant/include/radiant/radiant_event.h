 ...
 * buffer reused the instant the callback returns; radiant/tests/fake_radio.c
 ...
 * callback (radiant/tests/src/test_event.c asserts this against a
...
#include <radiant/radiant_frame.h>      /* struct radiant_channel_id */
#include <radiant/radiant_radio_hal.h>  /* radiant_time_t, struct radiant_rx_event */
 ...
 * leak: it is the one place in radiant that knows the shape of the seam
 * above it. radiant_wire.h is where every RADIANT_WIRE_* value below comes
 * from - both module-owned, so this header never has to leave radiant to
 * be complete.
 */
 ...
 * radiant/tests/src/test_event.c; cheap enough to ship.
 * ---------------------------------------------------------------------------
 */
