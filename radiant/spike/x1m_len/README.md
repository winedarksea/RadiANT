...
from `radiant/include/radiant/radiant_radio_hal.h` and
...
2. More decisively, `CONFIG_RADIANT_BACKEND_NRF` in `radiant/Kconfig`
...
`RADIANT_BACKEND_NULL`, and produces an image that boots, runs this loop,
...
`radiant/Kconfig` already records against a Zephyr symbol rename.
...
the null one; `.config` says `CONFIG_RADIANT_BACKEND_NRF=y`.
...
- **Transmit only.** Whether *radiant* can receive its own long frame is a
...
- **No change to shipping code, and that is a result rather than a constraint.**
  ...
  `radiant/src/radiant_radio_nrf.c` requires `RADIANT_LEN_FIXED` on 1 M
...
`CONFIG_RADIANT_BACKEND_NRF=y`) against the Feather in stock ANT firmware
