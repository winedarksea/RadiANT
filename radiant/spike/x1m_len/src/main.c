 ...
 * radiant/include/radiant/radiant_radio_hal.h and radiant_frame.h
 ...
 * and this repo's own radiant/spike/rx_raw/src/main.c for the shape of a
...
/*
 * The one that matters. CONFIG_RADIANT_BACKEND_NRF depends on
 ...
 * RADIANT_BACKEND_NULL, and the image builds, boots, and refuses every
 * arm with ENOTSUP, indistinguishable from a genuine "neither A nor B seen"
 * result. This trap already cost a session once (see radiant/Kconfig,
 * a Zephyr symbol rename); catch it at build time instead.
 */
BUILD_ASSERT(IS_ENABLED(CONFIG_RADIANT_BACKEND_NRF),
	     "This spike transmits. CONFIG_RADIANT_BACKEND_NRF did not "
 ...
 * block is byte-for-byte fmt_tracking's in radiant/src/radiant_frame.c:
...
static const struct radiant_radio_cbs cbs = {
	.rx = on_rx,
	.tx = on_tx,
	.ed = NULL,   /* CONFIG_RADIANT_ED_SCAN is off; the backend must not call it */
};
