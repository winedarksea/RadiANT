...
`radiant/include/radiant/radiant_radio_hal.h` and
`radiant_frame.h`, from `tools/ab_gates.toml`'s header (the
`loss (exact)` rule), and from this repository's own spikes
`radiant/spike/x1m_len` and `radiant/spike/rx_raw`. Nothing here
...
`radiant/spike/x1m_len/src/main.c`. That mode checks **all thirty body
...
`CONFIG_RADIANT_BACKEND_NRF` is `depends on SOC_COMPATIBLE_NRF52X ||
SOC_COMPATIBLE_NRF54LX`, which no nRF53 core sets.

**And the failure is silent.** Kconfig falls back to
`RADIANT_BACKEND_NULL`, and the image boots, accepts every console command
...
```c
BUILD_ASSERT(IS_ENABLED(CONFIG_RADIANT_BACKEND_NRF), …);
```
...
west -z C:\ncs\v3.2.4\zephyr build `
     -s C:\Users\Colin\ant_dongle\radiant\spike\phy_ladder `
...
Both `.config` files say `CONFIG_RADIANT_BACKEND_NRF=y` — **check this
...
owed is binary — a `radiant` receiver read the frame, byte for byte, or it
did not.
...
- **`.config` does not say `CONFIG_RADIANT_BACKEND_NRF=y`.** The null
...
- **nRF52840 errata 191 is not applied by `radiant`.** The nRF52840's RADIO
...
- **No change to shipping code.** Nothing under `radiant/src/`,
  `radiant/include/`, `src/` or any Kconfig was touched; this is a new
