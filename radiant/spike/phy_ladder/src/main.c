 ...
 * where the 1m-long objective comes from), radiant/include/radiant/
 ...
 * radiant_frame_lr_body()), this repo's own radiant/spike/x1m_len/src/
 ...
 * identical. `phy 1m-long` closes that gap: a radiant receiver reading
...
/*
 * Same guard as radiant/spike/x1m_len/src/main.c, and it has already
 * cost this project real bench sessions. CONFIG_RADIANT_BACKEND_NRF
...
BUILD_ASSERT(IS_ENABLED(CONFIG_RADIANT_BACKEND_NRF),
	     "This spike transmits and receives. CONFIG_RADIANT_BACKEND_NRF "
...
static int start(void)
{
	...
	if (cur_role == ROLE_TX && cur_mode == MODE_1M_LONG) {
		printk("refused: `1m-long` is a RECEIVE mode only. Its "
		       "transmitter is radiant/spike/x1m_len, which is "
