 ...
 *
 * The suite for radiant/src/radiant_crc_repair.c. The feature turns a
 ...
 *      RADIANT_CRC_REPAIR's help text, so it is measured here: every
...
ZTEST(radiant_crc_repair, test_three_bit_errors_collide_at_the_predicted_rate)
{
	 ...
	 * ~1 in 341 - the number RADIANT_CRC_REPAIR's help text promises.
	 *
	...
	zassert_true(false_repairs * 100u < (triples / 8u),
		     ...
		     "1 in 100, so RADIANT_CRC_REPAIR's false-accept "
