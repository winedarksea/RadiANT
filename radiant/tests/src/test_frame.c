 ...
 *
 * The suite for radiant/src/radiant_frame.c, which is pure logic with
 ...
 * local work executes them. See radiant/tests/testcase.yaml.
 */
...
ZTEST_SUITE(radiant_frame, NULL, NULL, NULL, NULL, NULL);
...
static const struct radiant_channel_id spike_id = {
...
static const struct ctrl_case ctrl_cases[] = {
	...
	{ 0x0Au, { false, false, false, false, true },
	  RADIANT_MSG_BROADCAST,    "broadcast" },
	{ 0x8Au, { true, false, false, false, true },
	  RADIANT_MSG_BURST_DATA,   "slot-opening burst packet, seq 0" },
	{ 0xAAu, { true, false, true, false, true },
	  RADIANT_MSG_BURST_LAST,   "slot-opening acknowledged data == one-packet burst" },
	/* In-slot data packets. */
	{ 0x82u, { true, false, false, false, false },
	  RADIANT_MSG_BURST_DATA,   "burst packet, seq 0" },
	{ 0x92u, { true, false, false, true, false },
	  RADIANT_MSG_BURST_DATA,   "burst packet, seq 1" },
	{ 0xA2u, { true, false, true, false, false },
	  RADIANT_MSG_BURST_LAST,   "burst last packet, seq 0" },
	{ 0xB2u, { true, false, true, true, false },
	  RADIANT_MSG_BURST_LAST,   "burst last packet, seq 1" },
	/* In-slot acknowledgements. */
	{ 0xC2u, { true, true, false, false, false },
	  RADIANT_MSG_BURST_ACK,    "ack of a non-final packet, next seq 0" },
	{ 0xD2u, { true, true, false, true, false },
	  RADIANT_MSG_BURST_ACK,    "ack of a non-final packet, next seq 1" },
	{ 0xE2u, { true, true, true, false, false },
	  RADIANT_MSG_TRANSFER_ACK, "ack of the final packet, seq 0" },
	{ 0xF2u, { true, true, true, true, false },
	  RADIANT_MSG_TRANSFER_ACK, "ack of the final packet, seq 1" },
};

/* The field sets the rest of this file builds frames with. A master's three. */
static const struct radiant_ctrl_fields ctrl_broadcast = {
	.slot_opener = true,
};
static const struct radiant_ctrl_fields ctrl_ack_data_open = {
	.exchange = true, .last = true, .slot_opener = true,
};
static const struct radiant_ctrl_fields ctrl_burst_open_seq0 = {
...
static const struct radiant_ctrl_fields ctrl_burst_last_seq0 = {
...
ZTEST(radiant_frame, test_crc_golden_vector)
{
	zassert_equal(radiant_crc16(RADIANT_CRC_INIT, golden15, sizeof(golden15)),
		      GOLDEN_CRC,
		      "CRC-16/CCITT-FALSE over the documented golden vector");

	/* And on the real frame, whose CRC the hardware also computed. */
	zassert_equal(radiant_crc16(RADIANT_CRC_INIT, spike15, sizeof(spike15)),
		      SPIKE_CRC,
 ...
 */
ZTEST(radiant_frame, test_crc_over_message_plus_crc_is_zero)
{
	...
	zassert_equal(radiant_crc16(RADIANT_CRC_INIT, buf, sizeof(buf)), 0u,
	...
	buf[7] ^= 0x01u;
	zassert_not_equal(radiant_crc16(RADIANT_CRC_INIT, buf, sizeof(buf)), 0u,
 ...
 */
ZTEST(radiant_frame, test_crc_chaining_constant)
{
	static const uint8_t net[2] = { 0xA6, 0xC5 };

	zassert_equal(radiant_crc16(RADIANT_CRC_INIT, net, sizeof(net)),
		      RADIANT_CRC_AFTER_ANT_PLUS_NET,
		      "the ANT+ network address no longer chains to 0x233E");

	zassert_equal(radiant_crc16(RADIANT_CRC_AFTER_ANT_PLUS_NET, &golden15[2],
				sizeof(golden15) - 2u),
		      GOLDEN_CRC,
		      "chaining from 0x233E over the remaining 13 bytes did not "
		      "reproduce the full-frame CRC");

	zassert_equal(radiant_crc16(RADIANT_CRC_AFTER_ANT_PLUS_NET, &spike15[2],
				sizeof(spike15) - 2u),
		      SPIKE_CRC, "chaining failed on the captured frame");
}

ZTEST(radiant_frame, test_crc_seed_and_empty_input)
{
	zassert_equal(radiant_crc16(RADIANT_CRC_INIT, golden15, 0), RADIANT_CRC_INIT,
		      "zero bytes must leave the seed alone");
	zassert_equal(radiant_crc16(0x1234u, NULL, 8), 0x1234u,
...
ZTEST(radiant_frame, test_rev8)
{
	/* The four in docs/ant-radio-link.md and docs/spike-a-results.md. */
	zassert_equal(radiant_rev8(0xA6u), 0x65u, NULL);
	zassert_equal(radiant_rev8(0xC5u), 0xA3u, NULL);
	zassert_equal(radiant_rev8(0x0Bu), 0xD0u, NULL);
	zassert_equal(radiant_rev8(0x17u), 0xE8u, NULL);
	zassert_equal(radiant_rev8(0x3Au), 0x5Cu, NULL);

	/* Boundaries, where an off-by-one in the mask ladder shows up. */
	zassert_equal(radiant_rev8(0x00u), 0x00u, NULL);
	zassert_equal(radiant_rev8(0xFFu), 0xFFu, NULL);
	zassert_equal(radiant_rev8(0x01u), 0x80u, NULL);
	zassert_equal(radiant_rev8(0x80u), 0x01u, NULL);
	...
	for (unsigned int b = 0; b < 256u; b++) {
		zassert_equal(radiant_rev8(radiant_rev8((uint8_t)b)), (uint8_t)b,
 ...
 */
ZTEST(radiant_frame, test_addr_pack_tracking)
{
	static const uint8_t addr[5] = { 0xA6, 0xC5, 0x17, 0x3A, 0x0B };

	zassert_equal(radiant_addr_pack_leading(addr, sizeof(addr)), 0x5CE8A365u,
		      "leading address word for A6 C5 17 3A 0B");
	zassert_equal(radiant_addr_pack_trailing(addr, sizeof(addr)), 0xD0u,
...
ZTEST(radiant_frame, test_addr_pack_search_is_shifted_not_truncated)
{
	...
	uint32_t lead = radiant_addr_pack_leading(addr, sizeof(addr));
	...
	zassert_equal(radiant_addr_pack_trailing(addr, sizeof(addr)), 0xE8u,
 ...
 */
ZTEST(radiant_frame, test_addr_pack_shift_collapses_at_full_length)
{
	static const uint8_t addr[5] = { 0x11, 0x22, 0x33, 0x44, 0x55 };
	uint32_t expect = ((uint32_t)radiant_rev8(0x44u) << 24) |
			  ((uint32_t)radiant_rev8(0x33u) << 16) |
			  ((uint32_t)radiant_rev8(0x22u) << 8) |
			  (uint32_t)radiant_rev8(0x11u);

	zassert_equal(radiant_addr_pack_leading(addr, sizeof(addr)), expect,
		      "at four leading bytes the shift must vanish");

	/* Three leading bytes: one byte of shift, nothing else moves. */
	zassert_equal(radiant_addr_pack_leading(addr, 4u),
		      (((uint32_t)radiant_rev8(0x33u) << 16) |
		       ((uint32_t)radiant_rev8(0x22u) << 8) |
		       (uint32_t)radiant_rev8(0x11u)) << 8,
		      "a four-byte address shifts by exactly one byte");
}

ZTEST(radiant_frame, test_addr_pack_rejects_bad_length)
{
	static const uint8_t addr[5] = { 0xA6, 0xC5, 0x17, 0x3A, 0x0B };

	/* One byte cannot be split into leading bytes plus a trailing one, and
	 * nothing planned matches more than five. */
	zassert_equal(radiant_addr_pack_leading(addr, 0u), 0u, NULL);
	zassert_equal(radiant_addr_pack_leading(addr, 1u), 0u, NULL);
	zassert_equal(radiant_addr_pack_leading(addr, 6u), 0u, NULL);
	zassert_equal(radiant_addr_pack_leading(NULL, 5u), 0u, NULL);
	zassert_equal(radiant_addr_pack_trailing(addr, 1u), 0u, NULL);
	zassert_equal(radiant_addr_pack_trailing(NULL, 5u), 0u, NULL);
}
 ...
 */
ZTEST(radiant_frame, test_crc_coverage_is_15_in_both_configs)
{
	zassert_equal(radiant_frame_addr_len(RADIANT_FRAME_CFG_TRACKING), 5, NULL);
	zassert_equal(radiant_frame_addr_len(RADIANT_FRAME_CFG_SEARCH), 3, NULL);
	zassert_equal(radiant_frame_body_len(RADIANT_FRAME_CFG_TRACKING, 8), 10, NULL);
	zassert_equal(radiant_frame_body_len(RADIANT_FRAME_CFG_SEARCH, 8), 12, NULL);

	zassert_equal(radiant_frame_covered_len(RADIANT_FRAME_CFG_TRACKING, 8), 15,
		      "tracking no longer covers 15 bytes");
	zassert_equal(radiant_frame_covered_len(RADIANT_FRAME_CFG_SEARCH, 8), 15,
	...
	for (uint8_t p = 0; p <= RADIANT_FRAME_PAYLOAD_MAX; p++) {
		zassert_equal(radiant_frame_covered_len(RADIANT_FRAME_CFG_TRACKING, p),
			      radiant_frame_covered_len(RADIANT_FRAME_CFG_SEARCH, p),
			      "coverage diverged at payload length %u", p);
		zassert_equal(radiant_frame_covered_len(RADIANT_FRAME_CFG_TRACKING, p),
			      7 + (int)p, NULL);
	}

	zassert_equal(radiant_frame_covered_len(RADIANT_FRAME_CFG_TRACKING,
					    RADIANT_FRAME_PAYLOAD_MAX + 1u),
		      RADIANT_FRAME_EINVAL, "an unrepresentable payload must fail");
	zassert_equal(radiant_frame_addr_len((enum radiant_frame_cfg)99),
		      RADIANT_FRAME_EINVAL, NULL);
}
...
{
	const struct radiant_pkt_format *t = radiant_frame_format(RADIANT_FRAME_CFG_TRACKING);
	const struct radiant_pkt_format *s = radiant_frame_format(RADIANT_FRAME_CFG_SEARCH);

	zassert_not_null(t, NULL);
	zassert_not_null(s, NULL);
	zassert_is_null(radiant_frame_format((enum radiant_frame_cfg)99), NULL);

	zassert_equal(t->phy, RADIANT_PHY_1M_GFSK,
		      "an ANT+ compatibility channel may only use the 1M PHY");
	zassert_equal(s->phy, RADIANT_PHY_1M_GFSK, NULL);

	zassert_equal(t->addr_len, 5, NULL);
	zassert_equal(t->len_mode, RADIANT_LEN_FIXED,
	...
	zassert_equal(s->len_mode, RADIANT_LEN_FIXED,
	...
	zassert_equal(t->crc.poly, RADIANT_CRC_POLY,
		      "normal form 0x1021, not the 0x11021 one register wants");
	zassert_equal(t->crc.init, RADIANT_CRC_INIT, NULL);
...
ZTEST(radiant_frame, test_encode_tracking_bytes)
{
	...
	zassert_equal(radiant_frame_make(&f, &spike_id, &ctrl_broadcast,
				     spike_payload, sizeof(spike_payload)),
		      RADIANT_FRAME_OK, NULL);
	zassert_equal(f.ctrl_byte, RADIANT_CTRL_BROADCAST,
		      "a broadcast of 8 payload bytes is 0x0A on air");

	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING,
	...
	zassert_equal(w.body[1], RADIANT_CTRL_BROADCAST, NULL);
	zassert_equal(w.crc, SPIKE_CRC, NULL);

	n = radiant_frame_to_bytes(&w, flat, sizeof(flat));
	...
	zassert_equal(radiant_frame_format(RADIANT_FRAME_CFG_TRACKING)->body_len,
		      w.body_len,
		      "a STATLEN-programmed receiver must expect exactly the "
		      "body this encoder produces");
}

ZTEST(radiant_frame, test_encode_search_bytes)
{
	struct radiant_frame f;
	struct radiant_frame_wire w;
	uint8_t flat[32];
	...
	zassert_equal(radiant_frame_make(&f, &spike_id, &ctrl_broadcast,
				     spike_payload, sizeof(spike_payload)),
		      RADIANT_FRAME_OK, NULL);
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_SEARCH,
				       radiant_net_addr_ant_plus, &f, &w),
		      RADIANT_FRAME_OK, NULL);
	...
	zassert_equal(w.body[3], RADIANT_CTRL_BROADCAST, NULL);
	zassert_mem_equal(&w.body[4], spike_payload, sizeof(spike_payload), NULL);

	zassert_equal(radiant_frame_to_bytes(&w, flat, sizeof(flat)), 17, NULL);
 ...
 */
ZTEST(radiant_frame, test_both_configs_put_the_same_bytes_on_air)
{
	struct radiant_frame f;
	struct radiant_frame_wire wt, ws;
	...
	zassert_equal(radiant_frame_make(&f, &id, &ctrl_broadcast, payload,
				     sizeof(payload)),
		      RADIANT_FRAME_OK, NULL);

	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING,
				       radiant_net_addr_ant_plus, &f, &wt),
		      RADIANT_FRAME_OK, NULL);
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_SEARCH,
				       radiant_net_addr_ant_plus, &f, &ws),
		      RADIANT_FRAME_OK, NULL);
	...
	zassert_equal(radiant_frame_to_bytes(&wt, flat_t, sizeof(flat_t)), 17, NULL);
	zassert_equal(radiant_frame_to_bytes(&ws, flat_s, sizeof(flat_s)), 17, NULL);
...
static void roundtrip(enum radiant_frame_cfg cfg, const struct radiant_channel_id *id,
		      const struct radiant_ctrl_fields *ctrl, const uint8_t *payload,
		      uint8_t payload_len)
{
	struct radiant_frame in, out;
	struct radiant_frame_wire w;

	zassert_equal(radiant_frame_make(&in, id, ctrl, payload, payload_len),
		      RADIANT_FRAME_OK, NULL);
	zassert_equal(radiant_frame_encode(cfg, radiant_net_addr_ant_plus, &in, &w),
		      RADIANT_FRAME_OK, NULL);
	zassert_true(radiant_frame_crc_ok(&w), "encode produced a bad CRC");
	zassert_equal(radiant_frame_decode(cfg, &w, 0, &out), RADIANT_FRAME_OK, NULL);
	...
	zassert_equal(radiant_ctrl_is_exchange(out.ctrl_byte), ctrl->exchange, NULL);
	zassert_equal(radiant_ctrl_is_ack(out.ctrl_byte), ctrl->ack, NULL);
	zassert_equal(radiant_ctrl_is_last(out.ctrl_byte), ctrl->last, NULL);
	zassert_equal(radiant_ctrl_seq(out.ctrl_byte) != 0u, ctrl->seq, NULL);
	zassert_equal(radiant_ctrl_is_slot_opener(out.ctrl_byte), ctrl->slot_opener,
		      NULL);
	zassert_true(radiant_ctrl_low_ok(out.ctrl_byte), NULL);
	zassert_equal(out.payload_len, payload_len, NULL);
	zassert_mem_equal(out.payload, payload, payload_len, NULL);
}

ZTEST(radiant_frame, test_roundtrip_both_configs)
{
	...
	static const struct radiant_channel_id ids[] = {
	...
	for (size_t i = 0; i < ARRAY_SIZE(ids); i++) {
		for (size_t c = 0; c < ARRAY_SIZE(ctrl_cases); c++) {
			roundtrip(RADIANT_FRAME_CFG_TRACKING, &ids[i],
				  &ctrl_cases[c].f, payload, sizeof(payload));
			roundtrip(RADIANT_FRAME_CFG_SEARCH, &ids[i],
				  &ctrl_cases[c].f, payload, sizeof(payload));
		}
	}
}

ZTEST(radiant_frame, test_pairing_bit_is_carried_not_interpreted)
{
	struct radiant_frame f, out;
	struct radiant_frame_wire w;
	const struct radiant_channel_id id = {
		.device_number = SPIKE_DEVNUM,
		.device_type = (uint8_t)(RADIANT_DEVICE_TYPE_PAIRING_BIT | SPIKE_DTYPE),
		.trans_type = SPIKE_TTYPE,
	};

	zassert_equal(radiant_frame_make(&f, &id, &ctrl_broadcast, spike_payload,
				     sizeof(spike_payload)), RADIANT_FRAME_OK, NULL);
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING,
				       radiant_net_addr_ant_plus, &f, &w),
		      RADIANT_FRAME_OK, NULL);
	...
	zassert_equal(radiant_frame_decode(RADIANT_FRAME_CFG_TRACKING, &w, 0, &out),
		      RADIANT_FRAME_OK, NULL);
	zassert_equal(out.id.device_type, id.device_type, NULL);
	zassert_equal(out.id.device_type & RADIANT_DEVICE_TYPE_MASK, SPIKE_DTYPE, NULL);
}
 ...
 */
ZTEST(radiant_frame, test_decode_captured_frame)
{
	uint8_t buf[sizeof(spike15) + 2];
	enum radiant_frame_cfg cfgs[] = { RADIANT_FRAME_CFG_TRACKING, RADIANT_FRAME_CFG_SEARCH };
	...
	for (size_t i = 0; i < ARRAY_SIZE(cfgs); i++) {
		...
		zassert_equal(radiant_frame_from_bytes(cfgs[i], buf, sizeof(buf), &w),
			      17, "the whole frame should have been consumed");
		zassert_equal(w.crc, SPIKE_CRC, NULL);
		zassert_true(radiant_frame_crc_ok(&w), NULL);

		zassert_equal(radiant_frame_decode(cfgs[i], &w, 0, &f),
			      RADIANT_FRAME_OK, NULL);
		...
		zassert_equal(f.ctrl_byte, RADIANT_CTRL_BROADCAST, NULL);
		zassert_equal(radiant_frame_msg_type(f.ctrl_byte), RADIANT_MSG_BROADCAST,
			      NULL);
...
ZTEST(radiant_frame, test_control_byte_low_bits_are_the_only_invariant)
{
	...
	for (size_t c = 0; c < ARRAY_SIZE(ctrl_cases); c++) {
		...
		zassert_equal(radiant_frame_decode(RADIANT_FRAME_CFG_TRACKING, &w, 0,
					       &out),
			      RADIANT_FRAME_OK,
	 ...
	 * CRC. */
	f.ctrl_byte = RADIANT_CTRL_BROADCAST;
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING,
				       radiant_net_addr_ant_plus, &f, &w),
		      RADIANT_FRAME_OK, NULL);
	w.body[1] = 0x0Cu;
	...
	zassert_equal(radiant_frame_msg_type(0x0Cu), RADIANT_MSG_UNKNOWN,
 ...
 * radiant_frame_make() refuses to invent one.
 */
ZTEST(radiant_frame, test_unmeasured_flags_are_carried_not_rejected)
{
	struct radiant_frame f, out;
	struct radiant_frame_wire w;
	struct radiant_ctrl_fields bad = { .ack = true, .last = true };
	...
	f.ctrl_byte = (uint8_t)(0x60u | RADIANT_CTRL_SLOT | RADIANT_CTRL_LOW_VALUE);
	zassert_equal(f.ctrl_byte, 0x6Au, NULL);
	zassert_false(radiant_ctrl_observed(f.ctrl_byte),
	...
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING,
		      ...
		      "encoding on the air without editing radiant_frame.c");
	...
	zassert_equal(radiant_frame_msg_type(out.ctrl_byte), RADIANT_MSG_UNKNOWN,
	...
	/* But radiant_frame_make() must refuse to originate it. That is the line
	 * between "the model can express it" and "the air has carried it". */
	zassert_equal(radiant_frame_make(&f, &spike_id, &bad, spike_payload,
				     sizeof(spike_payload)),
		      RADIANT_FRAME_EINVAL,
		      "radiant_frame_make invented a control byte nothing has "
		      "transmitted");
}

ZTEST(radiant_frame, test_truncated_input_is_rejected)
{
	uint8_t buf[sizeof(spike15) + 2];
	struct radiant_frame_wire w;
	struct radiant_frame f;
	...
	for (size_t n = 0; n < 17u; n++) {
		zassert_equal(radiant_frame_from_bytes(RADIANT_FRAME_CFG_TRACKING, buf,
						   n, &w),
			      RADIANT_FRAME_ETRUNC,
			      "a %zu-byte buffer was accepted as a 17-byte frame",
			      n);
	}
	zassert_equal(radiant_frame_from_bytes(RADIANT_FRAME_CFG_TRACKING, buf, 17u, &w),
		      17, NULL);
	...
	w.addr_len = 5;
	w.body_len = 1;
	zassert_equal(radiant_frame_decode(RADIANT_FRAME_CFG_TRACKING, &w, 0, &f),
		      RADIANT_FRAME_ETRUNC, NULL);

	/* And the output side: a buffer too small must fail rather than write
	 * as much as fits. */
	zassert_equal(radiant_frame_from_bytes(RADIANT_FRAME_CFG_TRACKING, buf, 17u, &w),
		      17, NULL);
	zassert_equal(radiant_frame_to_bytes(&w, small, sizeof(small)),
		      RADIANT_FRAME_ETRUNC, NULL);
}
 ...
 */
ZTEST(radiant_frame, test_crc_failure_is_reported_and_never_repaired)
{
	struct radiant_frame f, out, sentinel;
	struct radiant_frame_wire w;
	...
	w.body[4] ^= 0x01u;
	zassert_false(radiant_frame_crc_ok(&w), NULL);
	...
	zassert_equal(radiant_frame_decode(RADIANT_FRAME_CFG_TRACKING, &w, 0, &out),
		      RADIANT_FRAME_ECRC, NULL);
}

/*
 * RADIANT_FRAME_TRUSTED_CRC is opt-out rather than opt-in, so that forgetting it
 ...
 */
ZTEST(radiant_frame, test_trusted_crc_is_opt_out)
{
	struct radiant_frame f, out;
	struct radiant_frame_wire w;
	...
	w.crc = 0u;
	zassert_equal(radiant_frame_decode(RADIANT_FRAME_CFG_SEARCH, &w, 0, &out),
		      RADIANT_FRAME_ECRC,
		      "the default must verify, so a caller who forgets the "
		      "flag fails safely");
	zassert_equal(radiant_frame_decode(RADIANT_FRAME_CFG_SEARCH, &w,
				       RADIANT_FRAME_TRUSTED_CRC, &out),
		      RADIANT_FRAME_OK, NULL);
	zassert_equal(out.id.device_number, SPIKE_DEVNUM, NULL);
}

ZTEST(radiant_frame, test_null_and_unknown_configuration_are_rejected)
{
	struct radiant_frame f, out;
	struct radiant_frame_wire w;
	uint8_t buf[32];

	zassert_equal(radiant_frame_make(NULL, &spike_id, &ctrl_broadcast,
				     spike_payload, 8),
		      RADIANT_FRAME_EINVAL, NULL);
	zassert_equal(radiant_frame_make(&f, NULL, &ctrl_broadcast,
				     spike_payload, 8),
		      RADIANT_FRAME_EINVAL, NULL);
	zassert_equal(radiant_frame_make(&f, &spike_id, &ctrl_broadcast, NULL, 8),
		      RADIANT_FRAME_EINVAL, NULL);
	zassert_equal(radiant_frame_make(&f, &spike_id, &ctrl_broadcast,
				     spike_payload,
				     (uint8_t)(RADIANT_FRAME_PAYLOAD_MAX + 1u)),
		      RADIANT_FRAME_EINVAL, "an over-long payload must be refused");
	...
	zassert_equal(radiant_frame_make(&f, &spike_id, NULL, spike_payload, 8),
		      RADIANT_FRAME_EINVAL,
		      "radiant_frame_make must refuse to invent a control byte");

	zassert_equal(radiant_frame_make(&f, &spike_id, &ctrl_broadcast,
				     spike_payload, 8),
		      RADIANT_FRAME_OK, NULL);
	zassert_equal(radiant_frame_encode((enum radiant_frame_cfg)99,
				       radiant_net_addr_ant_plus, &f, &w),
		      RADIANT_FRAME_EINVAL, NULL);
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING, NULL, &f, &w),
		      RADIANT_FRAME_EINVAL, NULL);
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING,
				       radiant_net_addr_ant_plus, NULL, &w),
		      RADIANT_FRAME_EINVAL, NULL);
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING,
				       radiant_net_addr_ant_plus, &f, NULL),
		      RADIANT_FRAME_EINVAL, NULL);
	...
	zassert_equal(radiant_frame_decode(RADIANT_FRAME_CFG_TRACKING, NULL, 0, &out),
		      RADIANT_FRAME_EINVAL, NULL);
	zassert_equal(radiant_frame_from_bytes(RADIANT_FRAME_CFG_TRACKING, NULL,
					   sizeof(buf), &w),
		      RADIANT_FRAME_EINVAL, NULL);
	zassert_equal(radiant_frame_to_bytes(NULL, buf, sizeof(buf)),
		      RADIANT_FRAME_EINVAL, NULL);
}
...
ZTEST(radiant_frame, test_flat_parse_ignores_everything_but_bits_2_0)
{
	...
	for (unsigned int flags = 0; flags < 32u; flags++) {
		...
		buf[6] = (uint8_t)((flags << 3) | RADIANT_CTRL_LOW_VALUE);
		crc = radiant_crc16(RADIANT_CRC_INIT, buf, 15u);
	...
	for (size_t t = 0; t < ARRAY_SIZE(flag_sets); t++) {
		for (unsigned int low = 0; low < 8u; low++) {
			if (low == RADIANT_CTRL_LOW_VALUE) {
				continue;
			}
			memcpy(buf, spike15, sizeof(spike15));
			memset(&buf[15], 0, sizeof(buf) - 15u);
			buf[6] = (uint8_t)(flag_sets[t] | low);
			zassert_equal(radiant_frame_from_bytes(RADIANT_FRAME_CFG_TRACKING,
							   buf, sizeof(buf), &w),
				      RADIANT_FRAME_ECTRL,
...
ZTEST(radiant_frame, test_eleven_measured_control_bytes)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(ctrl_cases); i++) {
		...
		struct radiant_ctrl_fields got;

		/* Fields -> byte. */
		zassert_equal(radiant_ctrl_encode(&c->f), c->on_air,
		...
		zassert_equal(radiant_ctrl_decode(c->on_air, &got), RADIANT_FRAME_OK,
			      NULL);
		...
		zassert_equal(radiant_frame_msg_type(c->on_air), c->type,
			      "0x%02X (%s) decoded as the wrong message",
			      c->on_air, c->what);
		zassert_true(radiant_ctrl_observed(c->on_air), NULL);

		/* Measured on all 3,104 CRC-valid frames of both parts. */
		zassert_true(radiant_ctrl_low_ok(c->on_air),
	...
	zassert_equal(radiant_frame_msg_type(0x2Au), RADIANT_MSG_UNKNOWN, NULL);
	zassert_equal(radiant_frame_msg_type(0x4Au), RADIANT_MSG_UNKNOWN, NULL);
	zassert_equal(radiant_frame_msg_type(0x6Au), RADIANT_MSG_UNKNOWN, NULL);
	zassert_false(radiant_ctrl_observed(0x2Au), NULL);
	zassert_false(radiant_ctrl_observed(0x4Au), NULL);
	zassert_false(radiant_ctrl_observed(0x6Au), NULL);
	...
	zassert_false(radiant_ctrl_observed(0xC2u | RADIANT_CTRL_SLOT), NULL); /* 0xCA */
	zassert_false(radiant_ctrl_observed(0xD2u | RADIANT_CTRL_SLOT), NULL); /* 0xDA */
	zassert_false(radiant_ctrl_observed(0xE2u | RADIANT_CTRL_SLOT), NULL); /* 0xEA */
	zassert_false(radiant_ctrl_observed(0xF2u | RADIANT_CTRL_SLOT), NULL); /* 0xFA */
	zassert_false(radiant_ctrl_observed(0x92u | RADIANT_CTRL_SLOT), NULL); /* 0x9A */
	zassert_false(radiant_ctrl_observed(0xB2u | RADIANT_CTRL_SLOT), NULL); /* 0xBA */
}
...
{
	struct radiant_ctrl_fields open, in_slot;

	zassert_equal(radiant_ctrl_decode(0xAAu, &open), RADIANT_FRAME_OK, NULL);
	zassert_equal(radiant_ctrl_decode(0xA2u, &in_slot), RADIANT_FRAME_OK, NULL);
	 ...
	 * its own. */
	zassert_equal(radiant_frame_msg_type(0xAAu), RADIANT_MSG_BURST_LAST, NULL);
	zassert_equal(radiant_frame_msg_type(0xA2u), RADIANT_MSG_BURST_LAST, NULL);
	zassert_equal(radiant_frame_msg_type(0xAAu), radiant_frame_msg_type(0xA2u), NULL);

	/* And the same one bit apart for the burst-packet-0 pair. */
	zassert_equal((uint8_t)(0x82u | RADIANT_CTRL_SLOT), 0x8Au, NULL);
	zassert_equal(radiant_frame_msg_type(0x8Au), RADIANT_MSG_BURST_DATA, NULL);
	zassert_equal(radiant_frame_msg_type(0x82u), RADIANT_MSG_BURST_DATA, NULL);

	/* There is no RADIANT_MSG_ACKNOWLEDGED, and that absence is the finding. */
	zassert_not_equal(radiant_frame_msg_type(0xA2u), RADIANT_MSG_BURST_ACK,
...
ZTEST(radiant_frame, test_reply_frame_relationship)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(pairs); i++) {
		...
		uint8_t r = radiant_ctrl_reply_for(d);
		...
		zassert_true(radiant_ctrl_is_ack(r), "bit 6 must be set");
		zassert_false(radiant_ctrl_is_ack(d), "a data packet has bit 6 clear");
		zassert_equal(radiant_ctrl_is_last(r), radiant_ctrl_is_last(d),
			      "bit 5 is echoed, not complemented");
		zassert_not_equal(radiant_ctrl_seq(r), radiant_ctrl_seq(d),
				  "bit 4 is complemented, not echoed - an ack "
				  "carries the sequence bit it expects NEXT");
		zassert_equal(radiant_ctrl_is_exchange(r), radiant_ctrl_is_exchange(d),
			      NULL);
		zassert_equal(radiant_ctrl_is_slot_opener(r),
			      radiant_ctrl_is_slot_opener(d), NULL);
		zassert_true(radiant_ctrl_observed(r), NULL);
	}
 ...
 */
ZTEST(radiant_frame, test_same_payload_two_control_bytes)
{
	struct radiant_frame ack, bcast, out;
	struct radiant_frame_wire w_ack, w_bcast;
	uint8_t flat_ack[32], flat_bcast[32];
	unsigned int differing = 0;

	zassert_equal(radiant_frame_make(&ack, &spike_id, &ctrl_ack_data_open,
				     spike_b_payload, sizeof(spike_b_payload)),
		      RADIANT_FRAME_OK, NULL);
	zassert_equal(radiant_frame_make(&bcast, &spike_id, &ctrl_broadcast,
				     spike_b_payload, sizeof(spike_b_payload)),
		      RADIANT_FRAME_OK, NULL);
	...
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING,
				       radiant_net_addr_ant_plus, &ack, &w_ack),
		      RADIANT_FRAME_OK, NULL);
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING,
				       radiant_net_addr_ant_plus, &bcast, &w_bcast),
		      RADIANT_FRAME_OK, NULL);

	zassert_equal(w_ack.body_len, w_bcast.body_len, NULL);
	zassert_equal(radiant_frame_to_bytes(&w_ack, flat_ack, sizeof(flat_ack)),
		      17, NULL);
	zassert_equal(radiant_frame_to_bytes(&w_bcast, flat_bcast,
	...
	zassert_equal(radiant_frame_decode(RADIANT_FRAME_CFG_TRACKING, &w_ack, 0, &out),
		      RADIANT_FRAME_OK,
		      "an acknowledged frame must decode - a receiver that "
		      "drops these cannot set trainer resistance");
	zassert_equal(radiant_frame_msg_type(out.ctrl_byte), RADIANT_MSG_BURST_LAST,
	...
	zassert_equal(radiant_frame_decode(RADIANT_FRAME_CFG_TRACKING, &w_bcast, 0,
				       &out), RADIANT_FRAME_OK, NULL);
	zassert_equal(radiant_frame_msg_type(out.ctrl_byte), RADIANT_MSG_BROADCAST,
		      NULL);
	zassert_mem_equal(out.payload, spike_b_payload,
			  sizeof(spike_b_payload), NULL);
}
...
ZTEST(radiant_frame, test_a_slave_frame_is_not_rejected)
{
	...
	zassert_equal(radiant_frame_make(&f, &spike_id, &ctrl_burst_last_seq0,
				     spike_b_payload, sizeof(spike_b_payload)),
		      RADIANT_FRAME_OK,
		      "an in-slot acknowledged-data frame was refused");
	zassert_equal(f.ctrl_byte, 0xA2u, NULL);
	zassert_false(radiant_ctrl_is_slot_opener(f.ctrl_byte), NULL);
	...
	zassert_equal(radiant_frame_encode(RADIANT_FRAME_CFG_TRACKING,
				       radiant_net_addr_ant_plus, &f, &w),
		      RADIANT_FRAME_OK, "encode rejected a valid slave frame");
	zassert_equal(w.body[1], 0xA2u, NULL);
	zassert_equal(radiant_frame_to_bytes(&w, flat, sizeof(flat)), 17, NULL);

	zassert_equal(radiant_frame_from_bytes(RADIANT_FRAME_CFG_TRACKING, flat,
					   sizeof(flat), &w),
		      17, "the flat parser rejected a valid slave frame");
	zassert_equal(radiant_frame_decode(RADIANT_FRAME_CFG_TRACKING, &w, 0, &out),
		      RADIANT_FRAME_OK, "decode rejected a valid slave frame");
	...
	zassert_equal(radiant_ctrl_reply_for(out.ctrl_byte), 0xF2u, NULL);
}
 ...
 * accident.
 */
ZTEST(radiant_frame, test_flat_parse_accepts_every_measured_control_byte)
{
	uint8_t buf[64];
...
ZTEST(radiant_frame, test_long_payload_arithmetic_and_the_format_limit)
{
	...
	zassert_equal(radiant_frame_make(&f, &spike_id, &ctrl_burst_open_seq0,
				     payload, sizeof(payload)),
		      RADIANT_FRAME_OK, NULL);
	zassert_equal(radiant_frame_make(&f8, &spike_id, &ctrl_burst_open_seq0,
				     payload, 8u),
		      RADIANT_FRAME_OK, NULL);
	...
	zassert_equal(radiant_frame_msg_type(f.ctrl_byte), RADIANT_MSG_BURST_DATA, NULL);
	...
	zassert_equal(radiant_frame_covered_len(RADIANT_FRAME_CFG_TRACKING, 24), 31, NULL);
	zassert_equal(radiant_frame_covered_len(RADIANT_FRAME_CFG_SEARCH, 24), 31, NULL);
	zassert_equal(wt.crc, ws.crc, NULL);

	zassert_equal(radiant_frame_to_bytes(&wt, flat_t, sizeof(flat_t)), 33, NULL);
	zassert_equal(radiant_frame_to_bytes(&ws, flat_s, sizeof(flat_s)), 33, NULL);
	...
	roundtrip(RADIANT_FRAME_CFG_TRACKING, &spike_id, &ctrl_burst_open_seq0,
		  payload, sizeof(payload));
	roundtrip(RADIANT_FRAME_CFG_SEARCH, &spike_id, &ctrl_burst_open_seq0,
		  payload, sizeof(payload));
	...
	zassert_equal(radiant_frame_format(RADIANT_FRAME_CFG_TRACKING)->max_body_len, 10,
		      "tracking is STATLEN=10; a longer frame needs its own "
		      "format and a measurement to justify it");
	zassert_equal(radiant_frame_format(RADIANT_FRAME_CFG_SEARCH)->max_body_len, 12,
		      "search cannot receive a longer-than-standard frame, and "
		      "the format must keep saying so");
	zassert_true(wt.body_len >
			     radiant_frame_format(RADIANT_FRAME_CFG_TRACKING)->max_body_len,
