...
`docs/spike-a-results.md`, this repository's own `radiant_core/spike/rx_raw`, the
...
Push-Location C:\ncs\v3.2.4
west -z C:\ncs\v3.2.4\zephyr build -s C:\Users\Colin\ant_dongle\radiant_core\spike\promisc `
...
`radiant_core/spike/rx_raw/README.md` apply unchanged and all of them still matter:
...
```powershell
& C:\ncs\toolchains\dcbdc366a1\opt\bin\python.exe radiant_core\spike\promisc\spike_b_drive.py `
...
. .\scripts\env.ps1 -NcsVersion v3.2.4
Push-Location C:\ncs\v3.2.4
west -z C:\ncs\v3.2.4\zephyr build -s <repo>\radiant_core\spike\promisc `
     -d <repo>\build\promisc_net2 -b nrf5340dk/nrf5340/cpunet -p always
west -z C:\ncs\v3.2.4\zephyr build -s <repo>\radiant_core\spike\promisc\appcore `
...
- **`radiant_core` proper links none of this.** A standalone Zephyr application
