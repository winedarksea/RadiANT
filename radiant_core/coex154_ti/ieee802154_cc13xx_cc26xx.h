...
struct ieee802154_cc13xx_cc26xx_data {
	...
	volatile rfc_CMD_IEEE_ED_SCAN_t cmd_ieee_ed_scan;
	energy_scan_done_cb_t ed_scan_done_cb;
	...
	/*
	 * RADIANT FORK. Whether the background CMD_IEEE_RX is meant to be on the
	 * air right now, as opposed to having been torn down on purpose.
	 *
	 * The re-post in cmd_ieee_rx_callback() (see the .c) cannot tell a
	 * preemption by the ANT client apart from an abort the driver ITSELF
	 * asked for: ieee802154_cc13xx_cc26xx_stop() flushes with
	 * RF_CMDHANDLE_FLUSH_ALL, which delivers exactly the same
	 * Aborted/Cancelled events. Without this flag, stop() would be undone
	 * by its own callback, and do_set_channel() - which stops and then
	 * re-posts - would end up with two background receives queued.
	 */
