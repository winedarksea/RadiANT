...
/*
 * ── VENDORED FORK. Provenance and the one deliberate change ─────────────────
 *
 * Verbatim copy of zephyr/drivers/ieee802154/ieee802154_cc13xx_cc26xx.c from
 * NCS v3.4.0, plus its private header beside this file, with four edits and no
 * others - every one of them marked RADIANT FORK:
 *
 *   1. this comment block,
 *   2. the CONFIG_* fallback shims below,
 *   3. the re-post branch in cmd_ieee_rx_callback(),
 *   4. the `rx_active` flag it needs - one field in the header, set at the
 *      two places that post the background receive and cleared in stop().
 *
 * Diff it against the in-tree file before trusting anything else written here;
 * a fork that has silently drifted is worse than no fork.
 *
 * WHY IT EXISTS. Under CONFIG_RADIANT_CORE_BACKEND_CC26XX_COEX the ANT link
 * layer becomes a second RF-core client and its scheduler policy
 * (src/radiant_radio_cc26xx_arb.c) returns RF_ExecuteActionAbortOngoing to
 * displace this driver's background CMD_IEEE_RX for a tracked slot or a
 * transmit. The upstream driver has no re-post path for that command -
 ...
 * permanently. A coexistence run against the unmodified driver would report
 * "ANT healthy, neighbour silently dead, EXIT=0", which is the exact failure
 * class docs/decisions/0013 was written to prevent. The fix is a survival
 * fix, not a scheduling one: see docs/decisions/0015.
 *
 * WHY A FORK AND NOT A PATCH. Zephyr compiles the original from
 * drivers/ieee802154/CMakeLists.txt under CONFIG_IEEE802154_CC13XX_CC26XX with
 * no seam to exclude one file, and both files register NET_DEVICE against the
 * same DT_DRV_COMPAT node. So ti_coex.conf sets that symbol to n and turns
 * this one on instead - exactly one driver compiles, one NET_DEVICE is
 * registered, and the devicetree node is untouched either way. Turning the
 * upstream symbol off is REQUIRED rather than tidy for a second reason:
 * radiant_core/CMakeLists.txt compiles driverlib/rfc.c itself (hal_ti compiles
 * it only under that same symbol), so leaving it y would be a duplicate-symbol
 * link error.
 *
 * WHAT IS VERIFIED. That this compiles and links as arm 4, and that the
 * re-post mirrors the driver's own post at do_set_channel(). NOT verified: any
 * behaviour under real contention. There is no 802.15.4 peer on this bench, so
 * nothing here has received a frame after a preemption. Do not read a
 * coexistence number off this arm without one.
 */

#define DT_DRV_COMPAT ti_cc13xx_cc26xx_ieee802154

/*
 * The upstream sub-symbols all `depends on IEEE802154_CC13XX_CC26XX`, which
 * this build sets to n (see above), so they do not exist. Mirrored here at
 * their upstream defaults rather than duplicated into radiant_core/Kconfig:
 * the values are the vendor driver's business, and a second Kconfig copy is a
 * second thing to drift. #ifndef so that a build which somehow does have them
 * keeps the real ones.
 */
#ifndef CONFIG_IEEE802154_CC13XX_CC26XX_INIT_PRIO
#define CONFIG_IEEE802154_CC13XX_CC26XX_INIT_PRIO 80
#endif
#ifndef CONFIG_IEEE802154_CC13XX_CC26XX_RADIO_TX_RETRIES
#define CONFIG_IEEE802154_CC13XX_CC26XX_RADIO_TX_RETRIES 3
#endif
#ifndef CONFIG_IEEE802154_CC13XX_CC26XX_RADIO_CSMA_CA_MAX_BO
#define CONFIG_IEEE802154_CC13XX_CC26XX_RADIO_CSMA_CA_MAX_BO 4
#endif
#ifndef CONFIG_IEEE802154_CC13XX_CC26XX_RADIO_CSMA_CA_MIN_BE
#define CONFIG_IEEE802154_CC13XX_CC26XX_RADIO_CSMA_CA_MIN_BE 3
#endif
#ifndef CONFIG_IEEE802154_CC13XX_CC26XX_RADIO_CSMA_CA_MAX_BE
#define CONFIG_IEEE802154_CC13XX_CC26XX_RADIO_CSMA_CA_MAX_BE 5
#endif
#ifndef CONFIG_IEEE802154_DRIVER_LOG_LEVEL
#define CONFIG_IEEE802154_DRIVER_LOG_LEVEL 0
#endif
...
static void cmd_ieee_rx_callback(RF_Handle h, RF_CmdHandle ch, RF_EventMask e)
{
	...
	/*
	 * ── RADIANT FORK: the whole reason this file exists ─────────────────
	 *
	 * Upstream stops here, and upstream is right to: with one RF client
	 * nothing ever displaces the background receive, so there is nothing to
	 * recover from. Under CONFIG_RADIANT_CORE_BACKEND_CC26XX_COEX there is
	 * a second client, and radiant_cc26xx_execute() returns
	 * RF_ExecuteActionAbortOngoing to take the core for a tracked ANT slot
	 * or a transmit. Without this branch that first preemption is the last
	 * 802.15.4 frame this driver ever sees - silently, with no error
	 * anywhere and a network interface that still reports itself up.
	 *
	 * The three events are exactly what that abort produces (RF_abortCmd
	 * raises Cancelled and Preempted together; a running command also
	 * raises Aborted), so this is not a catch-all: RF_EventLastCmdDone and
	 * RF_EventCmdStopped are deliberately NOT here, because a background
	 * receive that ended on its own terms did not lose a fight.
	 *
	 * status must be reset first - the command struct is not otherwise
	 * reset between posts, and the RF driver refuses to run one that is not
	 * IDLE. RF_postCmd from a callback is legal (it is how the driver's own
	 * TX chain re-arms), and RF_postCmd rather than RF_scheduleCmd on
	 * purpose: this is the neighbour's own background receive going back on
	 * the air with no time constraint, and our arbiter is what decides
	 * whether it gets to stay there.
	 */
	if ((e & (RF_EventCmdAborted | RF_EventCmdCancelled |
		  RF_EventCmdPreempted)) && drv_data->rx_active) {
		...
		if (cmd_handle < 0) {
			/*
			 * Loud, and then give up rather than spin: a re-post
			 * that fails from inside its own failure callback would
			 * recurse. The interface is now deaf, which is exactly
			 * the state a coexistence gate has to be able to see.
			 */
			LOG_ERR("Failed to re-post RX after preemption (%d)",
				cmd_handle);
			drv_data->rx_active = false;
		} else {
						...
						uint16_t channel, bool force)
{
	...
	drv_data->rx_handle = cmd_handle;
	drv_data->rx_active = true; /* RADIANT FORK - see the header's field */
...
static int ieee802154_cc13xx_cc26xx_stop(const struct device *dev)
{
	...
	/*
	 * RADIANT FORK. BEFORE the flush, not after: the flush's callbacks can
	 * run before RF_flushCmd() returns, and a re-post that slipped through
	 * in that window would survive the very stop that caused it.
	 */
	drv_data->rx_active = false;
